...
2024-04-09 - dzentota - Added support of modifiers for template variables
...
class XTemplate
{
    ...
    */
    ...
    private array $modifiers = ['ESCAPE_HTML', 'ESCAPE_HTML_ATTR', 'ESCAPE_JS', 'ESCAPE_URL', 'ESCAPE_CSS', 'SANITIZE_HTML'];
            ...
            if ($sub[0] == "_BLOCK_") {
                ...
                foreach ($sub as $k1 => $v1) {
                    if ($v1 === $this->csrfVar) {
                        ...
                        $var = $var[$v1];
                    } elseif (in_array($v1, $this->modifiers, true)) {
                        $templateVariable = ($var instanceof TemplateVariable)? $var : new TemplateVariable($var);
                        switch ($v1) {
                            case 'ESCAPE_HTML':
                                $var = $templateVariable->escapeHtml();
                                break;
                            case 'ESCAPE_HTML_ATTR':
                                $var = $templateVariable->escapeHtmlAttr();
                                break;
                            case 'ESCAPE_JS':
                                $var = $templateVariable->escapeJs();
                                break;
                            case 'ESCAPE_URL':
                                $var = $templateVariable->escapeUrl();
                                break;
                            case 'ESCAPE_CSS':
                                $var = $templateVariable->escapeCss();
                                break;
                            case 'SANITIZE_HTML':
                                $var = $templateVariable->sanitizeHtml();
                                break;
                        }
                    } else {
                        $var = null;
