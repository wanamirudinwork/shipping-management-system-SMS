...
class ObjectType implements \PHPStan\Type\TypeWithClassName, \PHPStan\Type\SubtractableType
{
     ...
     * @var ?string
     */
    private $cachedDescription = null;
    /** @var array<string, list<EnumCaseObjectType>> */
    private static $enumCases = [];
    /** @api */
    ...
    public function isSuperTypeOfWithReason(\PHPStan\Type\Type $type) : \PHPStan\Type\IsSuperTypeOfResult
    {
        ...
        $transformResult = static function (\PHPStan\Type\IsSuperTypeOfResult $result) {
            return $result;
        };
        ...
        if ($thatClassNames[0] === $thisClassName) {
            return $transformResult(\PHPStan\Type\IsSuperTypeOfResult::createYes());
        }
    ...
    public function toArray() : \PHPStan\Type\Type
    {
        ...
        $reflectionProvider = ReflectionProviderStaticAccessor::getInstance();
        if (!$classReflection->getNativeReflection()->isUserDefined() || $classReflection->is(ArrayObject::class) || UniversalObjectCratesClassReflectionExtension::isUniversalObjectCrate($reflectionProvider, Broker::getInstance()->getUniversalObjectCratesClasses(), $classReflection)) {
            return new \PHPStan\Type\ArrayType(new \PHPStan\Type\MixedType(), new \PHPStan\Type\MixedType());
        }
        $arrayKeys = [];
        $arrayValues = [];
    ...
    public function getIterableKeyType() : \PHPStan\Type\Type
    {
        ...
        if ($this->isInstanceOf(IteratorAggregate::class)->yes()) {
            $keyType = \PHPStan\Type\RecursionGuard::run($this, function () : \PHPStan\Type\Type {
                return $this->getMethod('getIterator', new OutOfClassScope())->getOnlyVariant()->getReturnType()->getIterableKeyType();
            });
            $isTraversable = \true;
        ...
        if ($this->isInstanceOf(Iterator::class)->yes()) {
            return \PHPStan\Type\RecursionGuard::run($this, function () : \PHPStan\Type\Type {
                return $this->getMethod('key', new OutOfClassScope())->getOnlyVariant()->getReturnType();
            });
        }
    ...
    public function getIterableValueType() : \PHPStan\Type\Type
    {
        ...
        if ($this->isInstanceOf(IteratorAggregate::class)->yes()) {
            $valueType = \PHPStan\Type\RecursionGuard::run($this, function () : \PHPStan\Type\Type {
                return $this->getMethod('getIterator', new OutOfClassScope())->getOnlyVariant()->getReturnType()->getIterableValueType();
            });
            $isTraversable = \true;
        ...
        if ($this->isInstanceOf(Iterator::class)->yes()) {
            return \PHPStan\Type\RecursionGuard::run($this, function () : \PHPStan\Type\Type {
                return $this->getMethod('current', new OutOfClassScope())->getOnlyVariant()->getReturnType();
            });
        }
        if ($extraOffsetAccessible) {
            return new \PHPStan\Type\MixedType(\true);
    ...
    public function getOffsetValueType(\PHPStan\Type\Type $offsetType) : \PHPStan\Type\Type
    {
        ...
        if ($this->isInstanceOf(ArrayAccess::class)->yes()) {
            return \PHPStan\Type\RecursionGuard::run($this, function () : \PHPStan\Type\Type {
                return $this->getMethod('offsetGet', new OutOfClassScope())->getOnlyVariant()->getReturnType();
            });
        }
        return new \PHPStan\Type\ErrorType();
    }
    ...
    {
        $parametersAcceptors = \PHPStan\Type\RecursionGuard::run($this, function () {
            return $this->findCallableParametersAcceptors();
        });
