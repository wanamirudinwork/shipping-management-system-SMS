...
class ConstantArrayType extends ArrayType implements ConstantType
{
     ...
     * @param bool|TrinaryLogic $isList
     */
    ...
    public function acceptsWithReason(Type $type, bool $strictTypes) : AcceptsResult
    {
        ...
        foreach ($this->keyTypes as $i => $keyType) {
            ...
            $acceptsValue = $valueType->acceptsWithReason($otherValueType, $strictTypes)->decorateReasons(static function (string $reason) use($keyType, $valueType, $verbosity, $otherValueType) {
                return sprintf('Offset %s (%s) does not accept type %s: %s', $keyType->describe(VerbosityLevel::precise()), $valueType->describe($verbosity), $otherValueType->describe($verbosity), $reason);
            });
        ...
        if ($type instanceof self) {
            ...
            foreach ($this->keyTypes as $i => $keyType) {
                ...
                if ($isValueSuperType->no()) {
                    return $isValueSuperType->decorateReasons(static function (string $reason) use($keyType) {
                        return sprintf('Offset %s: %s', $keyType->describe(VerbosityLevel::value()), $reason);
                    });
                }
    ...
    public function sliceArray(Type $offsetType, Type $lengthType, TrinaryLogic $preserveKeys) : Type
    {
        ...
        $offset = $offsetType instanceof \PHPStan\Type\Constant\ConstantIntegerType ? $offsetType->getValue() : 0;
        $length = $lengthType instanceof \PHPStan\Type\Constant\ConstantIntegerType ? $lengthType->getValue() : $keyTypesCount;
        if ($length < 0) {
    ...
    {
        $array = $this->sliceArray(ConstantTypeHelper::getTypeFromValue($offset), ConstantTypeHelper::getTypeFromValue($limit), TrinaryLogic::createFromBoolean($preserveKeys));
        if (!$array instanceof self) {
            throw new ShouldNotHappenException();
        }
        return $array;
    ...
    public function generalize(GeneralizePrecision $precision) : Type
    {
        ...
        $arrayType = new ArrayType($this->getIterableKeyType()->generalize($precision), $this->getItemType()->generalize($precision));
        $keyTypesCount = count($this->keyTypes);
        $optionalKeysCount = count($this->optionalKeys);
        $accessoryTypes = [];
