    ...
    public function specifyTypes(FunctionReflection $functionReflection, FuncCall $node, Scope $scope, TypeSpecifierContext $context) : SpecifiedTypes
    {
        ...
        if (!$isStrictComparison) {
            if ($context->true() && $arrayType->isArray()->yes() && $arrayType->getIterableValueType()->isSuperTypeOf($needleType)->yes()) {
                return $this->typeSpecifier->create($node->getArgs()[1]->value, TypeCombinator::intersect($arrayType, new NonEmptyArrayType()), $context, \false, $scope);
            }
            return new SpecifiedTypes();
        }
        ...
        if ($context->true() || $context->false() && (count(TypeUtils::getConstantScalars($arrayValueType)) > 0 || count(TypeUtils::getEnumCaseObjects($arrayValueType)) > 0)) {
            $specifiedTypes = $this->typeSpecifier->create($needleExpr, $arrayValueType, $context, \false, $scope);
            if ($needleExpr instanceof AlwaysRememberedExpr) {
                $specifiedTypes = $specifiedTypes->unionWith($this->typeSpecifier->create($needleExpr->getExpr(), $arrayValueType, $context, \false, $scope));
            }
        }
