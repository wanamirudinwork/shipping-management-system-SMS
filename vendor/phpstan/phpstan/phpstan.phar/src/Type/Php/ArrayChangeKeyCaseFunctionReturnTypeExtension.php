    ...
    public function getTypeFromFunctionCall(FunctionReflection $functionReflection, FuncCall $functionCall, Scope $scope) : ?Type
    {
        ...
        if (count($constantArrays) > 0) {
            ...
            foreach ($constantArrays as $constantArray) {
                ...
                foreach ($constantArray->getKeyTypes() as $i => $keyType) {
                    ...
                    if (count($constantStrings) > 0) {
                        $keyType = TypeCombinator::union(...array_map(function (ConstantStringType $type) use($case) : Type {
            ...
            $keysType = TypeTraverser::map($keysType, function (Type $type, callable $traverse) use($case) : Type {
                ...
                if (count($constantStrings) > 0) {
                    return TypeCombinator::union(...array_map(function (ConstantStringType $type) use($case) : Type {
                        return $this->mapConstantString($type, $case);
                    }, $constantStrings));
                }
