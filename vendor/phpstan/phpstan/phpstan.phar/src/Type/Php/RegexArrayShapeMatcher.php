...
final class RegexArrayShapeMatcher
{
     ...
     */
    private $regexGroupParser;
    /**
     * @var RegexExpressionHelper
     */
    private $regexExpressionHelper;
    /**
     * @var PhpVersion
     */
    private $phpVersion;
    ...
    private function matchRegex(string $regex, ?int $flags, TrinaryLogic $wasMatched, bool $matchesAll) : ?Type
    {
        ...
        if (!$matchesAll && $wasMatched->yes() && $onlyOptionalTopLevelGroup !== null) {
            ...
            $combiType = $this->buildArrayType($groupList, $wasMatched, $trailingOptionals, $flags, $markVerbs, $matchesAll);
            if (!$this->containsUnmatchedAsNull($flags, $matchesAll)) {
                // positive match has a subject but not any capturing group
                $combiType = TypeCombinator::union(new ConstantArrayType([new ConstantIntegerType(0)], [$this->createSubjectValueType($flags, $matchesAll)], [1], [], \true), $combiType);
            }
            ...
            foreach ($onlyTopLevelAlternation->getGroupCombinations() as $groupCombo) {
                ...
                foreach ($comboList as $groupId => $group) {
                    if (in_array($groupId, $groupCombo, \true)) {
                        ...
                        $group->forceNonOptional();
                        $group->forceType($this->containsUnmatchedAsNull($flags, $matchesAll) ? new NullType() : new ConstantStringType(''));
                ...
                $combiType = $this->buildArrayType($comboList, $wasMatched, $trailingOptionals, $flags, $markVerbs, $matchesAll);
            ...
            if (!$this->containsUnmatchedAsNull($flags, $matchesAll) && ($onlyTopLevelAlternation->getAlternationsCount() !== count($onlyTopLevelAlternation->getGroupCombinations()) || $isOptionalAlternation)) {
                // positive match has a subject but not any capturing group
                $combiTypes[] = new ConstantArrayType([new ConstantIntegerType(0)], [$this->createSubjectValueType($flags, $matchesAll)], [1], [], \true);
            }
        ...
        return $this->buildArrayType($groupList, $wasMatched, $trailingOptionals, $flags, $markVerbs, $matchesAll);
    }
    /**
     ...
     */
    private function buildArrayType(array $captureGroups, TrinaryLogic $wasMatched, int $trailingOptionals, int $flags, array $markVerbs, bool $matchesAll) : Type
    {
        $builder = ConstantArrayTypeBuilder::createEmpty();
        // first item in matches contains the overall match.
        $builder->setOffsetValueType($this->getKeyType(0), $this->createSubjectValueType($flags, $matchesAll), $this->isSubjectOptional($wasMatched, $matchesAll));
        ...
        if ($matchesAll && $this->containsSetOrder($flags)) {
            ...
            if (!$wasMatched->yes()) {
                $arrayType = TypeCombinator::union(new ConstantArrayType([], []), $arrayType);
            }
            return $arrayType;
        }
