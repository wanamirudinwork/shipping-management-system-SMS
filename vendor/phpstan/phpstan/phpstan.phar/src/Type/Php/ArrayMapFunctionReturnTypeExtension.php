...
final class ArrayMapFunctionReturnTypeExtension implements DynamicFunctionReturnTypeExtension
{
    ...
    public function getTypeFromFunctionCall(FunctionReflection $functionReflection, FuncCall $functionCall, Scope $scope) : ?Type
    {
        ...
        if ($callableType->isCallable()->yes()) {
            ...
            $valueType = ParametersAcceptorSelector::selectFromTypes(array_map(static function (Node\Arg $arg) use($scope) {
                return $scope->getType($arg->value)->getIterableValueType();
            }, array_slice($functionCall->getArgs(), 1)), $callableParametersAcceptors, \false)->getReturnType();
            ...
            if (!$areAllSameSize) {
                ...
                $and = array_reduce($identities, static function (Node\Expr $a, Node\Expr $b) {
                    return new Node\Expr\BinaryOp\BooleanAnd($a, $b);
        ...
        if ($singleArrayArgument) {
            ...
            $mappedArrayType = AccessoryArrayListType::intersectWith(TypeCombinator::intersect(new ArrayType(new IntegerType(), $valueType), ...TypeUtils::getAccessoryTypes($arrayType)));
        }
