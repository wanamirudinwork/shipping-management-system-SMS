...
final class ValidateExcludePathsExtension extends CompilerExtension
{
    ...
    public function loadConfiguration() : void
    {
        ...
        foreach ($newExcludePaths as $key => $p) {
            $newExcludePaths[$key] = array_map(static function ($path) {
                return $path instanceof OptionalPath ? $path->path : $path;
            }, $p);
        }
        ...
        throw new \PHPStan\DependencyInjection\InvalidExcludePathsException($errors);
    }
}
