...
final class PhpClassReflectionExtension implements PropertiesClassReflectionExtension, MethodsClassReflectionExtension
{
    ...
    public function getNativeMethod(ClassReflection $classReflection, string $methodName) : ExtendedMethodReflection
    {
        ...
        $nativeMethodReflection = new \PHPStan\Reflection\Php\NativeBuiltinMethodReflection($reflectionMethod);
    ...
    private function createMethod(ClassReflection $classReflection, \PHPStan\Reflection\Php\BuiltinMethodReflection $methodReflection, bool $includingAnnotations) : ExtendedMethodReflection
    {
        ...
        if ($this->signatureMapProvider->hasMethodSignature($declaringClassName, $methodReflection->getName())) {
            ...
            foreach ($methodSignaturesResult as $signatureType => $methodSignatures) {
                ...
                foreach ($methodSignatures as $methodSignature) {
                    ...
                    if (count($methodSignatures) === 1) {
                        $stubPhpDocPair = $this->findMethodPhpDocIncludingAncestors($declaringClass, $declaringClass, $methodReflection->getName(), array_map(static function (ParameterSignature $parameterSignature) : string {
                        ...
                        if ($stubPhpDocPair !== null) {
                            ...
                            $stubImmediatelyInvokedCallableParameters = array_map(static function (bool $immediate) {
                                return TrinaryLogic::createFromBoolean($immediate);
                            }, $stubPhpDoc->getParamsImmediatelyInvokedCallable());
                            if ($returnTag !== null) {
                            ...
                            $stubClosureThisParameters = array_map(static function ($tag) {
                                return $tag->getType();
                            }, $stubPhpDoc->getParamClosureThisTags());
                    ...
                    if ($stubPhpDocPair === null && $reflectionMethod !== null && $reflectionMethod->getDocComment() !== \false) {
                        ...
                        if ($filename !== \false) {
                            ...
                            $immediatelyInvokedCallableParameters = array_map(static function ($immediate) {
                                return TrinaryLogic::createFromBoolean($immediate);
                            }, $phpDocBlock->getParamsImmediatelyInvokedCallable());
                            $closureThisParameters = array_map(static function ($tag) {
                                return $tag->getType();
                            }, $phpDocBlock->getParamClosureThisTags());
            ...
            return new NativeMethodReflection($this->reflectionProviderProvider->getReflectionProvider(), $declaringClass, $methodReflection, $variantsByType['positional'], $variantsByType['named'] ?? null, $hasSideEffects, $throwType, $asserts, $acceptsNamedArguments, $selfOutType, $phpDocComment);
        }
    ...
    public function createUserlandMethodReflection(ClassReflection $fileDeclaringClass, ClassReflection $actualDeclaringClass, \PHPStan\Reflection\Php\BuiltinMethodReflection $methodReflection, ?string $declaringTraitName) : \PHPStan\Reflection\Php\PhpMethodReflection
    {
        $resolvedPhpDoc = null;
        $stubPhpDocPair = $this->findMethodPhpDocIncludingAncestors($fileDeclaringClass, $fileDeclaringClass, $methodReflection->getName(), array_map(static function (ReflectionParameter $parameter) : string {
            return $parameter->getName();
        ...
        $immediatelyInvokedCallableParameters = array_map(static function (bool $immediate) {
            return TrinaryLogic::createFromBoolean($immediate);
        }, $resolvedPhpDoc->getParamsImmediatelyInvokedCallable());
        $closureThisParameters = array_map(static function ($tag) {
            return $tag->getType();
        ...
        return $this->methodReflectionFactory->create($actualDeclaringClass, $declaringTrait, $methodReflection, $templateTypeMap, $phpDocParameterTypes, $phpDocReturnType, $phpDocThrowType, $deprecatedDescription, $isDeprecated, $isInternal, $isFinal, $isPure, $asserts, $selfOutType, $phpDocComment, $phpDocParameterOutTypes, $immediatelyInvokedCallableParameters, $closureThisParameters, $acceptsNamedArguments);
    }
    /**
    ...
    private function createNativeMethodVariant(FunctionSignature $methodSignature, array $stubPhpDocParameterTypes, array $stubPhpDocParameterVariadicity, ?Type $stubPhpDocReturnType, array $phpDocParameterTypes, ?Type $phpDocReturnType, array $phpDocParameterNameMapping, array $stubPhpDocParameterOutTypes, array $phpDocParameterOutTypes, array $stubImmediatelyInvokedCallableParameters, array $immediatelyInvokedCallableParameters, array $stubClosureThisParameters, array $closureThisParameters, bool $usePhpDocParameterNames) : FunctionVariantWithPhpDocs
    {
        ...
        return new FunctionVariantWithPhpDocs(TemplateTypeMap::createEmpty(), null, $parameters, $methodSignature->isVariadic(), $returnType, $phpDocReturnType ?? new MixedType(), $methodSignature->getNativeReturnType());
    }
