...
class ParametersAcceptorSelector
{
    ...
    private static function hasAcceptorTemplateOrLateResolvableType(\PHPStan\Reflection\ParametersAcceptor $acceptor) : bool
    {
        ...
        foreach ($acceptor->getParameters() as $parameter) {
            if ($parameter instanceof \PHPStan\Reflection\ParameterReflectionWithPhpDocs && $parameter->getOutType() !== null && self::hasTemplateOrLateResolvableType($parameter->getOutType())) {
                return \true;
            }
            if ($parameter instanceof \PHPStan\Reflection\ParameterReflectionWithPhpDocs && $parameter->getClosureThisType() !== null && self::hasTemplateOrLateResolvableType($parameter->getClosureThisType())) {
                return \true;
            }
    ...
    public static function combineAcceptors(array $acceptors) : \PHPStan\Reflection\ParametersAcceptorWithPhpDocs
    {
        ...
        if ($callableOccurred) {
            return new \PHPStan\Reflection\CallableFunctionVariantWithPhpDocs(TemplateTypeMap::createEmpty(), null, $parameters, $isVariadic, $returnType, $phpDocReturnType ?? $returnType, $nativeReturnType ?? new MixedType(), null, $throwPoints, $isPure, $impurePoints, $invalidateExpressions, $usedVariables, $acceptsNamedArguments);
        }
    ...
    private static function wrapAcceptor(\PHPStan\Reflection\ParametersAcceptor $acceptor) : \PHPStan\Reflection\ParametersAcceptorWithPhpDocs
    {
        ...
        if ($acceptor instanceof CallableParametersAcceptor) {
            return new \PHPStan\Reflection\CallableFunctionVariantWithPhpDocs($acceptor->getTemplateTypeMap(), $acceptor->getResolvedTemplateTypeMap(), array_map(static function (\PHPStan\Reflection\ParameterReflection $parameter) : \PHPStan\Reflection\ParameterReflectionWithPhpDocs {
                return self::wrapParameter($parameter);
            }, $acceptor->getParameters()), $acceptor->isVariadic(), $acceptor->getReturnType(), $acceptor->getReturnType(), new MixedType(), TemplateTypeVarianceMap::createEmpty(), $acceptor->getThrowPoints(), $acceptor->isPure(), $acceptor->getImpurePoints(), $acceptor->getInvalidateExpressions(), $acceptor->getUsedVariables(), $acceptor->acceptsNamedArguments());
        }
