...
final class RichParser implements \PHPStan\Parser\Parser
{
     ...
     * @var bool
     */
    private $enableIgnoreErrorsWithinPhpDocs;
    ...
    public function __construct(\PhpParser\Parser $parser, Lexer $lexer, NameResolver $nameResolver, Container $container, IgnoreLexer $ignoreLexer, bool $enableIgnoreErrorsWithinPhpDocs = \false)
    {
        $this->parser = $parser;
        $this->lexer = $lexer;
        ...
        $this->enableIgnoreErrorsWithinPhpDocs = $enableIgnoreErrorsWithinPhpDocs;
    }
    /**
    ...
    public function parseString(string $sourceCode) : array
    {
        ...
        foreach ($traitCollectingVisitor->traits as $trait) {
            ...
            $filteredLinesToIgnore = array_filter($linesToIgnore, static function (int $line) use($trait) : bool {
     ...
     */
    private function getLinesToIgnoreForTokenByIgnoreComment(string $tokenText, int $tokenLine, string $ignoreComment, bool $ignoreNextLine = \false) : array
    {
        $lines = [];
    ...
    private function parseIdentifiers(string $text, int $ignorePos) : array
    {
        ...
        for ($i = 0; $i < $c; $i++) {
            ...
            if ($expected !== null && !in_array($tokenType, $expected, \true)) {
                ...
                $expectedLabels = implode(' or ', array_map(function ($token) {
                    return $this->ignoreLexer->getLabel($token);
                }, $expected));
