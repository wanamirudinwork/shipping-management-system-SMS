...
final class UnreachableIfBranchesRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope) : array
    {
        ...
        foreach ($node->elseifs as $elseif) {
            if ($nextBranchIsDead) {
                $errors[] = $addTip(RuleErrorBuilder::message('Elseif branch is unreachable because previous condition is always true.'))->identifier('elseif.unreachable')->line($elseif->getStartLine())->build();
                continue;
            }
        ...
        if ($node->else !== null && $nextBranchIsDead) {
            $errors[] = $addTip(RuleErrorBuilder::message('Else branch is unreachable because previous condition is always true.'))->identifier('else.unreachable')->line($node->else->getStartLine())->build();
        }
        return $errors;
    }
}
