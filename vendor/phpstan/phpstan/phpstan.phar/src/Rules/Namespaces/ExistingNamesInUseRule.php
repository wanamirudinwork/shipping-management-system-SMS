...
final class ExistingNamesInUseRule implements Rule
{
    ...
    public function __construct(ReflectionProvider $reflectionProvider, ClassNameCheck $classCheck, bool $checkFunctionNameCase)
    {
    ...
    private function checkConstants(array $uses) : array
    {
        ...
        foreach ($uses as $use) {
            ...
            $errors[] = RuleErrorBuilder::message(sprintf('Used constant %s not found.', (string) $use->name))->line($use->name->getStartLine())->identifier('constant.notFound')->discoveringSymbolsTip()->build();
        }
        return $errors;
    }
    /**
    ...
    private function checkFunctions(array $uses) : array
    {
        ...
        foreach ($uses as $use) {
            if (!$this->reflectionProvider->hasFunction($use->name, null)) {
                $errors[] = RuleErrorBuilder::message(sprintf('Used function %s not found.', (string) $use->name))->line($use->name->getStartLine())->identifier('function.notFound')->discoveringSymbolsTip()->build();
    ...
    {
        return $this->classCheck->checkClassNames(array_map(static function (Node\Stmt\UseUse $use) : ClassNameNodePair {
