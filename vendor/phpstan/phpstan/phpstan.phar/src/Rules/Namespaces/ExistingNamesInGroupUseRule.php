...
final class ExistingNamesInGroupUseRule implements Rule
{
    ...
    private function checkConstant(Node\Name $name) : ?IdentifierRuleError
    {
        if (!$this->reflectionProvider->hasConstant($name, null)) {
            return RuleErrorBuilder::message(sprintf('Used constant %s not found.', (string) $name))->discoveringSymbolsTip()->line($name->getStartLine())->identifier('constant.notFound')->build();
        }
        return null;
    }
    private function checkFunction(Node\Name $name) : ?IdentifierRuleError
    {
        if (!$this->reflectionProvider->hasFunction($name, null)) {
            return RuleErrorBuilder::message(sprintf('Used function %s not found.', (string) $name))->discoveringSymbolsTip()->line($name->getStartLine())->identifier('function.notFound')->build();
        }
    ...
    {
        $errors = $this->classCheck->checkClassNames([new ClassNameNodePair((string) $name, $name)]);
        if (count($errors) === 0) {
            return null;
