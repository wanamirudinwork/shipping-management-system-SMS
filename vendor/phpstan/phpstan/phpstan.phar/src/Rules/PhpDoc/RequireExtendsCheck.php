    ...
    public function checkExtendsTags(Node $node, array $extendsTags) : array
    {
        ...
        foreach ($extendsTags as $extendsTag) {
            ...
            if ($referencedClassReflection === null) {
                $errors[] = RuleErrorBuilder::message(sprintf('PHPDoc tag @phpstan-require-extends contains unknown class %s.', $class))->discoveringSymbolsTip()->identifier('class.notFound')->build();
                continue;
            }
