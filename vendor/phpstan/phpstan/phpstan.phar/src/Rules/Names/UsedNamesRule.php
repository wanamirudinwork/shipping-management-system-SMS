...
final class UsedNamesRule implements Rule
{
    ...
    private function findErrorsForNode(Node $node, string $namespace, array &$usedNames) : array
    {
        ...
        if ($node instanceof ClassLike) {
            ...
            if (in_array($name, $usedNames[$lowerNamespace] ?? [], \true)) {
                return [RuleErrorBuilder::message(sprintf('Cannot declare %s %s because the name is already in use.', $type, $namespace !== '' ? $namespace . '\\' . $node->name->toString() : $node->name->toString()))->identifier(sprintf('%s.nameInUse', $type))->line($node->getLine())->nonIgnorable()->build()];
            }
    ...
    private function findErrorsInUses(array $uses, string $useGroupPrefix, string $lowerNamespace, array &$usedNames) : array
    {
        ...
        foreach ($uses as $use) {
            ...
            if (in_array($useAlias, $usedNames[$lowerNamespace] ?? [], \true)) {
                $errors[] = RuleErrorBuilder::message(sprintf('Cannot use %s as %s because the name is already in use.', $useGroupPrefix !== '' ? $useGroupPrefix . '\\' . $use->name->toString() : $use->name->toString(), $use->getAlias()->toString()))->identifier('use.nameInUse')->line($use->getLine())->nonIgnorable()->build();
                continue;
            }
    ...
    /**
     * @param Use_|GroupUse|UseUse $use
     */
