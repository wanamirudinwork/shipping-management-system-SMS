...
final class UnusedFunctionParametersCheck
{
     ...
     */
    public function getUnusedParameters(Scope $scope, array $parameterNames, array $statements, string $unusedParameterMessage, string $identifier) : array
    {
        ...
        foreach (array_keys($unusedParameters) as $name) {
            $errors[] = \PHPStan\Rules\RuleErrorBuilder::message(sprintf($unusedParameterMessage, $name))->identifier($identifier)->build();
        }
        return $errors;
    }
    /**
