...
final class AppendedArrayItemTypeRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope) : array
    {
        ...
        if (!$accepts->result) {
            ...
            return [RuleErrorBuilder::message(sprintf('Array (%s) does not accept %s.', $assignedToType->describe($verbosityLevel), $assignedValueType->describe($verbosityLevel)))->acceptsReasonsTip($accepts->reasons)->identifier('array.valueType')->build()];
        }
        return [];
    }
}
