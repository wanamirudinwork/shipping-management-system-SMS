...
final class AppendedArrayKeyTypeRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope) : array
    {
        ...
        if (!$arrayType->getIterableKeyType()->isSuperTypeOf($keyType)->yes()) {
            ...
            return [RuleErrorBuilder::message(sprintf('Array (%s) does not accept key %s.', $arrayType->describe($verbosity), $keyType->describe(VerbosityLevel::value())))->identifier('array.keyType')->build()];
        }
        return [];
    }
}
