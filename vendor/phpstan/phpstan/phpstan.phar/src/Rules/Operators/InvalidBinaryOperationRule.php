...
final class InvalidBinaryOperationRule implements Rule
{
    ...
    public function __construct(ExprPrinter $exprPrinter, RuleLevelHelper $ruleLevelHelper, bool $bleedingEdge)
    {
    ...
    public function processNode(Node $node, Scope $scope) : array
    {
        ...
        if ($node instanceof Node\Expr\AssignOp\Concat || $node instanceof Node\Expr\BinaryOp\Concat) {
            ...
            $callback = static function (Type $type) : bool {
                return !$type->toNumber() instanceof ErrorType || $type->isArray()->yes();
            };
        } else {
            $callback = static function (Type $type) : bool {
