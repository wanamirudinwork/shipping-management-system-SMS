...
final class MethodTagCheck
{
    ...
    public function __construct(ReflectionProvider $reflectionProvider, ClassNameCheck $classCheck, GenericObjectTypeCheck $genericObjectTypeCheck, MissingTypehintCheck $missingTypehintCheck, UnresolvableTypeHelper $unresolvableTypeHelper, bool $checkClassCaseSensitivity, bool $checkMissingTypehints)
    {
     ...
     * @return list<IdentifierRuleError>
     */
    public function checkInTraitUseContext(ClassReflection $classReflection, ClassReflection $implementingClass, ClassLike $node) : array
    {
    ...
    private function checkMethodTypeInTraitUseContext(ClassReflection $classReflection, string $methodName, string $description, Type $type, ClassLike $node) : array
    {
        ...
        foreach ($type->getReferencedClasses() as $class) {
            if (!$this->reflectionProvider->hasClass($class)) {
                $errors[] = RuleErrorBuilder::message(sprintf('PHPDoc tag @method for method %s::%s() %s contains unknown class %s.', $classReflection->getDisplayName(), $methodName, $description, $class))->identifier('class.notFound')->discoveringSymbolsTip()->build();
