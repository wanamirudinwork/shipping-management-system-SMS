...
final class LocalTypeAliasesCheck
{
     ...
     */
    public function __construct(array $globalTypeAliases, ReflectionProvider $reflectionProvider, TypeNodeResolver $typeNodeResolver, MissingTypehintCheck $missingTypehintCheck, ClassNameCheck $classCheck, UnresolvableTypeHelper $unresolvableTypeHelper, GenericObjectTypeCheck $genericObjectTypeCheck, bool $checkMissingTypehints, bool $checkClassCaseSensitivity, bool $absentTypeChecks)
    {
     ...
     * @return list<IdentifierRuleError>
     */
    public function checkInTraitUseContext(ClassReflection $reflection, ClassReflection $implementingClassReflection, ClassLike $node) : array
    {
        ...
        foreach ($phpDoc->getTypeAliasTags() as $typeAliasTag) {
            ...
            foreach ($resolvedType->getReferencedClasses() as $class) {
                if (!$this->reflectionProvider->hasClass($class)) {
                    $errors[] = RuleErrorBuilder::message(sprintf('Type alias %s contains unknown class %s.', $aliasName, $class))->identifier('class.notFound')->discoveringSymbolsTip()->build();
