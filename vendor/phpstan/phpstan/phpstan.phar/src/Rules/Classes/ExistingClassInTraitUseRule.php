...
final class ExistingClassInTraitUseRule implements Rule
{
    ...
    {
        $messages = $this->classCheck->checkClassNames(array_map(static function (Node\Name $traitName) : ClassNameNodePair {
        ...
        if ($classReflection->isInterface()) {
            ...
            foreach ($node->traits as $trait) {
                ...
                if (!$this->reflectionProvider->hasClass($traitName)) {
                    $messages[] = RuleErrorBuilder::message(sprintf('%s uses unknown trait %s.', $currentName, $traitName))->identifier('trait.notFound')->nonIgnorable()->discoveringSymbolsTip()->build();
                } else {
