...
final class MethodSignatureRule implements Rule
{
     ...
     * @var bool
     */
    private $abstractTraitMethod;
    public function __construct(PhpClassReflectionExtension $phpClassReflectionExtension, bool $reportMaybes, bool $reportStatic, bool $abstractTraitMethod)
    {
        $this->phpClassReflectionExtension = $phpClassReflectionExtension;
        $this->reportMaybes = $reportMaybes;
        $this->reportStatic = $reportStatic;
        $this->abstractTraitMethod = $abstractTraitMethod;
    }
    ...
    private function collectParentMethods(string $methodName, ClassReflection $class) : array
    {
        ...
        if ($this->abstractTraitMethod) {
            foreach ($class->getTraits(\true) as $trait) {
                ...
                $parentMethods[] = [$this->phpClassReflectionExtension->createUserlandMethodReflection($trait, $class, new NativeBuiltinMethodReflection($methodReflection), $declaringTrait->getName()), $declaringTrait];
            }
        }
