...
{
    /**
     * @var GenericAncestorsCheck
     */
    private $genericAncestorsCheck;
    /**
     * @var CrossCheckInterfacesHelper
     */
    ...
    public function processNode(Node $node, Scope $scope) : array
    {
        ...
        $extendsErrors = $this->genericAncestorsCheck->check([], array_map(static function (ExtendsTag $tag) : Type {
            return $tag->getType();
        }, $classReflection->getExtendsTags()), sprintf('Enum %s @extends tag contains incompatible type %%s.', $escapedEnumName), sprintf('Enum %s @extends tag contains unresolvable type.', $enumName), sprintf('Enum %s has @extends tag, but cannot extend anything.', $escapedEnumName), '', '', '', '', '', '', '', '', '');
