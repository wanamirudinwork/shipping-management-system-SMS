...
final class FunctionCallParametersCheck
{
     ...
     */
    public function check(ParametersAcceptor $parametersAcceptor, Scope $scope, bool $isBuiltin, $funcCall, array $messages, string $nodeType = 'function', bool $acceptsNamedArguments = \true) : array
    {
        ...
        if (!$hasNamedArguments) {
            ...
            if ($invokedParametersCount < $functionParametersMinCount || $this->checkExtraArguments && $invokedParametersCount > $functionParametersMaxCount) {
                if ($functionParametersMinCount === $functionParametersMaxCount) {
                    $errors[] = \PHPStan\Rules\RuleErrorBuilder::message(sprintf($invokedParametersCount === 1 ? $messages[0] : $messages[1], $invokedParametersCount, $functionParametersMinCount))->identifier('arguments.count')->line($funcCall->getStartLine())->build();
                } elseif ($functionParametersMaxCount === -1 && $invokedParametersCount < $functionParametersMinCount) {
                    $errors[] = \PHPStan\Rules\RuleErrorBuilder::message(sprintf($invokedParametersCount === 1 ? $messages[2] : $messages[3], $invokedParametersCount, $functionParametersMinCount))->identifier('arguments.count')->line($funcCall->getStartLine())->build();
                } elseif ($functionParametersMaxCount !== -1) {
                    $errors[] = \PHPStan\Rules\RuleErrorBuilder::message(sprintf($invokedParametersCount === 1 ? $messages[4] : $messages[5], $invokedParametersCount, $functionParametersMinCount, $functionParametersMaxCount))->identifier('arguments.count')->line($funcCall->getStartLine())->build();
                }
            }
        }
        if (!$funcCall instanceof Node\Expr\New_ && !$scope->isInFirstLevelStatement() && $scope->getKeepVoidType($funcCall)->isVoid()->yes()) {
            $errors[] = \PHPStan\Rules\RuleErrorBuilder::message($messages[7])->identifier(sprintf('%s.void', $nodeType))->line($funcCall->getStartLine())->build();
        }
        ...
        foreach ($argumentsWithParameters as $i => [$argumentValue, $argumentValueType, $unpack, $argumentName, $argumentLine, $parameter, $originalParameter]) {
            ...
            if (!$acceptsNamedArguments && $this->checkUnresolvableParameterTypes && isset($messages[14])) {
                if ($argumentName !== null) {
                    $errors[] = \PHPStan\Rules\RuleErrorBuilder::message(sprintf($messages[14], sprintf('named argument $%s', $argumentName)))->identifier('argument.named')->line($argumentLine)->build();
                } elseif ($unpack) {
                    $unpackedArrayType = $scope->getType($argumentValue);
                    $hasStringKey = $unpackedArrayType->getIterableKeyType()->isString();
                    if (!$hasStringKey->no()) {
                        $errors[] = \PHPStan\Rules\RuleErrorBuilder::message(sprintf($messages[14], sprintf('unpacked array with %s', $hasStringKey->yes() ? 'string key' : 'possibly string key')))->identifier('argument.named')->line($argumentLine)->build();
                    }
                }
            }
            if ($this->checkArgumentTypes) {
                $parameterType = TypeUtils::resolveLateResolvableTypes($parameter->getType());
                if (!$parameter->passedByReference()->createsNewVariable() || !$isBuiltin && $this->checkUnresolvableParameterTypes && !$argumentValueType instanceof ErrorType) {
                    ...
                    if (!$accepts->result) {
                        ...
                        $errors[] = \PHPStan\Rules\RuleErrorBuilder::message(sprintf($messages[6], $this->describeParameter($parameter, $argumentName === null ? $i + 1 : null), $parameterType->describe($verbosityLevel), $argumentValueType->describe($verbosityLevel)))->identifier('argument.type')->line($argumentLine)->acceptsReasonsTip($accepts->reasons)->build();
                    }
                }
                if ($this->checkUnresolvableParameterTypes && $originalParameter !== null && isset($messages[13]) && !$this->unresolvableTypeHelper->containsUnresolvableType($originalParameter->getType()) && $this->unresolvableTypeHelper->containsUnresolvableType($parameterType)) {
                    $errors[] = \PHPStan\Rules\RuleErrorBuilder::message(sprintf($messages[13], $this->describeParameter($parameter, $argumentName === null ? $i + 1 : null)))->identifier('argument.unresolvableType')->line($argumentLine)->build();
                }
                if ($parameter instanceof ParameterReflectionWithPhpDocs && $parameter->getClosureThisType() !== null && ($argumentValue instanceof Expr\Closure || $argumentValue instanceof Expr\ArrowFunction) && $argumentValue->static) {
                    $errors[] = \PHPStan\Rules\RuleErrorBuilder::message(sprintf($messages[6], $this->describeParameter($parameter, $argumentName === null ? $i + 1 : null), 'bindable closure', 'static closure'))->identifier('argument.staticClosure')->line($argumentLine)->build();
                }
            }
            ...
            if ($this->nullsafeCheck->containsNullSafe($argumentValue)) {
                $errors[] = \PHPStan\Rules\RuleErrorBuilder::message(sprintf($messages[8], $this->describeParameter($parameter, $argumentName === null ? $i + 1 : null)))->identifier('argument.byRef')->line($argumentLine)->build();
                continue;
            }
            if ($argumentValue instanceof Node\Expr\PropertyFetch || $argumentValue instanceof Node\Expr\StaticPropertyFetch) {
                ...
                foreach ($propertyReflections as $propertyReflection) {
                    ...
                    $errors[] = \PHPStan\Rules\RuleErrorBuilder::message(sprintf('Parameter %s is passed by reference so it does not accept %s.', $this->describeParameter($parameter, $argumentName === null ? $i + 1 : null), $propertyDescription))->identifier('argument.byRef')->line($argumentLine)->build();
                }
            }
        ...
        if ($this->checkMissingTypehints && $parametersAcceptor instanceof ResolvedFunctionVariant) {
            ...
            if (count($resolvedTypes) > 0) {
                ...
                foreach ($resolvedTypes as $name => $type) {
                    ...
                    $errors[] = \PHPStan\Rules\RuleErrorBuilder::message(sprintf($messages[9], $name))->identifier('argument.templateType')->line($funcCall->getStartLine())->tip('See: https://phpstan.org/blog/solving-phpstan-error-unable-to-resolve-template-type')->build();
                }
            }
            if (!$this->unresolvableTypeHelper->containsUnresolvableType($originalParametersAcceptor->getReturnType()) && $this->unresolvableTypeHelper->containsUnresolvableType($parametersAcceptor->getReturnType())) {
                $errors[] = \PHPStan\Rules\RuleErrorBuilder::message($messages[12])->identifier(sprintf('%s.unresolvableReturnType', $nodeType))->line($funcCall->getStartLine())->build();
            }
        }
        return $errors;
    }
