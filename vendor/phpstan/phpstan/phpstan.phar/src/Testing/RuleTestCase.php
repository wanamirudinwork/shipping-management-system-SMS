...
abstract class RuleTestCase extends \PHPStan\Testing\PHPStanTestCase
{
    ...
    public function gatherAnalyserErrors(array $files) : array
    {
        ...
        if (count($analyserResult->getInternalErrors()) > 0) {
            $this->fail(implode("\n", array_map(static function (InternalError $internalError) {
        ...
        if ($this->shouldFailOnPhpErrors() && count($analyserResult->getAllPhpErrors()) > 0) {
            $this->fail(implode("\n", array_map(static function (Error $error) : string {
                return sprintf('%s on %s:%d', $error->getMessage(), $error->getFile(), $error->getLine());
            }, $analyserResult->getAllPhpErrors())));
        }
        $finalizer = new AnalyserResultFinalizer($ruleRegistry, new RuleErrorTransformer(), $this->createScopeFactory($this->createReflectionProvider(), $this->getTypeSpecifier()), new LocalIgnoresProcessor(), \true);
        return $finalizer->finalize($analyserResult, \false, \true)->getAnalyserResult()->getUnorderedErrors();
    }
