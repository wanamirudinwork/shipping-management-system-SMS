...
abstract class TypeInferenceTestCase extends \PHPStan\Testing\PHPStanTestCase
{
    ...
    public function assertFileAsserts(string $assertType, string $file, ...$args) : void
    {
        if ($assertType === 'type') {
            ...
            $variableName = $args[2];
            $this->assertTrue($expectedCertainty->equals($actualCertainty), sprintf('Expected %s, actual certainty of %s is %s in %s on line %d.', $expectedCertainty->describe(), $variableName, $actualCertainty->describe(), $file, $args[3]));
        }
    }
    /**
     * @api
        ...
        self::processFile($file, static function (Node $node, Scope $scope) use(&$asserts, $file, $relativePathHelper) : void {
            ...
            if (in_array(strtolower($functionName), ['asserttype', 'assertnativetype', 'assertvariablecertainty'], \true)) {
                ...
                if (!$expectedType instanceof ConstantScalarType) {
                    self::fail(sprintf('Expected type must be a literal string, %s given in %s on line %d.', $expectedType->describe(VerbosityLevel::precise()), $relativePathHelper->getRelativePath($file), $node->getLine()));
                }
