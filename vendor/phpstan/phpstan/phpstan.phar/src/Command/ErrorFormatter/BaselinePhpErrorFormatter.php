        ...
        foreach ($fileErrors as $file => $errors) {
            ...
            foreach ($errors as $error) {
                ...
                if (!isset($fileErrorsByMessage[$errorMessage])) {
                    $fileErrorsByMessage[$errorMessage] = [1, $error->getIdentifier() !== null ? [$error->getIdentifier() => \true] : []];
                    continue;
                }
                ...
                $fileErrorsByMessage[$errorMessage][1][$error->getIdentifier()] = \true;
            }
            ...
            foreach ($fileErrorsByMessage as $message => [$count, $identifiersInKeys]) {
                ...
                $php .= sprintf("\$ignoreErrors[] = [%s\n\t'message' => %s,\n\t'count' => %d,\n\t'path' => __DIR__ . %s,\n];\n", $identifiersComment, var_export(Helpers::escape('#^' . preg_quote($message, '#') . '$#'), \true), var_export($count, \true), var_export(Helpers::escape($file), \true));
            }
        }
        $php .= "\n";
