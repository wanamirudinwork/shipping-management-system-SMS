...
use _PHPStan_14faee166\Clue\React\NDJson\Decoder;
use _PHPStan_14faee166\Clue\React\NDJson\Encoder;
use _PHPStan_14faee166\Composer\CaBundle\CaBundle;
use DateTime;
...
use _PHPStan_14faee166\Psr\Http\Message\ResponseInterface;
...
use function _PHPStan_14faee166\React\Async\await;
...
final class FixerApplication
{
    ...
    public function run(?string $projectConfigFile, InputInterface $input, OutputInterface $output, int $filesCount, string $mainScript) : int
    {
        ...
        $server->on('connection', function (ConnectionInterface $connection) use($loop, $projectConfigFile, $input, $output, $mainScript, $filesCount) : void {
            ...
            $this->fileMonitor->initialize(array_merge($this->analysedPaths, $this->getComposerLocks(), $this->getComposerInstalled(), $this->getExecutedFiles(), $this->getStubFiles(), $this->allConfigFiles));
    ...
    private function analyse(LoopInterface $loop, string $mainScript, ?string $projectConfigFile, InputInterface $input, OutputInterface $output, Encoder $phpstanFixerEncoder) : void
    {
        ...
        $process = new ProcessPromise($loop, 'changedFileAnalysis', ProcessHelper::getWorkerCommand($mainScript, 'fixer:worker', $projectConfigFile, ['--server-port', (string) $serverPort], $input));
