...
use _PHPStan_14faee166\Composer\Semver\Semver;
use _PHPStan_14faee166\Composer\XdebugHandler\XdebugHandler;
use _PHPStan_14faee166\Nette\DI\Helpers;
use _PHPStan_14faee166\Nette\DI\InvalidConfigurationException;
use _PHPStan_14faee166\Nette\DI\ServiceCreationException;
use _PHPStan_14faee166\Nette\FileNotFoundException;
use _PHPStan_14faee166\Nette\InvalidStateException;
use _PHPStan_14faee166\Nette\Schema\ValidationException;
use _PHPStan_14faee166\Nette\Utils\AssertionException;
...
final class CommandHelper
{
    ...
    public static function begin(InputInterface $input, OutputInterface $output, array $paths, ?string $memoryLimit, ?string $autoloadFile, array $composerAutoloaderProjectPaths, ?string $projectConfigFile, ?string $generateBaselineFile, ?string $level, bool $allowXdebug, bool $debugEnabled = \false, bool $cleanupContainerCache = \true) : \PHPStan\Command\InceptionResult
    {
        ...
        if ($projectConfig !== null) {
            ...
            $checkMissingIterableValueType = $parameters['checkMissingIterableValueType'] ?? \true;
            ...
            $checkGenericClassInNonGenericObjectType = $parameters['checkGenericClassInNonGenericObjectType'] ?? \true;
        ...
        $filesCallback = static function () use($currentWorkingDirectoryFileHelper, $stubFilesProvider, $fileFinder, $pathRoutingParser, $paths, $errorOutput) : array {
            ...
            $stubFilesExcluder = new FileExcluder($currentWorkingDirectoryFileHelper, $stubFilesProvider->getProjectStubFiles(), \true);
