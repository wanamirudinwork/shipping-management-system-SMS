...
final class FileAnalyser
{
    ...
    public function __construct(\PHPStan\Analyser\ScopeFactory $scopeFactory, \PHPStan\Analyser\NodeScopeResolver $nodeScopeResolver, Parser $parser, DependencyResolver $dependencyResolver, \PHPStan\Analyser\RuleErrorTransformer $ruleErrorTransformer, \PHPStan\Analyser\LocalIgnoresProcessor $localIgnoresProcessor)
    {
    ...
    public function analyseFile(string $file, array $analysedFiles, RuleRegistry $ruleRegistry, CollectorRegistry $collectorRegistry, ?callable $outerNodeCallback) : \PHPStan\Analyser\FileAnalyserResult
    {
        ...
        return new \PHPStan\Analyser\FileAnalyserResult($fileErrors, $this->filteredPhpErrors, $this->allPhpErrors, $locallyIgnoredErrors, $fileCollectedData, array_values(array_unique($fileDependencies)), $exportedNodes, $linesToIgnore, $unmatchedLineIgnores);
    }
    /**
     * @param Node[] $nodes
    ...
    private function collectErrors(array $analysedFiles) : void
    {
        ...
        set_error_handler(function (int $errno, string $errstr, string $errfile, int $errline) use($analysedFiles) : bool {
            ...
            $this->allPhpErrors[] = (new \PHPStan\Analyser\Error($errorMessage, $errfile, $errline, \true))->withIdentifier('phpstan.php');
            ...
            $this->filteredPhpErrors[] = (new \PHPStan\Analyser\Error($errorMessage, $errfile, $errline, \true))->withIdentifier('phpstan.php');
            return \true;
        });
    }
