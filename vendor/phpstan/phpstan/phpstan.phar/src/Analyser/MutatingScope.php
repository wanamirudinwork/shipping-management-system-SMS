...
final class MutatingScope implements \PHPStan\Analyser\Scope
{
    ...
    /**
     * @var DynamicReturnTypeExtensionRegistry
     */
    private $dynamicReturnTypeExtensionRegistry;
    /**
     * @var ExpressionTypeResolverExtensionRegistry
     */
    private $expressionTypeResolverExtensionRegistry;
    /**
     * @var ExprPrinter
     */
    private $exprPrinter;
    ...
    /**
     * @var PhpFunctionFromParserNodeReflection|null
     */
    private $function;
    /**
     ...
     * @var ?self
     */
    private $scopeOutOfFirstLevelStatement = null;
    /**
     * @var ?self
     */
    private $scopeWithPromotedNativeTypes = null;
    /**
     * @var int
     */
    private static $resolveClosureTypeDepth = 0;
    /**
     ...
     * @param PhpFunctionFromParserNodeReflection|null $function
     */
    ...
    private function resolveType(string $exprString, Expr $node) : Type
    {
        ...
        if ($node instanceof LNumber) {
            ...
            $throwPointsForClosureType = array_map(static function (\PHPStan\Analyser\ThrowPoint $throwPoint) {
                return $throwPoint->isExplicit() ? SimpleThrowPoint::createExplicit($throwPoint->getType(), $throwPoint->canContainAnyThrowable()) : SimpleThrowPoint::createImplicit();
            }, $throwPoints);
            $impurePointsForClosureType = array_map(static function (\PHPStan\Analyser\ImpurePoint $impurePoint) {
                return new SimpleImpurePoint($impurePoint->getIdentifier(), $impurePoint->getDescription(), $impurePoint->isCertain());
            }, $impurePoints);
            $cachedTypes = $node->getAttribute('phpstanCachedTypes', []);
            ...
            return new ClosureType($parameters, $returnType, $isVariadic, TemplateTypeMap::createEmpty(), TemplateTypeMap::createEmpty(), TemplateTypeVarianceMap::createEmpty(), [], $throwPointsForClosureType, $impurePointsForClosureType, $invalidateExpressions, $usedVariables, \true);
     ...
     * @param FunctionReflection|ExtendedMethodReflection|null $function
     */
    ...
    {
        return array_values(array_filter(array_map(static function ($values) {
            return $values[0];
        }, $this->inFunctionCallsStack), static function ($reflection) {
            return $reflection !== null;
        }));
    }
    public function getFunctionCallStackWithParameters() : array
    {
        return array_values(array_filter($this->inFunctionCallsStack, static function ($item) {
            return $item[0] !== null;
        }));
    }
    /** @api */
    ...
    public function enterClassMethod(Node\Stmt\ClassMethod $classMethod, TemplateTypeMap $templateTypeMap, array $phpDocParameterTypes, ?Type $phpDocReturnType, ?Type $throwType, ?string $deprecatedDescription, bool $isDeprecated, bool $isInternal, bool $isFinal, ?bool $isPure = null, bool $acceptsNamedArguments = \true, ?Assertions $asserts = null, ?Type $selfOutType = null, ?string $phpDocComment = null, array $parameterOutTypes = [], array $immediatelyInvokedCallableParameters = [], array $phpDocClosureThisTypeParameters = [], bool $isConstructor = \false) : self
    {
        ...
        return $this->enterFunctionLike(new PhpMethodFromParserNodeReflection($this->getClassReflection(), $classMethod, $this->getFile(), $templateTypeMap, $this->getRealParameterTypes($classMethod), array_map(function (Type $type) : Type {
            return $this->transformStaticType(TemplateTypeHelper::toArgument($type));
        }, $phpDocParameterTypes), $this->getRealParameterDefaultValues($classMethod), $this->transformStaticType($this->getFunctionType($classMethod->returnType, \false, \false)), $phpDocReturnType !== null ? $this->transformStaticType(TemplateTypeHelper::toArgument($phpDocReturnType)) : null, $throwType, $deprecatedDescription, $isDeprecated, $isInternal, $isFinal, $isPure, $acceptsNamedArguments, $asserts ?? Assertions::createEmpty(), $selfOutType, $phpDocComment, array_map(function (Type $type) : Type {
            return $this->transformStaticType(TemplateTypeHelper::toArgument($type));
        }, $parameterOutTypes), $immediatelyInvokedCallableParameters, array_map(function (Type $type) : Type {
            return $this->transformStaticType(TemplateTypeHelper::toArgument($type));
        }, $phpDocClosureThisTypeParameters), $isConstructor), !$classMethod->isStatic());
    }
    ...
    public function enterFunction(Node\Stmt\Function_ $function, TemplateTypeMap $templateTypeMap, array $phpDocParameterTypes, ?Type $phpDocReturnType, ?Type $throwType, ?string $deprecatedDescription, bool $isDeprecated, bool $isInternal, bool $isFinal, ?bool $isPure = null, bool $acceptsNamedArguments = \true, ?Assertions $asserts = null, ?string $phpDocComment = null, array $parameterOutTypes = [], array $immediatelyInvokedCallableParameters = [], array $phpDocClosureThisTypeParameters = []) : self
    {
        return $this->enterFunctionLike(new PhpFunctionFromParserNodeReflection($function, $this->getFile(), $templateTypeMap, $this->getRealParameterTypes($function), array_map(static function (Type $type) : Type {
            ...
            return TemplateTypeHelper::toArgument($type);
        }, $parameterOutTypes), $immediatelyInvokedCallableParameters, $phpDocClosureThisTypeParameters), \false);
    }
    ...
    public function filterBySpecifiedTypes(\PHPStan\Analyser\SpecifiedTypes $specifiedTypes) : self
    {
        ...
        foreach ($conditions as $conditionalExprString => $expressions) {
            $certainty = TrinaryLogic::lazyExtremeIdentity($expressions, static function (\PHPStan\Analyser\ConditionalExpressionHolder $holder) {
                return $holder->getTypeHolder()->getCertainty();
            });
            if ($certainty->no()) {
                unset($scope->expressionTypes[$conditionalExprString]);
            } else {
                $type = TypeCombinator::intersect(...array_map(static function (\PHPStan\Analyser\ConditionalExpressionHolder $holder) {
                    return $holder->getTypeHolder()->getType();
                }, $expressions));
    ...
    private function exactInstantiation(New_ $node, string $className) : ?Type
    {
        ...
        if ($constructorMethod->getDeclaringClass()->getName() !== $classReflection->getName()) {
            ...
            return new GenericObjectType($resolvedClassName, $classReflection->typeMapToList(new TemplateTypeMap($resolvedTypeMap)));
        }
        $parametersAcceptor = ParametersAcceptorSelector::selectFromArgs($this, $methodCall->getArgs(), $constructorMethod->getVariants(), $constructorMethod->getNamedArgumentsVariants());
        if ($this->explicitMixedInUnknownGenericNew) {
            $resolvedTemplateTypeMap = $parametersAcceptor->getResolvedTemplateTypeMap();
            $newGenericType = new GenericObjectType($resolvedClassName, $classReflection->typeMapToList($classReflection->getTemplateTypeMap()));
            if ($isStatic) {
                $newGenericType = new GenericStaticType($classReflection, $classReflection->typeMapToList($classReflection->getTemplateTypeMap()), null, []);
            }
        ...
        if ($isStatic) {
            return new GenericStaticType($classReflection, $list, null, []);
        }
    ...
    private function filterTypeWithMethod(Type $typeWithMethod, string $methodName) : ?Type
    {
        if ($typeWithMethod instanceof UnionType) {
            $typeWithMethod = $typeWithMethod->filterTypes(static function (Type $innerType) use($methodName) {
                return $innerType->hasMethod($methodName)->yes();
            });
        }
    ...
    public function getPropertyReflection(Type $typeWithProperty, string $propertyName) : ?ExtendedPropertyReflection
    {
        if ($typeWithProperty instanceof UnionType) {
            $typeWithProperty = $typeWithProperty->filterTypes(static function (Type $innerType) use($propertyName) {
                return $innerType->hasProperty($propertyName)->yes();
            });
        }
    ...
    public function getConstantReflection(Type $typeWithConstant, string $constantName) : ?ConstantReflection
    {
        if ($typeWithConstant instanceof UnionType) {
            $typeWithConstant = $typeWithConstant->filterTypes(static function (Type $innerType) use($constantName) {
                return $innerType->hasConstant($constantName)->yes();
            });
        }
    ...
    public function getIterableKeyType(Type $iteratee) : Type
    {
        if ($iteratee instanceof UnionType) {
            $filtered = $iteratee->filterTypes(static function (Type $innerType) {
                return $innerType->isIterable()->yes();
            });
