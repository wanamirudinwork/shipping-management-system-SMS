...
final class ResultCacheManager
{
     ...
     */
    public function __construct(ExportedNodeFetcher $exportedNodeFetcher, FileFinder $scanFileFinder, ReflectionProvider $reflectionProvider, StubFilesProvider $stubFilesProvider, FileHelper $fileHelper, string $cacheFilePath, array $analysedPaths, array $composerAutoloaderProjectPaths, string $usedLevel, ?string $cliAutoloadFile, array $bootstrapFiles, array $scanFiles, array $scanDirectories, bool $checkDependenciesOfProjectExtensionFiles)
    {
    ...
    public function restore(array $allAnalysedFiles, bool $debug, bool $onlyFiles, ?array $projectConfigArray, Output $output) : \PHPStan\Analyser\ResultCache\ResultCache
    {
        ...
        if ($output->isVeryVerbose()) {
            ...
            $elapsedString = $elapsed > 5 ? sprintf(' in %f seconds', round($elapsed, 1)) : '';
        ...
        return new \PHPStan\Analyser\ResultCache\ResultCache($filesToAnalyse, \false, $data['lastFullAnalysisTime'], $meta, $filteredErrors, $filteredLocallyIgnoredErrors, $filteredLinesToIgnore, $filteredUnmatchedLineIgnores, $filteredCollectedData, $invertedDependenciesToReturn, $filteredExportedNodes, $data['projectExtensionFiles']);
    }
    /**
     ...
     * @param mixed[] $meta
     */
    private function save(int $lastFullAnalysisTime, array $errors, array $locallyIgnoredErrors, array $linesToIgnore, array $unmatchedLineIgnores, array $collectedData, array $dependencies, array $exportedNodes, array $projectExtensionFiles, array $meta) : void
    {
