...
final class TypeSpecifier
{
    ...
    public function specifyTypesInCondition(\PHPStan\Analyser\Scope $scope, Expr $expr, \PHPStan\Analyser\TypeSpecifierContext $context, ?Expr $rootExpr = null) : \PHPStan\Analyser\SpecifiedTypes
    {
        ...
        if ($expr instanceof Instanceof_) {
            ...
            return $this->specifyTypesInCondition($scope, new Node\Expr\BinaryOp\Equal($expr->expr, new ConstFetch(new Name\FullyQualified('true'))), $context, $rootExpr);
        } elseif ($expr instanceof Expr\Cast\String_) {
            return $this->specifyTypesInCondition($scope, new Node\Expr\BinaryOp\NotEqual($expr->expr, new Node\Scalar\String_('')), $context, $rootExpr);
        } elseif ($expr instanceof Expr\Cast\Int_) {
            return $this->specifyTypesInCondition($scope, new Node\Expr\BinaryOp\NotEqual($expr->expr, new Node\Scalar\LNumber(0)), $context, $rootExpr);
        } elseif ($expr instanceof Expr\Cast\Double) {
            return $this->specifyTypesInCondition($scope, new Node\Expr\BinaryOp\NotEqual($expr->expr, new Node\Scalar\DNumber(0.0)), $context, $rootExpr);
            ...
            if (!$context->null() && $expr->right instanceof FuncCall && count($expr->right->getArgs()) >= 1 && $expr->right->name instanceof Name && in_array(strtolower((string) $expr->right->name), ['count', 'sizeof'], \true) && $leftType->isInteger()->yes()) {
                ...
                if ($context->true() && IntegerRangeType::createAllGreaterThanOrEqualTo(1 - $offset)->isSuperTypeOf($leftType)->yes() || $context->false() && (new ConstantIntegerType(1 - $offset))->isSuperTypeOf($leftType)->yes()) {
                    ...
                    if ($argType->isArray()->yes()) {
                        ...
                        $result = $result->unionWith($this->create($expr->right->getArgs()[0]->value, $newType, $context, \false, $scope, $rootExpr));
                    }
                }
            }
            if (!$context->null() && $expr->right instanceof FuncCall && count($expr->right->getArgs()) >= 3 && $expr->right->name instanceof Name && in_array(strtolower((string) $expr->right->name), ['preg_match'], \true) && IntegerRangeType::fromInterval(0, null)->isSuperTypeOf($leftType)->yes()) {
                return $this->specifyTypesInCondition($scope, new Expr\BinaryOp\NotIdentical($expr->right, new ConstFetch(new Name('false'))), $context, $rootExpr);
            }
            ...
            if ($this->reflectionProvider->hasFunction($expr->name, $scope)) {
                ...
                if ($assertions->getAll() !== []) {
                    $parametersAcceptor = $parametersAcceptor ?? ParametersAcceptorSelector::selectFromArgs($scope, $expr->getArgs(), $functionReflection->getVariants(), $functionReflection->getNamedArgumentsVariants());
                    $asserts = $assertions->mapTypes(static function (Type $type) use($parametersAcceptor) {
            ...
            if ($methodReflection !== null) {
                ...
                if ($assertions->getAll() !== []) {
                    $parametersAcceptor = $parametersAcceptor ?? ParametersAcceptorSelector::selectFromArgs($scope, $expr->getArgs(), $methodReflection->getVariants(), $methodReflection->getNamedArgumentsVariants());
                    $asserts = $assertions->mapTypes(static function (Type $type) use($parametersAcceptor) {
                        return TemplateTypeHelper::resolveTemplateTypes($type, $parametersAcceptor->getResolvedTemplateTypeMap(), $parametersAcceptor instanceof ParametersAcceptorWithPhpDocs ? $parametersAcceptor->getCallSiteVarianceMap() : TemplateTypeVarianceMap::createEmpty(), TemplateTypeVariance::createInvariant());
                    });
                    $specifiedTypes = $this->specifyTypesFromAsserts($context, $expr, $asserts, $parametersAcceptor, $scope);
            ...
            if ($staticMethodReflection !== null) {
                ...
                if ($assertions->getAll() !== []) {
                    $parametersAcceptor = $parametersAcceptor ?? ParametersAcceptorSelector::selectFromArgs($scope, $expr->getArgs(), $staticMethodReflection->getVariants(), $staticMethodReflection->getNamedArgumentsVariants());
                    $asserts = $assertions->mapTypes(static function (Type $type) use($parametersAcceptor) {
                        return TemplateTypeHelper::resolveTemplateTypes($type, $parametersAcceptor->getResolvedTemplateTypeMap(), $parametersAcceptor instanceof ParametersAcceptorWithPhpDocs ? $parametersAcceptor->getCallSiteVarianceMap() : TemplateTypeVarianceMap::createEmpty(), TemplateTypeVariance::createInvariant());
                    });
                    $specifiedTypes = $this->specifyTypesFromAsserts($context, $expr, $asserts, $parametersAcceptor, $scope);
            ...
            if ($context->false()) {
                return new \PHPStan\Analyser\SpecifiedTypes($types->getSureTypes(), $types->getSureNotTypes(), \false, array_merge($this->processBooleanNotSureConditionalTypes($scope, $leftTypes, $rightTypes), $this->processBooleanNotSureConditionalTypes($scope, $rightTypes, $leftTypes), $this->processBooleanSureConditionalTypes($scope, $leftTypes, $rightTypes), $this->processBooleanSureConditionalTypes($scope, $rightTypes, $leftTypes)), $rootExpr);
            }
            return $types;
            ...
            if (!$context->true()) {
                ...
                $isset = $scope->issetCheck($issetExpr, static function () {
                    return \true;
                });
                if ($isset === \false) {
                ...
                $exprType = $this->create($issetExpr, new NullType(), $context->negate(), \false, $scope, $rootExpr);
                if ($issetExpr instanceof Expr\Variable && is_string($issetExpr->name)) {
                    if ($isset === \true) {
                        ...
                        return $exprType->unionWith($this->create(new IssetExpr($issetExpr), new NullType(), $context, \false, $scope, $rootExpr));
                    }
                    if ($isNullable) {
                        // reduces variable certainty to maybe
                        return $exprType->unionWith($this->create(new IssetExpr($issetExpr), new NullType(), $context->negate(), \false, $scope, $rootExpr));
                    }
                    // variable cannot exist in !isset()
                    return $this->create(new IssetExpr($issetExpr), new NullType(), $context, \false, $scope, $rootExpr);
                }
            ...
            foreach ($vars as $var) {
                ...
                if ($var instanceof ArrayDimFetch && $var->dim !== null && !$scope->getType($var->var) instanceof MixedType) {
                    ...
                    if ($dimType instanceof ConstantIntegerType || $dimType instanceof ConstantStringType) {
                        $types = $types->unionWith($this->create($var->var, new HasOffsetType($dimType), $context, \false, $scope, $rootExpr));
                    } else {
                        $varType = $scope->getType($var->var);
                        $narrowedKey = AllowedArrayKeysTypes::narrowOffsetKeyType($varType, $dimType);
                        if ($narrowedKey !== null) {
                            $types = $types->unionWith($this->create($var->dim, $narrowedKey, $context, \false, $scope, $rootExpr));
                        }
                    }
                }
                if ($var instanceof PropertyFetch && $var->name instanceof Node\Identifier) {
                    $types = $types->unionWith($this->create($var->var, new IntersectionType([new ObjectWithoutClassType(), new HasPropertyType($var->name->toString())]), \PHPStan\Analyser\TypeSpecifierContext::createTruthy(), \false, $scope, $rootExpr));
                } elseif ($var instanceof StaticPropertyFetch && $var->class instanceof Expr && $var->name instanceof Node\VarLikeIdentifier) {
                    $types = $types->unionWith($this->create($var->class, new IntersectionType([new ObjectWithoutClassType(), new HasPropertyType($var->name->toString())]), \PHPStan\Analyser\TypeSpecifierContext::createTruthy(), \false, $scope, $rootExpr));
                }
                $types = $types->unionWith($this->create($var, new NullType(), \PHPStan\Analyser\TypeSpecifierContext::createFalse(), \false, $scope, $rootExpr));
            }
            return $types;
            ...
            if (!$context->true()) {
                ...
                $isset = $scope->issetCheck($expr->left, static function () {
                    return \true;
                });
                if ($isset !== \true) {
                    return new \PHPStan\Analyser\SpecifiedTypes();
                }
                return $this->create($expr->left, new NullType(), $context->negate(), \false, $scope, $rootExpr);
            }
            ...
            $isset = $scope->issetCheck($expr->expr, static function () {
                return \true;
            });
            if ($isset === \false) {
                return new \PHPStan\Analyser\SpecifiedTypes();
            ...
            if ($classReflection->hasConstructor()) {
                ...
                if ($asserts->getAll() !== []) {
                    $parametersAcceptor = ParametersAcceptorSelector::selectFromArgs($scope, $expr->getArgs(), $methodReflection->getVariants(), $methodReflection->getNamedArgumentsVariants());
                    $asserts = $asserts->mapTypes(static function (Type $type) use($parametersAcceptor) {
                        return TemplateTypeHelper::resolveTemplateTypes($type, $parametersAcceptor->getResolvedTemplateTypeMap(), $parametersAcceptor instanceof ParametersAcceptorWithPhpDocs ? $parametersAcceptor->getCallSiteVarianceMap() : TemplateTypeVarianceMap::createEmpty(), TemplateTypeVariance::createInvariant());
                    });
                    $specifiedTypes = $this->specifyTypesFromAsserts($context, $expr, $asserts, $parametersAcceptor, $scope);
                    if ($specifiedTypes !== null) {
    ...
    private function narrowUnionByArraySize(FuncCall $countFuncCall, UnionType $argType, ?Type $sizeType, \PHPStan\Analyser\TypeSpecifierContext $context, \PHPStan\Analyser\Scope $scope, ?Expr $rootExpr) : ?\PHPStan\Analyser\SpecifiedTypes
    {
        if ($sizeType === null) {
            return null;
        }
        ...
        if ($isNormalCount->yes() && $argType->isConstantArray()->yes()) {
            $result = [];
            foreach ($argType->getTypes() as $innerType) {
    ...
    private function turnListIntoConstantArray(FuncCall $countFuncCall, Type $type, Type $sizeType, \PHPStan\Analyser\Scope $scope) : ?Type
    {
        ...
        if ($isNormalCount->yes() && $type->isList()->yes() && $sizeType instanceof ConstantIntegerType && $sizeType->getValue() < ConstantArrayTypeBuilder::ARRAY_COUNT_LIMIT) {
        ...
        if ($isNormalCount->yes() && $type->isList()->yes() && $sizeType instanceof IntegerRangeType && $sizeType->getMin() !== null) {
            // turn optional offsets non-optional
            $valueTypesBuilder = ConstantArrayTypeBuilder::createEmpty();
            ...
            if ($sizeType->getMax() !== null) {
                for ($i = $sizeType->getMin(); $i < $sizeType->getMax(); $i++) {
                    $offsetType = new ConstantIntegerType($i);
                    $valueTypesBuilder->setOffsetValueType($offsetType, $type->getOffsetValueType($offsetType), \true);
                }
    ...
    public function resolveEqual(Expr\BinaryOp\Equal $expr, \PHPStan\Analyser\Scope $scope, \PHPStan\Analyser\TypeSpecifierContext $context, ?Expr $rootExpr) : \PHPStan\Analyser\SpecifiedTypes
    {
    ...
    public function resolveIdentical(Expr\BinaryOp\Identical $expr, \PHPStan\Analyser\Scope $scope, \PHPStan\Analyser\TypeSpecifierContext $context, ?Expr $rootExpr) : \PHPStan\Analyser\SpecifiedTypes
    {
        ...
        if (!$context->null() && $unwrappedLeftExpr instanceof FuncCall && count($unwrappedLeftExpr->getArgs()) >= 1 && $unwrappedLeftExpr->name instanceof Name && in_array(strtolower((string) $unwrappedLeftExpr->name), ['count', 'sizeof'], \true) && $rightType->isInteger()->yes()) {
            ...
            if ($isZero->yes()) {
                ...
                return $funcTypes->unionWith($this->create($unwrappedLeftExpr->getArgs()[0]->value, $newArgType, $context, \false, $scope, $rootExpr));
            }
            if ($argType instanceof UnionType) {
            ...
            if ($context->truthy()) {
                if ($argType->isArray()->yes()) {
                    if ($argType->isConstantArray()->yes() && $rightType->isSuperTypeOf($argType->getArraySize())->no()) {
                        return $this->create($unwrappedLeftExpr->getArgs()[0]->value, new NeverType(), $context, \false, $scope, $rootExpr);
                    }
                    $funcTypes = $this->create($unwrappedLeftExpr, $rightType, $context, \false, $scope, $rootExpr);
                    $constArray = $this->turnListIntoConstantArray($unwrappedLeftExpr, $argType, $rightType, $scope);
                    if ($constArray !== null) {
                        return $funcTypes->unionWith($this->create($unwrappedLeftExpr->getArgs()[0]->value, $constArray, $context, \false, $scope, $rootExpr));
                    } elseif (IntegerRangeType::fromInterval(1, null)->isSuperTypeOf($rightType)->yes()) {
                        return $funcTypes->unionWith($this->create($unwrappedLeftExpr->getArgs()[0]->value, new NonEmptyArrayType(), $context, \false, $scope, $rootExpr));
                    }
                    return $funcTypes;
                }
            }
        ...
        if (!$context->null() && $unwrappedLeftExpr instanceof FuncCall && count($unwrappedLeftExpr->getArgs()) === 1 && $unwrappedLeftExpr->name instanceof Name && in_array(strtolower((string) $unwrappedLeftExpr->name), ['strlen', 'mb_strlen'], \true) && $rightType->isInteger()->yes()) {
            ...
            if ($isZero->yes()) {
                $funcTypes = $this->create($unwrappedLeftExpr, $rightType, $context, \false, $scope, $rootExpr);
                return $funcTypes->unionWith($this->create($unwrappedLeftExpr->getArgs()[0]->value, new ConstantStringType(''), $context, \false, $scope, $rootExpr));
            }
        ...
        if ($context->true() && $unwrappedLeftExpr instanceof FuncCall && $unwrappedLeftExpr->name instanceof Name && $unwrappedLeftExpr->name->toLowerString() === 'preg_match' && (new ConstantIntegerType(1))->isSuperTypeOf($rightType)->yes()) {
            return $this->specifyTypesInCondition($scope, $leftExpr, $context, $rootExpr);
        }
        // get_class($a) === 'Foo'
        ...
        if ($context->truthy() && $unwrappedLeftExpr instanceof FuncCall && $unwrappedLeftExpr->name instanceof Name && in_array(strtolower($unwrappedLeftExpr->name->toString()), ['substr', 'strstr', 'stristr', 'strchr', 'strrchr', 'strtolower', 'strtoupper', 'ucfirst', 'lcfirst', 'mb_substr', 'mb_strstr', 'mb_stristr', 'mb_strchr', 'mb_strrchr', 'mb_strtolower', 'mb_strtoupper', 'mb_ucfirst', 'mb_lcfirst', 'ucwords', 'mb_convert_case', 'mb_convert_kana'], \true) && isset($unwrappedLeftExpr->getArgs()[0]) && $rightType->isNonEmptyString()->yes()) {
            ...
            if ($argType->isString()->yes()) {
                if ($rightType->isNonFalsyString()->yes()) {
                    return $this->create($unwrappedLeftExpr->getArgs()[0]->value, TypeCombinator::intersect($argType, new AccessoryNonFalsyStringType()), $context, \false, $scope);
                }
                return $this->create($unwrappedLeftExpr->getArgs()[0]->value, TypeCombinator::intersect($argType, new AccessoryNonEmptyStringType()), $context, \false, $scope);
            }
        }
        ...
        if ($expressions !== null) {
            ...
            if ($specifiedType !== null) {
                if ($exprNode !== $unwrappedExprNode) {
                    $specifiedType = $specifiedType->unionWith($this->create($exprNode, $constantType, $context, \false, $scope, $rootExpr));
                }
