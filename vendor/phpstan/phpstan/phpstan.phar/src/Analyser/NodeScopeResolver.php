...
final class NodeScopeResolver
{
    ...
    /**
     * @readonly
     * @var ParameterOutTypeExtensionProvider
     */
    private $parameterOutTypeExtensionProvider;
    /**
     * @readonly
     * @var Parser
     */
    ...
    /**
     * @readonly
     * @var ParameterClosureTypeExtensionProvider
     */
    private $parameterClosureTypeExtensionProvider;
    /**
     * @readonly
     * @var ScopeFactory
     */
     ...
     * @var bool
     */
    private $paramOutType;
    /**
     * @readonly
     * @var bool
     */
    private $preciseMissingReturn;
    /**
     * @readonly
     * @var bool
     */
    private $explicitThrow;
    ...
    public function __construct(ReflectionProvider $reflectionProvider, InitializerExprTypeResolver $initializerExprTypeResolver, Reflector $reflector, ClassReflectionExtensionRegistryProvider $classReflectionExtensionRegistryProvider, ParameterOutTypeExtensionProvider $parameterOutTypeExtensionProvider, Parser $parser, FileTypeMapper $fileTypeMapper, StubPhpDocProvider $stubPhpDocProvider, PhpVersion $phpVersion, SignatureMapProvider $signatureMapProvider, PhpDocInheritanceResolver $phpDocInheritanceResolver, FileHelper $fileHelper, \PHPStan\Analyser\TypeSpecifier $typeSpecifier, DynamicThrowTypeExtensionProvider $dynamicThrowTypeExtensionProvider, ReadWritePropertiesExtensionProvider $readWritePropertiesExtensionProvider, ParameterClosureTypeExtensionProvider $parameterClosureTypeExtensionProvider, \PHPStan\Analyser\ScopeFactory $scopeFactory, bool $polluteScopeWithLoopInitialAssignments, bool $polluteScopeWithAlwaysIterableForeach, array $earlyTerminatingMethodCalls, array $earlyTerminatingFunctionCalls, array $universalObjectCratesClasses, bool $implicitThrows, bool $treatPhpDocTypesAsCertain, bool $detectDeadTypeInMultiCatch, bool $paramOutType, bool $preciseMissingReturn, bool $explicitThrow)
    {
        ...
        $this->preciseMissingReturn = $preciseMissingReturn;
        $this->explicitThrow = $explicitThrow;
            ...
            if ($shouldCheckLastStatement && $isLast) {
                ...
                if ($this->preciseMissingReturn && count($endStatements) > 0) {
                    foreach ($endStatements as $endStatement) {
                        ...
                        $nodeCallback(new ExecutionEndNode($endStatement->getStatement(), new \PHPStan\Analyser\StatementResult($endStatementResult->getScope(), $hasYield, $endStatementResult->isAlwaysTerminating(), $endStatementResult->getExitPoints(), $endStatementResult->getThrowPoints(), $endStatementResult->getImpurePoints()), $parentNode->returnType !== null), $endStatementResult->getScope());
                    }
                } else {
                    $nodeCallback(new ExecutionEndNode($stmt, new \PHPStan\Analyser\StatementResult($scope, $hasYield, $statementResult->isAlwaysTerminating(), $statementResult->getExitPoints(), $statementResult->getThrowPoints(), $statementResult->getImpurePoints()), $parentNode->returnType !== null), $scope);
                }
            }
    ...
    private function processStmtNode(Node\Stmt $stmt, \PHPStan\Analyser\MutatingScope $scope, callable $nodeCallback, \PHPStan\Analyser\StatementContext $context) : \PHPStan\Analyser\StatementResult
    {
        ...
        if ($stmt instanceof Node\Stmt\Declare_) {
            ...
            $functionScope = $scope->enterFunction($stmt, $templateTypeMap, $phpDocParameterTypes, $phpDocReturnType, $phpDocThrowType, $deprecatedDescription, $isDeprecated, $isInternal, $isFinal, $isPure, $acceptsNamedArguments, $asserts, $phpDocComment, $phpDocParameterOutTypes, $phpDocImmediatelyInvokedCallableParameters, $phpDocClosureThisTypeParameters);
            ...
            $throwPoints = array_filter($result->getThrowPoints(), static function ($throwPoint) {
            ...
            $impurePoints = $result->getImpurePoints();
            return new \PHPStan\Analyser\StatementResult($result->getScope(), $result->hasYield(), \true, [new \PHPStan\Analyser\StatementExitPoint($stmt, $scope)], $throwPoints, $impurePoints);
            ...
            foreach ($stmt->catches as $catchNode) {
                ...
                if (count($matchingThrowPoints) === 0) {
                    foreach ($throwPoints as $throwPointIndex => $throwPoint) {
                        foreach ($catchTypes as $catchTypeIndex => $catchTypeItem) {
                            ...
                            if (!$throwNode instanceof Throw_ && !$throwNode instanceof Expr\Throw_ && !($throwNode instanceof Node\Stmt\Expression && $throwNode->expr instanceof Expr\Throw_)) {
                ...
                // implicit only
                if (count($matchingThrowPoints) === 0 || $this->explicitThrow && $onlyExplicitIsThrow) {
                    foreach ($throwPoints as $throwPointIndex => $throwPoint) {
                        if ($throwPoint->isExplicit()) {
                            continue;
                        }
    ...
    public function processExprNode(Node\Stmt $stmt, Expr $expr, \PHPStan\Analyser\MutatingScope $scope, callable $nodeCallback, \PHPStan\Analyser\ExpressionContext $context) : \PHPStan\Analyser\ExpressionResult
    {
        ...
        if ($expr instanceof Variable) {
            ...
            if ($expr->name instanceof Expr) {
                ...
                if ($nameType->isObject()->yes() && $nameType->isCallable()->yes() && (new ObjectType(Closure::class))->isSuperTypeOf($nameType)->no()) {
                    ...
                    $callableThrowPoints = array_map(static function (SimpleThrowPoint $throwPoint) use($scope, $expr) {
                        return $throwPoint->isExplicit() ? \PHPStan\Analyser\ThrowPoint::createExplicit($scope, $throwPoint->getType(), $expr, $throwPoint->canContainAnyThrowable()) : \PHPStan\Analyser\ThrowPoint::createImplicit($scope, $expr);
                    }, $parametersAcceptor->getThrowPoints());
                    if (!$this->implicitThrows) {
                        $callableThrowPoints = array_values(array_filter($callableThrowPoints, static function (\PHPStan\Analyser\ThrowPoint $throwPoint) {
                            return $throwPoint->isExplicit();
                        }));
                    }
                    $throwPoints = array_merge($throwPoints, $callableThrowPoints);
                    $impurePoints = array_merge($impurePoints, array_map(static function (SimpleImpurePoint $impurePoint) use($scope, $expr) {
                        return new \PHPStan\Analyser\ImpurePoint($scope, $expr, $impurePoint->getIdentifier(), $impurePoint->getDescription(), $impurePoint->isCertain());
            ...
            return new \PHPStan\Analyser\ExpressionResult($scope, $exprResult->hasYield(), $exprResult->getThrowPoints(), $exprResult->getImpurePoints(), static function () use($scope, $expr) : \PHPStan\Analyser\MutatingScope {
            ...
            if ($methodReflection !== null && !$methodReflection->isStatic() && $methodReflection->getName() === '__construct' && $scopeFunction instanceof MethodReflection && !$scopeFunction->isStatic() && $scope->isInClass() && $scope->getClassReflection()->isSubclassOf($methodReflection->getDeclaringClass()->getName())) {
            ...
            return new \PHPStan\Analyser\ExpressionResult($finalScope, $ternaryCondResult->hasYield(), $throwPoints, $impurePoints, static function () use($finalScope, $expr) : \PHPStan\Analyser\MutatingScope {
            ...
            if ($condType->isEnum()->yes()) {
                ...
                if (count($enumCases) > 0) {
                    ...
                    foreach ($arms as $i => $arm) {
                        ...
                        $matchArmBodyScope = $matchScope->addTypeToExpression($expr->cond, $conditionCaseType);
            ...
            foreach ($arms as $i => $arm) {
                ...
                if (count($filteringExprs) === 1) {
                    ...
                    $filteringExpr = new FuncCall(new Name\FullyQualified('in_array'), [new Arg($expr->cond), new Arg(new Array_($items)), new Arg(new ConstFetch(new Name\FullyQualified('true')))]);
                }
        ...
        return new \PHPStan\Analyser\ExpressionResult($scope, $hasYield, $throwPoints, $impurePoints, static function () use($scope, $expr) : \PHPStan\Analyser\MutatingScope {
            return $scope->filterByTruthyValue($expr);
        }, static function () use($scope, $expr) : \PHPStan\Analyser\MutatingScope {
            return $scope->filterByFalseyValue($expr);
        });
        ...
        if (count($constantArrays) > 0) {
            ...
            foreach ($constantArrays as $constantArray) {
                ...
                if ($constantArray->isConstantArray()->yes() && $nonConstantArrayWasUnpacked) {
                    ...
                    $constantArray = $isList ? AccessoryArrayListType::intersectWith($constantArray) : $constantArray;
                }
    ...
    private function processImmediatelyCalledCallable(\PHPStan\Analyser\MutatingScope $scope, array $invalidatedExpressions, array $uses) : \PHPStan\Analyser\MutatingScope
    {
        ...
        foreach ($invalidatedExpressions as $invalidateExpression) {
            $found = \false;
            foreach ($uses as $use) {
                $result = $finder->findFirst([$invalidateExpression->getExpr()], static function ($node) use($use) {
                    return $node instanceof Variable && $node->name === $use;
                });
                if ($result === null) {
                    continue;
                }
            ...
            if (count($acceptors) > 0) {
                foreach ($acceptors as $acceptor) {
                    if ($callableParameters === null) {
                        $callableParameters = array_map(static function (ParameterReflection $callableParameter) {
                            return new NativeParameterReflection($callableParameter->getName(), $callableParameter->isOptional(), $callableParameter->getType(), $callableParameter->passedByReference(), $callableParameter->isVariadic(), $callableParameter->getDefaultValue());
    ...
    private function processArgs(Node\Stmt $stmt, $calleeReflection, ?ExtendedMethodReflection $nakedMethodReflection, ?ParametersAcceptor $parametersAcceptor, CallLike $callLike, \PHPStan\Analyser\MutatingScope $scope, callable $nodeCallback, \PHPStan\Analyser\ExpressionContext $context, ?\PHPStan\Analyser\MutatingScope $closureBindScope = null) : \PHPStan\Analyser\ExpressionResult
    {
        ...
        foreach ($args as $i => $arg) {
            ...
            if ($arg->value instanceof Expr\Closure) {
                ...
                if ($closureBindScope === null && $parameter instanceof ParameterReflectionWithPhpDocs && $parameter->getClosureThisType() !== null && !$arg->value->static) {
                ...
                if ($callCallbackImmediately) {
                    $throwPoints = array_merge($throwPoints, array_map(static function (\PHPStan\Analyser\ThrowPoint $throwPoint) use($scope, $arg) {
                        return $throwPoint->isExplicit() ? \PHPStan\Analyser\ThrowPoint::createExplicit($scope, $throwPoint->getType(), $arg->value, $throwPoint->canContainAnyThrowable()) : \PHPStan\Analyser\ThrowPoint::createImplicit($scope, $arg->value);
                ...
                if ($callCallbackImmediately) {
                    $throwPoints = array_merge($throwPoints, array_map(static function (\PHPStan\Analyser\ThrowPoint $throwPoint) use($scope, $arg) {
                        return $throwPoint->isExplicit() ? \PHPStan\Analyser\ThrowPoint::createExplicit($scope, $throwPoint->getType(), $arg->value, $throwPoint->canContainAnyThrowable()) : \PHPStan\Analyser\ThrowPoint::createImplicit($scope, $arg->value);
                    }, $arrowFunctionResult->getThrowPoints()));
                ...
                if ($exprType->isCallable()->yes()) {
                    ...
                    if (count($acceptors) === 1) {
                        ...
                        if ($callCallbackImmediately) {
                            $callableThrowPoints = array_map(static function (SimpleThrowPoint $throwPoint) use($scope, $arg) {
                                return $throwPoint->isExplicit() ? \PHPStan\Analyser\ThrowPoint::createExplicit($scope, $throwPoint->getType(), $arg->value, $throwPoint->canContainAnyThrowable()) : \PHPStan\Analyser\ThrowPoint::createImplicit($scope, $arg->value);
                            }, $acceptors[0]->getThrowPoints());
                            if (!$this->implicitThrows) {
                                $callableThrowPoints = array_values(array_filter($callableThrowPoints, static function (\PHPStan\Analyser\ThrowPoint $throwPoint) {
                                    return $throwPoint->isExplicit();
                                }));
                            ...
                            $throwPoints = array_merge($throwPoints, $callableThrowPoints);
                            $impurePoints = array_merge($impurePoints, array_map(static function (SimpleImpurePoint $impurePoint) use($scope, $arg) {
                                return new \PHPStan\Analyser\ImpurePoint($scope, $arg->value, $impurePoint->getIdentifier(), $impurePoint->getDescription(), $impurePoint->isCertain());
                            }, $acceptors[0]->getImpurePoints()));
                        }
                    }
                }
            }
        ...
        foreach ($args as $i => $arg) {
            ...
            if ($currentParameter !== null) {
                ...
                if ($assignByReference) {
                    ...
                    } elseif ($calleeReflection instanceof MethodReflection && !$calleeReflection->getDeclaringClass()->isBuiltin() && $this->paramOutType) {
                        $byRefType = $currentParameter->getType();
                    } elseif ($calleeReflection instanceof FunctionReflection && !$calleeReflection->isBuiltin() && $this->paramOutType) {
                        $byRefType = $currentParameter->getType();
                    }
                }
            }
    ...
    private function processAssignVar(\PHPStan\Analyser\MutatingScope $scope, Node\Stmt $stmt, Expr $var, Expr $assignedExpr, callable $nodeCallback, \PHPStan\Analyser\ExpressionContext $context, Closure $processExprCallback, bool $enterExpressionAssign) : \PHPStan\Analyser\ExpressionResult
    {
        ...
        if ($var instanceof Variable && is_string($var->name)) {
            ...
            foreach ($var->items as $i => $arrayItem) {
                ...
                $result = $this->processAssignVar($scope, $stmt, $arrayItem->value, new GetOffsetValueTypeExpr($assignedExpr, $dimExpr), $nodeCallback, $context, static function (\PHPStan\Analyser\MutatingScope $scope) : \PHPStan\Analyser\ExpressionResult {
                    return new \PHPStan\Analyser\ExpressionResult($scope, \false, [], []);
     ...
     * @param Node\FunctionLike|Node\Stmt\Property $node
     */
