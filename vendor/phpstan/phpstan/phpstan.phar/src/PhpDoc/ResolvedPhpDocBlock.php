...
class ResolvedPhpDocBlock
{
     ...
     * @var ?bool
     */
    private $isAllowedPrivateMutation = null;
    /**
     * @var ?bool
     */
    ...
    private static function mergeAssertTags(array $assertTags, array $parents, array $parentPhpDocBlocks) : array
    {
        ...
        foreach ($parents as $i => $parent) {
            ...
            return array_map(static function (AssertTag $assertTag) use($phpDocBlock) {
                return self::resolveTemplateTypeInTag($assertTag->withParameter($phpDocBlock->transformAssertTagParameterWithParameterNameMapping($assertTag->getParameter()))->toImplicit(), $phpDocBlock, TemplateTypeVariance::createCovariant());
            }, $result);
        }
        return $assertTags;
    }
