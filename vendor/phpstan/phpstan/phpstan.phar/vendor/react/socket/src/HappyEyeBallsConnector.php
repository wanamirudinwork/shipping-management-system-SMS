...
namespace _PHPStan_14faee166\React\Socket;

use _PHPStan_14faee166\React\Dns\Resolver\ResolverInterface;
use _PHPStan_14faee166\React\EventLoop\Loop;
use _PHPStan_14faee166\React\EventLoop\LoopInterface;
use _PHPStan_14faee166\React\Promise;
final class HappyEyeBallsConnector implements ConnectorInterface
{
    ...
    public function __construct($loop = null, $connector = null, $resolver = null)
    {
        ...
        if ($loop !== null && !$loop instanceof LoopInterface) {
            ...
            throw new \InvalidArgumentException('_PHPStan_14faee166\\Argument #1 ($loop) expected null|React\\EventLoop\\LoopInterface');
        }
        if (!$connector instanceof ConnectorInterface) {
            // manual type check to support legacy PHP < 7.1
            throw new \InvalidArgumentException('_PHPStan_14faee166\\Argument #2 ($connector) expected React\\Socket\\ConnectorInterface');
        }
        if (!$resolver instanceof ResolverInterface) {
            // manual type check to support legacy PHP < 7.1
            throw new \InvalidArgumentException('_PHPStan_14faee166\\Argument #3 ($resolver) expected React\\Dns\\Resolver\\ResolverInterface');
        }
