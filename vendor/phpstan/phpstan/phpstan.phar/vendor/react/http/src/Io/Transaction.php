...
use _PHPStan_14faee166\React\Http\Message\Response;
use _PHPStan_14faee166\React\Http\Message\ResponseException;
use _PHPStan_14faee166\React\Http\Message\Uri;
use _PHPStan_14faee166\React\Promise\Deferred;
use _PHPStan_14faee166\React\Promise\Promise;
...
class Transaction
{
    ...
    public function bufferResponse(ResponseInterface $response, Deferred $deferred, ClientRequestState $state)
    {
        ...
        if ($size !== null && $size > $this->maximumSize) {
            $body->close();
            return \_PHPStan_14faee166\React\Promise\reject(new \OverflowException('Response body size of ' . $size . ' bytes exceeds maximum of ' . $this->maximumSize . ' bytes', \defined('SOCKET_EMSGSIZE') ? \SOCKET_EMSGSIZE : 90));
        }
        // body is not streaming => already buffered
        if (!$body instanceof ReadableStreamInterface) {
            return \_PHPStan_14faee166\React\Promise\resolve($response);
        }
        /** @var ?\Closure $closer */
        $closer = null;
