...
namespace _PHPStan_14faee166\React\EventLoop;

use _PHPStan_14faee166\React\EventLoop\Tick\FutureTickQueue;
use _PHPStan_14faee166\React\EventLoop\Timer\Timer;
...
final class ExtUvLoop implements LoopInterface
{
    ...
    public function addSignal($signal, $listener)
    {
        ...
        if (!isset($this->signalEvents[$signal])) {
            ...
            $this->signalEvents[$signal] = \_PHPStan_14faee166\uv_signal_init($this->uv);
            \_PHPStan_14faee166\uv_signal_start($this->signalEvents[$signal], function () use($signals, $signal) {
                $signals->call($signal);
            }, $signal);
        }
    }
    ...
    {
        if (!isset($this->streamEvents[(int) $stream])) {
            $this->streamEvents[(int) $stream] = \_PHPStan_14faee166\uv_poll_init_socket($this->uv, $stream);
        }
