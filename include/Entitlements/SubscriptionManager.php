...
class SubscriptionManager
{
    ...
    /**
     * flag for caching user count
     */
    protected const THRESHOLD_USER_LIMIT_FOR_CACHE = 100;

    /**
     * user count cache key
     */
    protected const USER_COUNT_CACHE_KEY = 'user_count_cache_key';

    /**
     * user count cache ttl
     * 2 minutes
     */
    protected const USER_COUNT_CACHE_TTL = 120;
    ...
    public function downloadSubscriptionContent(?string $licenseKey)
    {
        ...
        SubscriptionManager::instance()->clearUserCountCache();
    }
     ...
     * @return array
     */
    public function getAllHiddenProducts() : array
    {
        return [];
    }
    /**
    ...
    public function getSystemLicenseTypesExceededLimit(int &$license_seats_needed, bool $ignoreCache = false): array
    {
        ...
        foreach ($userCountByType as $licenseType => $count) {
            if ($count > 0 && !in_array($licenseType, Subscription::EXCEPTED_LIMITCHECK_LICENSE_KEYS, true)) {
    ...
    public function getUserExceededLicenseTypes(\User $user): array
    {
        ...
        return array_filter($userTypes, function ($type) use ($allowedSeats, $usedSeats) {
            return !safeInArray($type, Subscription::EXCEPTED_LIMITCHECK_LICENSE_KEYS, true) &&
                (empty($allowedSeats[$type]) || $allowedSeats[$type]['quantity'] - $usedSeats[$type] <= 0);
        });
    }
     ...
     * @return array
     */
    public function getSystemUserCountByLicenseTypes(bool $useCache = true): array
    {
        if ($this->getTotalNumberOfMangoUsers() > self::THRESHOLD_USER_LIMIT_FOR_CACHE && $useCache) {
            // go with cached results
            return $this->getSystemUserCountByLicenseTypesUsingCache($useCache);
        }
        global $db;
        ...
        while (($row = $db->fetchByAssoc($result, false)) != null) {
            $this->getSystemLicenseTypesByRow($row, $userCountByType);
        }

        return $userCountByType;
    }

    protected function getSystemLicenseTypesByRow(array $row, array &$userCountByType): bool
    {
        $foundUnknownType = false;
        $unknownTypes = '';
        if (empty($row['license_type'])) {
            $type = $this->getUserDefaultLicenseType();
            if (empty($type) || !in_array($type, $this->getAllSupportedProducts())) {
                if (!empty($type)) {
                    $unknownTypes .= $type . ' ';
                }
            ...
            if (!is_array($types)) {
                ...
                foreach ($types as $type) {
                    ...
                    if (!in_array($type, $this->getAllSupportedProducts())) {
                        $foundUnknownType = true;
                        $unknownTypes .= $type . ' ';
        ...
        return $foundUnknownType;
    }
    /**
     * Get system active users by license types using cache
     *
     * @param bool $useCache use cache or not
     * @return array
     */
    protected function getSystemUserCountByLicenseTypesUsingCache(bool $useCache = true): array
    {
        if ($useCache) {
            $cacheValue = sugar_cache_retrieve(self::USER_COUNT_CACHE_KEY);
            if ($cacheValue && is_array($cacheValue) && array_sum($cacheValue) > 0) {
                return $cacheValue;
            }
        }

        global $db;
        $where = sprintf(' status = %s', $db->quoted('Active'));

        $query = 'SELECT user_name, license_type, deleted, is_group from users WHERE ' . $where;
        $result = $db->query($query, true, 'Error filling in user array: ');
        $supportedTypes = $this->getAllSupportedProducts();
        $userCountByType = [];
        foreach ($supportedTypes as $type) {
        ...
        while (($row = $db->fetchByAssoc($result, false)) != null) {
            if ((empty($row['user_name']) || in_array($row['user_name'], \User::SUPPORT_USERS))
            || (!empty($row['deleted']) && $row['deleted'] == 1)
            || (!empty($row['is_group']) && $row['is_group'] == 1)) {
                continue;
            }
        ...
        sugar_cache_put(self::USER_COUNT_CACHE_KEY, $userCountByType, self::USER_COUNT_CACHE_TTL);
        return $userCountByType;
    }
    ...
    {
        sugar_cache_clear(self::USER_COUNT_CACHE_KEY);
    }
