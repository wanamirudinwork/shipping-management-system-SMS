...
if (!function_exists('recursiveParse')) {
...
if (!isset($exec) || $exec) {
    ...
    $checkCondition = function () {
        return true; // Always execute since we want to rebuild the cache
    };

    $longRunningFunction = function (int $attempt) use ($silent) {
        buildCache(sugar_cached('Expressions'), $silent, true);
    };

    $refuseFunction = $longRunningFunction;

    // Prevent process race conditions during cache building
    $systemProcessLock = new SystemProcessLock(__FILE__ . 'buildCache', 'expression_cache_builder');
