...
abstract class DBManager implements LoggerAwareInterface
{
    ...
    public function repairTable(SugarBean $bean, $execute = true)
    {
        ...
        foreach ($indices as $ind_def) {
            if (!isset($ind_def['name'])) {
                $this->logger->error('Invalid index def passed: ' . (new Exception)->getTraceAsString());
                continue;
            }
    ...
    /**
     * Returns array with sanitized `length` and `precision` values
     * @param array $def Vardef-format field def
     * @return void
     */
    protected function sanitizeLenPrecision(array &$def): void
    {
        // Handle length if it exists
        if (isset($def['len'])) {
            if (str_contains($def['len'], ',')) {
                // Handle combined length,precision format
                [$len, $precision] = explode(',', $def['len'], 2);
                $def['len'] = intval($len) . ',' . intval($precision);
            } else {
                // Handle simple length format
                $def['len'] = intval($def['len']);
            }
        }

        // Handle precision if it exists
        if (isset($def['precision'])) {
            $def['precision'] = intval($def['precision']);
        }
    }
    ...
    protected function oneColumnSQLRep($fieldDef, $ignoreRequired = false, $table = '', $return_as_array = false, $action = null)
    {
        $colBaseType = null;
        $defLen = null;
        $this->sanitizeLenPrecision($fieldDef);
        $name = $fieldDef['name'];
        $type = $this->getFieldType($fieldDef);
        ...
        if (isset($fieldDef['isnull']) && (strtolower($fieldDef['isnull']) == 'false' || $fieldDef['isnull'] === false)
            && (!empty($fieldDef['required']) || ($fieldDef['type'] ?? '') === 'bool')) {
            $required = 'NOT NULL';
        }
        ...
        foreach ($this->standardQueries as $qstart => $check) {
            if (strncasecmp($qstart, $query, strlen($qstart)) == 0) {
                if (is_callable([$this, $check])) {
                    ...
                    if (!safeInArray($table, $skipTables)) {
