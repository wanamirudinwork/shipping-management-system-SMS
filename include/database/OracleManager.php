...
WHERE OWNER = ?
    ...
    public function getColumnFunctionalIndices(string $table, string $column): array
    {
        return array_filter($this->get_indices($table), function (array $index) use ($column) {
            ...
            if (safeInArray(sprintf('upper(%s)', $column), $index['fields'], true)) {
                return true;
            }
            // case where field other than $column has an upper function applied to it
            if (safeInArray($column, $index['fields']) && !empty(array_filter($index['fields'], function ($field) {
    ...
    public function getAutoIncrement($table, $field_name)
    {
        ...
        $sequenceOwner = strtoupper($this->getValidDBName($this->configOptions['db_schema_name'] ?? ''));
        // Querying system table for latest autoincrement value works correctly only if the sequence is NOCACHE
        // Sugar defines sequences as NOCACHE (except temporary tables in denormalization), so we can rely on this
        $currval = $this->getOne(<<<SQL
            SELECT LAST_NUMBER
            FROM ALL_SEQUENCES
            WHERE SEQUENCE_NAME = '{$seqName}' AND SEQUENCE_OWNER = '{$sequenceOwner}'
            SQL
        );
        return $currval !== false ? $currval : '';
    }
    ...
    public function setAutoIncrementStart($table, $field_name, $start_value)
    {
        ...
        return $this->query("ALTER SEQUENCE {$sequence_name} RESTART START WITH {$start}");
    }
