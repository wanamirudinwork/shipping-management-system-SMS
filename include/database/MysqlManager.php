...
abstract class MysqlManager extends DBManager
{
     ...
     * This includes a join handled by merging derived tables and views in the FROM clause into the outer query block
     * @see https://dev.mysql.com/doc/refman/8.4/en/join.html
     * Setting it to 50, so custom code can safely add a few more.
     */
    public const MYSQL_MAX_JOINS = 50;
...
WHERE TABLE_SCHEMA = ?
    ...
    public function repairTableParams($tablename, $fielddefs, array $indices, $execute = true, $engine = null)
    {
        foreach ($indices as $key => $ind) {
            if (isset($ind['type']) && strtolower($ind['type']) == 'primary') {
    ...
    {
        $tableSanitized = $this->getValidDBName($table, true, 'table');
        // SHOW TABLE STATUS uses cached metadata value, so we use ANALYZE to refresh it
        $this->query("ANALYZE TABLE $tableSanitized");
        $result = $this->query("SHOW TABLE STATUS LIKE '$tableSanitized'");
...
FROM information_schema.statistics';
    ...
    {
        $q = "SHOW COLLATION LIKE 'utf8mb4_%0900_ai%'";
        $r = $this->query($q);
        $res = [];
        while ($a = $this->fetchByAssoc($r)) {
            $res[] = $a['Collation'];
