...
class SugarHeartbeatClient extends \SoapClient
{
    ...
    public function __construct()
    {
        $this->defaultOptions = [
            ...
            'exceptions' => 0,
            'user_agent' => $this->getUserAgentForSoap(),
        ];
        $options = $this->getOptions();
        parent::__construct(null, $options);
    }

    private function getUserAgentForSoap()
    {
        if (class_exists(UserAgent::class)) {
            return (string) UserAgent::forSoap();
        }
        return sprintf('SugarCRM/%s/SOAPClient/%s', $GLOBALS['sugar_version'] ?: 'unknown_version', '1.0');
    }
