...
class SubPanelDefinitions
{
    ...
    public $layout_defs;
    public $platform;
    public static $refreshHiddenSubpanels = false;
     ...
     */
    public function get_available_tabs($FromGetModuleSubpanels = false, $filter = true)
    {
        global $modListHeader;
        global $modules_exempt_from_availability_check;
     ...
     */
    public function load_subpanel($name, $reload = false, $original_only = false, $forApi = false)
    {
        ...
        if (empty($this->layout_defs) || $reload || (!empty($layout_def_key) && !isset($layout_defs [$layout_def_key]))) {
            if (!$original_only) {
                if (isModuleBWC($this->_focus->module_dir)) {
                    $def_path = ['modules/' . $this->_focus->module_dir . '/metadata/' . ($this->platform == 'mobile' ? 'wireless.' : '') . 'subpaneldefs.php'];
                    ...
                    $viewdefs = !empty($viewdefs['subpanels']['meta']['components']) ? $viewdefs['subpanels']['meta']['components'] : [];
                }
            }
    ...
    public static function set_hidden_subpanels($panels)
    {
        ...
        $administration->saveSetting('MySettings', 'hide_subpanels', json_encode(array_keys($panels)));
