...
class VardefManager
{
    ...
    public static function ignoreTemplate($object, $template)
    {
        if (isset($GLOBALS['dictionary'][$object]['ignore_templates'])) {
            if (is_array($GLOBALS['dictionary'][$object]['ignore_templates'])) {
                return safeInArray($template, $GLOBALS['dictionary'][$object]['ignore_templates']);
            } else {
    ...
    protected static function addSugarACLStatic($object, $module)
    {
        ...
        if (!empty($dictionary[$object]) && !isset($dictionary[$object]['acls']['SugarACLStatic'])) {
            ...
            if (!class_exists($class) && !empty($beanFiles[$module]) && file_exists($beanFiles[$module])) {
                require_once $beanFiles[$module];
            }

            $logFatal = function () use ($class, $module, $object) {
                LoggerManager::getLogger()
                    ->fatal(
                        sprintf(
                            '%s was called with ($object = %s, $module = %s) parameters. Class %s cannot be loaded',
                            __METHOD__,
                            $object,
                            $module,
                            $class
            ...
            if (class_exists($class)) {
                try {
                    ...
                    $logFatal();
                    LoggerManager::getLogger()->fatal($e->getMessage() . PHP_EOL . $e->getTraceAsString());
                }
            } else {
                $logFatal();
            }
