...
class SugarWidgetReportField extends SugarWidgetField
{
    ...
    {
        static $subclassCache = [];
        $layoutDefHash = sha1(serialize($layout_def));
        if (!isset($subclassCache[$layoutDefHash])) {
            $subclassCache[$layoutDefHash] = $this->getSubClass($layout_def);
        }
        $obj = $subclassCache[$layoutDefHash];
