...
// @codingStandardsIgnoreEnd
    ...
    protected function queryDateOp($arg1, $arg2, $op, $type)
    {
        ...
        if ($type === 'datetime' || $type === 'datetimecombo') {
            ...
            $targetDateTime = $reportFormatter->getDBDatetimeFromUserInput($arg2);
        }

        return "$arg1 $op " . $this->reporter->db->convert($this->reporter->db->quoted($targetDateTime), $type) . "\n";
    }
    ...
    public function queryFilterTP_last_n_days($layout_def)
    {
        ...
        if (is_numeric($def0)) {
            ...
            trigger_error(
                'Invalid usage of method %s: array argument with numeric value expected'
    ...
    public function getFieldControllerData(array $layoutDef)
    {
        ...
        $reportFormatter = ReportFormatterFactory::getFormatter($layoutDef['type']);

        return $reportFormatter->getDBDatetimeFromUserInput($value, true);
    }
