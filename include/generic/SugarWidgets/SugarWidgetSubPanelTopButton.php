...
class SugarWidgetSubPanelTopButton extends SugarWidget
{
    ...
    public function _get_form_sidecar($defines)
    {
        global $app_strings;
        global $subpanel_item_count;
        global $current_language;

        $sidecarReadySubPanelCreates = [
        ...
        if (!isModuleBWC($module) && in_array($defines['widget_class'], $sidecarReadySubPanelCreates)) {
            ...
            $escapedFormId = Escape::htmlAttr($form);
            $escapedModule = Escape::js($module);
            $escapedParentId = Escape::js($parentId);
            $escapedLink = Escape::js($link);
            $escapedId = Escape::htmlAttr($id);
            $escapedLabel = Escape::html($label);
            $escapedLabelAttr = Escape::htmlAttr($label);

            if ($this->enableActionMenu) {
                $button = <<<HTML
                <form data-legacy-subpanel-create="1" action="index.php" method="post" name="form" id="$escapedFormId">
                    <a
                        href="#"
                        onClick="javascript:subp_nav_sidecar('$escapedModule', '$escapedParentId', 'c', '$escapedLink');"
                        class="create_from_bwc_to_sidecar"
                        id="$escapedId"
                    >$escapedLabel</a>
                </form>
                HTML;
