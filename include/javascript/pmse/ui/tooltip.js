...
PMSE.Tooltip.prototype.show = function(x, y) {
    var msgDiv;
    const tooltipWidth = 144; //the width of the tooltip is set in css as 128px with 8px padding each side
    ...
    if (!this.messageObject) {
        ...
        msgDiv.style.top = (y + extraMargin) + 'px';
        if (x + 2 * extraMargin + tooltipWidth < window.innerWidth) {
            msgDiv.style.left = (x + extraMargin) + 'px';
        } else {
            msgDiv.style.right = extraMargin + 'px';
        }
