 ...
 */
ExpressionControl.prototype.setValidationError = function (validationError) {
 ...
 * @return {string}
 */
ExpressionControl.prototype.getValidationError = function() {
...
ExpressionControl.prototype.isValid = function() {
    ...
    for (i = 0; i < items.length; i += 1) {
        ...
        if (!valid) {
            if (!(prev && (prev.expType === 'GROUP' && prev.expValue === ')'))) {
                this.setValidationError('LBL_PMSE_FORM_ERROR_MULTIPLE_CRITERIA');
            }
            break;
        }
        prev = current;
