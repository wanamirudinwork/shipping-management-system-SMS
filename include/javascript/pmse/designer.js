...
/**
 * Validates a field, selected in Settings for a 'Change Field' Action.
 * @param {Object} field An object with field settings like type, module, field, value, etc.
 * @param {Object} element is the element on the canvas that is currently being examined/validated
 */
var validateChangeField = function(field, element) {
    // validate a DropDown in case it has an optionItem value (available options)
    if (field.type === 'DropDown' && 'optionItem' in field && !_.isObject(field.optionItem)) {
        createWarning(element, 'LBL_PMSE_ERROR_ACTION_CHANGE_FIELD_INVALID_OPTIONS', field.field);
    }
};
...
var getSearchInfo = function(criteria) {
    ...
    switch (type) {
        ...
        case 'USER_IDENTITY':
            ...
            if (criteria.add_group_users) {
                args.add_group_users = 1;
            }
            break;
        case 'USER_ROLE':
        case 'ROLE':
