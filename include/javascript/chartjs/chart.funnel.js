	...
	Chart2.controllers.funnel = Chart2.DatasetController.extend({
		...
		updateElement: function updateElement(trapezium, index, reset) {
			...
			if (opts.keep === 'left') {
				...
				const chartOffset = chart.chartOffset || 0;
				x = (chartArea.left + chartArea.right) / 2 + chartOffset;
			}
...
module.exports = function(Chart2) {
	...
	Chart2.elements.Trapezium = Chart2.Element.extend({
        ...
        getCorners: function () {
			...
			const isCompactChart = chart.isCompactChart;
			const chartOffset = chart.chartOffset || 0;

            calculatedWidth =  isCompactChart ? chart.getCalculatedWidth() : chart.width - chartPadding;
            contextChartWidth = chart.width;
            calculatedCenter = contextChartWidth / 2 + chartOffset;// ox center
            calculatedHeight = isCompactChart ? Math.min(calculatedWidth * heightScaleFactor, (calculatedWidth - calculatedWidth * r) / (2 * r)) : chart.height - topOffset;
