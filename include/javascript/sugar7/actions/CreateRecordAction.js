...
(function register(app) {
    app.events.on('app:init', function init() {
        ...
        CreateRecord.prototype._populateAsyncModelWithCustomAttributes = function(
            ...
            const parentIsHome = parentRecordView.model.module === 'Home';
            const recordModule = parentIsHome ? opts.recordModel.get('_module') : parentRecordView.model.module;
            const recordId = parentIsHome ? opts.recordModel.get('id') : parentRecordView.model.id;
         ...
         * @return {Object} Sidecar model update data
         */
        CreateRecord.prototype.normalizeRelateValue = function(fieldDef, fieldData, model) {
            ...
            value[idName] = !_.isEmpty(model.get(idName)) ? model.get(idName) : fieldData.value[fieldData.fieldName];
            return value;
        };
         ...
         * @return {Object} Sidecar model update data
         */
        CreateRecord.prototype.normalizeParentValue = function(fieldDef, fieldData, model) {
            ...
            if (_.isObject(fieldData.value.parent)) {
                ...
                if (recordModule && recordId && recordName) {
                    value[fieldData.fieldName] = recordName;
                    value[typeName] = recordModule;
                    value[idName] = recordId;
                } else if (_.isObject(model.get('parent'))) {
                    const parent = model.get('parent');
                    value[fieldData.fieldName] = parent.name;
                    value[typeName] = parent._module;
                    value[idName] = parent.id;
                } else if (!_.isEmpty(model.get(typeName)) && !_.isEmpty(model.get(idName))) {
                    value[fieldData.fieldName] = fieldData.value[fieldData.fieldName];
                    value[typeName] = model.get(typeName);
                    value[idName] = model.get(idName);
                }
            }
        ...
        CreateRecord.prototype.normalizeFieldValue = function(fieldData, model) {
            ...
            switch (fieldDef.type) {
                ...
                case 'datetimecombo':
                    ...
                    break;
                case 'relate':
                    result.push(this.normalizeRelateValue(fieldDef, fieldData, model));
                    break;
                case 'parent':
                    result.push(this.normalizeParentValue(fieldDef, fieldData, model));
                    break;
                case 'tag':
