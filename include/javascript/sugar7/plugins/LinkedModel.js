    ...
    app.events.on('app:init', function() {
        app.plugins.register('LinkedModel', ['view', 'field'], {
            ...
            createLinkModel: function(parentModel, link) {
                ...
                // If the link is a parent relationship, we need to create a
                // bean of the related module instead of a related bean
                // so that the new bean will be saved using ModuleApi to allow users
                // to change the parent field.
                let model = this._isParentRelationship(parentModel.module, link) ?
                    app.data.createBean(app.data.getRelatedModule(parentModel.module, link)) :
                    app.data.createRelatedBean(parentModel, null, link);
                let relatedFields = app.data.getRelateFields(parentModel.module, link);
                let parentModule = app.metadata.getModule(parentModel.module);
            ...
            /**
             * Check if a link is a parent relationship.
             *
             * @param {string} module Module name
             * @param {string} link Link name
             * @return {boolean} True if the link is a parent relationship
             * @private
             */
            _isParentRelationship: function(module, link) {
                let vardefs = app.metadata.getModule(module) || {};
                let relationshipName = vardefs.fields && vardefs.fields[link] && vardefs.fields[link].relationship;
                let relationship = app.metadata.getRelationship(relationshipName) || {};
                if (relationship.relationship_role_column === 'parent_type' &&
                    relationship.relationship_role_column_value && relationship.rhs_key === 'parent_id') {
                    return true;
                }
                return false;
            },
