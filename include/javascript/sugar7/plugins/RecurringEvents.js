            ...
            */
            ...
            _fieldOnAttach: function(component) {
                this.before('render', function() {
                    ...
                    if (this.name === 'repeat_month') {
                        this.prepareRepeatMonthDefaultValue();
                    }
                });
            },
            ...
            /**
             * Sets the value of the month field to the current month
             */
            prepareRepeatMonthDefaultValue: function() {
                if (this.model) {
                    const hasNoRset = _.isUndefined(this.model.get('rset')) || _.isEmpty(this.model.get('rset'));
                    const hasRepeatTypeChangedToYearly = this.model._previousAttributes.hasOwnProperty('repeat_type') &&
                        this.model._previousAttributes.repeat_type !== 'Yearly' &&
                        this.model.changedAttributes().repeat_type === 'Yearly';

                    if (this.action === 'edit' && (hasNoRset || hasRepeatTypeChangedToYearly)) {
                        const currentMonth = new Date().getMonth() + 1;
                        this.model.setDefault('repeat_month', currentMonth.toString());
                        this.model.set('repeat_month', currentMonth.toString());
                    }
                }
            },
