    ...
    app.events.on('app:init', function() {
        ...
        app.plugins.register('ToggleVisibility', 'view', {
                ...
                this.on('init', function() {
                    if (!this.meta.last_state) {
                        this.meta.last_state = {
                            id: this.dashModel.get('id') + ':' + this.name + ':' + this.cid,
                            defaults: {}
                        };
                    }
            ...
            visibilitySwitcher: function(event) {
                const visibility = this.$(event.currentTarget).val();
                ...
                if (_.isFunction(this.updateVisibilityConfig)) {
            ...
            /**
             * Update visibility buttons state.
             */
            updateVisibilityButtons: function() {
                const activeVisibility = this.getVisibility();

                const visibilityBtns = this.$('[data-action=visibility-switcher]');

                if (visibilityBtns.length) {
                    _.each(visibilityBtns, btn => {
                        const isActive = $(btn).val() === activeVisibility;
