...
(function(app) {
    ...
    // Set the proper style of the content div ("floating" or full-frame) when necessary
    app.events.on('app:start', function() {
        app.utils.toggleFullContentDiv(!app.api.isAuthenticated());
    });
    app.events.on('app:login:success', function() {
        app.utils.toggleFullContentDiv(false);
    });
    app.events.on('app:logout:success', function() {
        app.utils.toggleFullContentDiv(true);
    });
