...
function get_sugar_config_defaults()
{
    ...
    $sugar_config_defaults = [
        ...
        'upload_badext' => [
            ...
            'prune_delay' => 86400000, // 24h in milliseconds
    ];
...
function return_app_list_strings_language($language, $useCache = true)
{
    ...
    if ($useCache) {
        ...
        $cache_entry = Container::getInstance()->get(InMemory::class)->get($cache_key);
    ...
    if ($useCache) {
        Container::getInstance()->get(InMemory::class)->set($cache_key, $return_value);
    }
...
/**
 * Check if the current user has Market license
 *
 * @return bool
 */
function hasMarketLicense(): bool
{
    global $current_user;
    ...
    return safeInArray(Subscription::SUGAR_MARKET_KEY, $licenses);
}
 ...
 * @return string
 */
function getMajorVersion(string $version): string
{
    $arr = explode('.', $version);
    return $arr[0] ?? '0';
}
...
function getOptionsFromVardef($vardef)
{
    ...
    // Only cache when using 'options' (not 'function')
    if (empty($vardef['function']) && isset($vardef['options'])) {
        // Build a cache key based on options and module
        ...
        $result = translate($vardef['options'], $vardef['module'] ?? '');
...
function validateRequiredFormula($fieldsToValidate, $bean)
{
    ...
    foreach ($fieldsToValidate as $fieldName => $formula) {
        ...
        if ((empty($formula) || $result === AbstractExpression::$TRUE) &&
            ...
            (!isset($bean->field_defs[$fieldName]['hidden']) || !$bean->field_defs[$fieldName]['hidden'])) {
                $fieldDef = $bean->field_defs[$fieldName] ?? [];

                return $fieldDef['labelValue'] ?? translate($fieldDef['vname'] ?? '');
        }
    }
}
