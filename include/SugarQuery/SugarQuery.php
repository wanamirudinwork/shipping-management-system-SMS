...
class SugarQuery
{
    ...
    /**
     * Custom parameters that can be used to pass additional information to the query
     * @var array
     */
     ...
     * @return mixed|null
     */
    public function getCustomParam(?string $name = null): mixed
    {
        return $this->customParams[$name] ?? null;
    }
     ...
     * @param mixed $value
     */
    public function setCustomParam(string $name, mixed $value): void
    {
    ...
    {
        if ($this->select instanceof \SugarQuery_Builder_Select) {
            $this->select = clone $this->select;
        }

        if ($this->union instanceof \SugarQuery_Builder_Union) {
            $this->union = clone $this->union;
        }

        if ($this->where instanceof \SugarQuery_Builder_Where) {
            $this->where = clone $this->where;
        }

        if ($this->having instanceof \SugarQuery_Builder_Where) {
            $this->having = clone $this->having;
        }

        foreach ($this->group_by as $i => $groupBy) {
            $this->group_by[$i] = clone $groupBy;
        }

        foreach ($this->order_by as $i => $orderBy) {
            $this->order_by[$i] = clone $orderBy;
        }

        foreach ($this->join as $alias => $join) {
            $this->join[$alias] = clone $join;
            $this->join[$alias]->query = $this;
        }
    }
}
