...
class SugarEmailAddress extends SugarBean
{
    ...
    protected static array $emailAddressRuntimeCache = [];
            ...
            if (empty($this->addresses) || !isset($_REQUEST['massupdate'])) {
                ...
                if (!empty($bean->email) && is_array($bean->email)) {
                    foreach ($bean->email as $emailAddr) {
                        ...
                        $replyToE = $this->boolVal($emailAddr['reply_to_address'] ?? false);
                        $this->addAddress($address, $primaryE, $replyToE, $invalidE, $optO);
                    }
                }
                ...
                for ($i = 1; $i <= self::MAX_LEGACY_EMAILS; $i++) {
                    ...
                    if (isset($bean->$email) && !empty($bean->$email) && $handleField) {
                        ...
                        $reply_to_field = $this->boolVal($bean->email[$i - 1]['reply_to_address'] ?? false);
                        $this->addAddress($bean->$email, $isPrimary, $reply_to_field, $field_invalid, $field_optOut);
                        $isPrimary = false;
                    }
                }
    ...
    public function addAddress($addr, $primary = false, $replyTo = false, $invalid = false, $optOut = false, $email_id = null, $validate = true)
    {
        ...
        foreach ($this->addresses as $k => $address) {
            ...
            // to prevent several primary emails
            if ($primary) {
                $this->addresses[$k]['primary_address'] = false;
            }
        }
    ...
    public function AddUpdateEmailAddress($addr, $invalid = null, $opt_out = null, $id = null)
    {
        ...
        if (!empty($address) || !empty($existingBean)) {
            ...
            $this->logUpdateEmailProperties($existingBean, $opt_out, $invalid);
            if (!is_null($invalid)) {
    ...
    public function getPrimaryAddress($focus, $parent_id = null, $parent_type = null)
    {
        ...
        $cacheKey = __METHOD__ . '_' . $parent_type . $parent_id;
        if (isset(self::$emailAddressRuntimeCache[$cacheKey])) {
            return self::$emailAddressRuntimeCache[$cacheKey];
        }

        $q = 'SELECT ea.email_address FROM ' . $this->table_name . ' ea
                INNER JOIN email_addr_bean_rel ear ON ea.id = ear.email_address_id
                AND ear.bean_module = ?
                AND ear.bean_id = ?
                AND ear.deleted = 0
        ...
        self::$emailAddressRuntimeCache[$cacheKey] = $address;
    ...
    {
        $cacheKey = __METHOD__ . '_' . $focus->module_dir . $focus->id;
        if (isset(self::$emailAddressRuntimeCache[$cacheKey])) {
            return self::$emailAddressRuntimeCache[$cacheKey];
        }
     ...
     */
    public function getCorrectedModule($module): string
    {
        return ($module == 'Employees') ? 'Users' : (string)$module;
    }
    ...
    /**
     * Log if Opt Out status was updated during creating a new record
     * @param SugarEmailAddress|null $existingBean
     * @param bool|null $opt_out
     * @param bool|null $invalid
     * @return void
     */
    public function logUpdateEmailProperties(?SugarEmailAddress $existingBean, ?bool $opt_out, ?bool $invalid)
    {
        if (!empty($existingBean) && empty($existingBean->processed_dates_times)) {
            if ($existingBean->opt_out !== $opt_out) {
                LoggerManager::getLogger()
                    ->warn(
                        sprintf(
                            'Opt Out status was updated during creating a new record. From %s to %s',
                            $existingBean->opt_out,
                            empty($opt_out) ? 0 : $opt_out
                        ),
                    );
            }
            if ($existingBean->invalid_email !== $invalid) {
                LoggerManager::getLogger()
                    ->warn(
                        sprintf(
                            'Invalid email status was updated during creating a new record. From %s to %s',
                            $existingBean->invalid_email,
                            empty($invalid) ? 0 : $invalid
                        ),
                    );
            }
        }
