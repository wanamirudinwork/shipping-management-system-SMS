...
class SugarOAuth2ServerOIDC extends SugarOAuth2Server implements LoggerAwareInterface
{
    ...
    public function grantAccessToken(?array $inputData = null, ?array $authHeaders = null): array
    {
        ...
        try {
            ...
            $refreshToken = $resultToken->getAttribute('refresh_token');
            $refreshExpiresIn = time() + $this->getVariable(self::CONFIG_REFRESH_LIFETIME);
            $idmConfig = $this->getIdmModeConfig();

            if (!empty($idmConfig['urlResourceOwnerDetails']) && $refreshToken) {
                $result = $this->getIdmProvider($idmConfig)->introspectRefreshToken($refreshToken, $idmConfig['urlResourceOwnerDetails']);

                if (isset($result['exp'])) {
                    $refreshExpiresIn = $result['exp'];
                }
            }
    ...
    /**
     * @param array $idmConfig
     * @return IdmProvider
     */
    protected function getIdmProvider($idmConfig)
    {
        return new IdmProvider($idmConfig);
    }
}
