...
abstract class SugarApi
{
    ...
    /**
     * List of modules that are only accessible by admin users
     * @var array
     */
    public static $adminOnlyModules = [
        'Shippers',
        'InboundEmail',
        //The below modules were got from sugarcrm/include/modules.php:452.
        //These modules are admin only
        'DynamicFields',
        'Currencies',
        'EditCustomFields',
        'ACLActions',
    ...
    /**
     * ensure admin user or developer for the module
     * @param ServiceBase $api
     * @param string $module
     *
     * @return bool
     */
    protected function hasAdminOrDeveloperAccess(ServiceBase $api, string $module) : bool
    {
        global $current_user;
        $currentUser = !empty($api->user) ? $api->user : $current_user;
        return !empty($currentUser) && ($currentUser->isAdminForModule($module) || $currentUser->isDeveloperForModule($module));
    }
     ...
     * @throws SugarApiExceptionNotAuthorized
     */
    protected function ensureAdminOrDeveloperAccess(ServiceBase $api, string $module) : void
    {
        if (!$this->hasAdminOrDeveloperAccess($api, $module)) {
            throw new SugarApiExceptionNotAuthorized('EXCEPTION_NOT_AUTHORIZED', null, $module);
        }
    }
     ...
     * @throws SugarApiExceptionNotAuthorized
     */
    protected function restrictAccessToAdminOnlyModules(ServiceBase $api, array $args): void
    {
        if (isset($args['module']) && is_string($args['module']) && in_array($args['module'], self::$adminOnlyModules)) {
            $this->ensureAdminOrDeveloperAccess($api, $args['module']);
        }
    }
}
