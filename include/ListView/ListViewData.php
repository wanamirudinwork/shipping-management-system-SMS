...
class ListViewData
{
     ...
     */
    public function getListViewData($seed, $where, $offset = -1, $limit = -1, $filter_fields = [], $params = [], $id_field = 'id', $singleSelect = true)
    {
        $seedACL = [];
        $tempACL = [];
        $pageData = [];
        ...
        while (($row = $this->db->fetchByAssoc($result)) != null) {
            if ($count < $limit) {
                ...
                $idIndex[$row[$id_field]][] = safeCount($rows);
                $rows[] = $row;
            }
            $count++;
        }
        ...
        if ($count != 0) {
            ...
            if (!empty($ret_array['secondary_select'])) {
                ...
                foreach ($rows_keys as $key) {
                    $rows[$key]['secondary_select_count'] = safeCount($ref_id_count[$rows[$key]['ref_id']]);
                }
            }
            ...
            while ($row = current($rows)) {
                ...
                $pageData['rowAccess'][$dataIndex] = ['view' => $tempACL['DetailView'], 'edit' => $tempACL['EditView']];
        ...
        if (isset($_REQUEST['searchFormTab']) && $_REQUEST['searchFormTab'] == 'advanced_search' ||
            isset($_REQUEST['type_basic']) && (safeCount($_REQUEST['type_basic']) > 1 || $_REQUEST['type_basic'][0] != '') ||
            ...
            if ($seed->module_dir == 'Reports') {
                ...
                $searchMetaData = method_exists('SearchForm', 'retrieveSearchDefs') ? SearchForm::retrieveSearchDefs($seed->module_dir) : [];
            }
        ...
        return ['data' => $data, 'pageData' => $pageData, 'query' => $searchString];
    }
