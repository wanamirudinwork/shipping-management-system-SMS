...
class TimeDate
{
    ...
    /**
     * Convert sting of different formats to SugarDateTime object
     *
     * @param string $str
     * @return SugarDateTime|bool
     */
    public function stringToDate(string $str): \SugarDateTime|bool
    {
        $timestamp = strtotime($str);

        if ($timestamp !== false) {
            $fDate = date(TimeDate::DB_DATE_FORMAT, $timestamp);
            $date = $this->fromIsoDate($fDate);
        } else {
            // use date format from user preferences
            $date = SugarDateTime::createFromFormat(
                TimeDate::DB_DATE_FORMAT,
                $this->to_db_date($str, false)
            );
        }
