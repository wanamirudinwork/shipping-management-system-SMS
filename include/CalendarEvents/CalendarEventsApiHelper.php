...
{
    /**
     * List of fields that are going to be retrieved all the time
     * Even if the bean has been deleted
     * @var array
     */
    public $deletedMandatoryFields = [
        'original_start_date',
        'repeat_parent_id',
    ];
    ...
    public function formatForApi(SugarBean $bean, array $fieldList = [], array $options = [])
    {
        ...
        if ($bean->deleted) {
            ...
            foreach ($this->deletedMandatoryFields as $fieldName) {
                $properties = array_key_exists($fieldName, $bean->field_defs) ? $bean->field_defs[$fieldName] : [];
