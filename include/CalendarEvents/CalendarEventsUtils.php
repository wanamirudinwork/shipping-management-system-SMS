...
class CalendarEventsUtils
{
    ...
    public function createRsetAndGetOccurrences(string $givenRrule = ''): array
    {
        ...
        try {
            ...
            // this means the $givenRrule is not a valid rrule
            return [];
        }
        ...
        if (isset($rrule['BYDAY'])) {
            if ($rrule['FREQ'] === 'WEEKLY') { //BYDAY=MO,TH => repeat_dow => 14
                ...
                if (isset($rrule['BYMONTH'])) {
                    $params['repeat_month'] = $rrule['BYMONTH'];
                }
            }
        }
    ...
    public function translateSugarRecurrenceToRRule(array $event): array
    {
        ...
        $repeatMonth = isset($event['month']) ? $event['month'] : null;
        ...
        if ($repeatSelector === 'Each') {
            ...
            if ($repeatType === 'Yearly') {
                $rrule['RRULE']['BYMONTH'] = $repeatMonth;
            }
        }

        if ($repeatSelector === 'On' && !empty($repeatOrdinal) && !empty($repeatUnit)) {
            $rrule = $this->setOnRRule($rrule, $repeatOrdinal, $repeatUnit, $repeatType, $dateStartFormatted, $repeatMonth);
        }
     ...
     * @return array
     */
    public function setOnRRule(array $rrule, string $repeatOrdinal, string $repeatUnit, string $repeatType, string $dateStartFormatted, string $repeatMonth): array
    {
        ...
        if (isset($ordinalMap[$repeatOrdinal])) { //regular unitMap functionality
            ...
            if (in_array($repeatUnit, $specialUnit)) {//specialUnit for Day,Weekday and Weekend day
                if ($repeatUnit === 'Day') {
                    ...
                    if ($repeatType === 'Yearly') {// first/second/third/fourth/fifth/last DAY of the YEAR on the month selected
                        $rrule['RRULE']['BYMONTHDAY'] = $ordinalMap[$repeatOrdinal];
                        $rrule['RRULE']['BYMONTH'] = $repeatMonth;
    ...
    public function getHumanReadableString(string $rruleString): string
    {
        ...
        try {
            ...
            // this means the $rruleString is not a valid rrule
            return '';
        }
    ...
    public function massageParams(array $args): array
    {
        ...
        $params['month'] = $args['repeat_month'] ?? '';
