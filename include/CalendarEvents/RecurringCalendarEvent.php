...
class RecurringCalendarEvent extends CalendarEvent
{
    ...
    /**
     * Whether the bean has been deleted via a series delete or not
     * @var boolean
     */
    public $series_deleted;
    ...
    public function generateRset()
    {
        ...
        if ($this->repeat_type  && empty($this->repeat_parent_id)) {
            $repeatFields = ['repeat_count', 'repeat_until', 'repeat_interval', 'repeat_dow', 'repeat_selector', 'repeat_days', 'repeat_ordinal', 'repeat_type', 'repeat_unit', 'date_start', 'repeat_month'];
            $newArgs = [];
    ...
    public function generateRecurrencePattern(string $rset)
    {
        ...
        $repeatFields = ['repeat_count', 'repeat_until', 'repeat_interval', 'repeat_dow', 'repeat_selector', 'repeat_days', 'repeat_ordinal', 'repeat_type', 'date_start', 'repeat_month'];
     ...
     * @inheritdoc
     */
    public function safeRetrieveRrule(SugarBean $bean): string
    {
        $rrule = '';
        ...
        try {
            ...
            // this means the $rrule is not a valid rrule
            $rrule = '';
            $bean->rset = '';
            $bean->save();
    ...
    public function save($check_notify = false, $manageRecurrence = true)
    {
        ...
        // $this->send_invites could be true, false or empty.
        if (!$this->send_invites && $this->isEmailNotificationNeeded()) {
