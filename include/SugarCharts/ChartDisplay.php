...
class ChartDisplay
{
    ...
    protected function parseChartRows()
    {
        ...
        foreach ($this->reporter->chart_rows as $row) {
            $row['cells'] = ReportUtils::formatChartRowData($this->reporter, $row['cells']);
    ...
    protected function print_currency_symbol()
    {
        ...
        if ((isset($report_defs['numerical_chart_column_type']) &&
            ...
            $currency = $this->getChartDefaultCurrency();
    ...
    /**
     * Get the default currency for the chart
     *
     * @return SugarBean
     */
    protected function getChartDefaultCurrency()
    {
        $currency = BeanFactory::newBean('Currencies')->getUserCurrency();
        $user = $currency->getCurrentUser();

        if (!$user instanceof User) {
            return $currency;
        }

        $config = SugarConfig::getInstance()->all();
        $showPreferredCurrency = $user->getPreference('currency_show_preferred');

        if (!isset($showPreferredCurrency) || (bool) $showPreferredCurrency === false) {
            $currency = BeanFactory::getBean('Currencies');
            if (is_array($config) && array_key_exists('default_currency_symbol', $config) &&
                !empty($config['default_currency_symbol'])) {
                $currencyId = $currency->retrieveIDBySymbol($config['default_currency_symbol']);
