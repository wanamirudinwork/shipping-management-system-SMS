...
(function($) {
     ...
     * @return {Array|string}
     */
    BaseChart.prototype.getFormattedLabel = function(value, context) {
        if (!_.isUndefined(this.params.plugins) &&
            !_.isUndefined(this.params.plugins.datalabels) &&
            !_.isUndefined(this.params.plugins.datalabels.formatter) &&
            _.isFunction(this.params.plugins.datalabels.formatter)) {
            return this.params.plugins.datalabels.formatter(value, context);
        }
        return false;
    };
    ...
    function FunnelChart(data, params) {
        ...
        this.isCompactChart = params.isCompactChart || false;
    }
    ...
    FunnelChart.prototype.getChartOptions = function(rules) {
        const chartConfig = {
            ...
            options: {
                ...
                maintainAspectRatio: this.params.maintainAspectRatio ? this.params.maintainAspectRatio : false,
                aspectRatio: this.params.aspectRatio ? this.params.aspectRatio : 2,
                isCompactChart: this.isCompactChart,
                sort: 'desc',
                legend: {
                    display: this.params.show_legend,
                    labels: {
                ...
                plugins: {
                    datalabels: {
                        ...
                        align: 'center',
                        formatter: (function(value, context) {
                            let label = this.getFormattedLabel(value, context);

                            if (!label) {
                                let props = this.getProperties();
                                label = this.formatNumericTicks(value, props.yDataType === 'currency', 2);
                            }
                            return label;
                        }).bind(this),
                        display: (function(context) {
        ...
        const customPlugins = this.params.customPlugins || [];
        const datalabels = this.params.datalabels || {};
        const display = this.params.display || null;
        const tooltipOptions = this.params.tooltip || {};
        const tooltipBackgroundColor = tooltipOptions.backgroundColor || '';
        const disableChartClick = this.params.disableChartClick || false;

        _.each(customPlugins, plugin => chartConfig.plugins.push(plugin));

        _.each(datalabels, (value, key) => {
            chartConfig.options.plugins.datalabels[key] = value;
        });

        if (disableChartClick) {
            chartConfig.options.onClick = null;
        }

        if (display && _.isFunction(display)) {
            chartConfig.options.plugins.datalabels.display = display;
        }

        if (tooltipBackgroundColor) {
            chartConfig.options.tooltips.backgroundColor = tooltipBackgroundColor;
        }
    ...
    SUGAR.charts = {
        ...
        getColorPalette: function() {
            ...
            const colors = baseChart.baseColorList;
        ...
        getUserPreferences: function() {
            const pref =  this.sugarApp.user.get('preferences') || {};

            if (!pref.currency_show_preferred) {
