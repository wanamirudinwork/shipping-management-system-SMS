...
class ExtAPIMicrosoftEmail extends ExternalAPIBase
{
    ...
    public function getClient(string $refreshToken = 'token'): GraphProxy
    {
        ...
        $graph = new GraphProxy($refreshToken, $config);
    ...
    public function getUserName($eapmId)
    {
        ...
        if (!empty($eapmBean->id)) {
            try {
                ...
                // Use the Graph API to get the userPrincipalName
                $client = $this->getClient($token['refresh_token'] ?? '');
    ...
    public function getEmailAddress($eapmId)
    {
        try {
            ...
            // Get a token for the EAPM bean
            $token = json_decode($eapmBean->api_data, true, 512, JSON_THROW_ON_ERROR);
    ...
    /**
     * Helper function for getting email Address from user profile
     *
     * @param GraphProxy $client
     * @return String
     */
    protected function getProfileEmailAddress($client)
    {
        // Query the Outlook REST API to get the user's information
        return $client->me()->get()->wait()->getMail();
    }

    /**
     * Takes the EAPM ID, grabs an access token from that EAPM record, and attempts
     * to make a basic call to the service (getting Email Address)
     *
     * @param string $eapmId the ID of the EAPM bean storing the account's Oauth2 token
     * @return bool  true/false if the call was successful
     */
    public function testConnection($eapmId)
    {
        $eapmBean = $this->getEAPMBean($eapmId);
        ...
        if (!empty($eapmBean->id)) {
            try {
                ...
                $client = $this->getClient($token['refresh_token'] ?? '');
                return boolval($this->getProfileEmailAddress($client));
            } catch (Exception $e) {
                $GLOBALS['log']->error($e->getMessage());
            }
        }
