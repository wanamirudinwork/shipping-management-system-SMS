...
class ExtAPIGoogleEmail extends ExternalAPIBase
{
     ...
     *
     * @param Gmail\Resource\Users $users
     * @return String
     */
    protected function getProfileEmailAddress($users)
    {
        return $users->getProfile('me')->emailAddress;
    }
    ...
    public function testConnection($eapmId)
    {
        ...
        if (!empty($eapmBean->id)) {
            try {
                ...
                if ($client->isAccessTokenExpired() && !$this->refreshToken($eapmId)) {
                    return false;
                }

                $gmailClient = new Gmail($client);
                return boolval($this->getProfileEmailAddress($gmailClient->users));
            } catch (Exception $e) {
                $GLOBALS['log']->error($e->getMessage());
            }
        }
