...
({
    events: {
        ...
        'blur .newEmail, .existingAddress': 'handleMouseLeave',
    },
    ...
    addNewAddress: function(evt){
        ...
        const elIndex = evt.delegateTarget.children.length - 2;
        let $optOutProperty = $(evt.delegateTarget.children[elIndex]).find('[data-emailproperty="opt_out"]');
        let $invalidProperty = $(evt.delegateTarget.children[elIndex]).find('[data-emailproperty="invalid_email"]');
        app.api.call('GET', app.api.buildURL('Mail/findByEmail', null, null, {'email': email}), null, {
            success: _.bind(this._updateEmailPropertiesOnAdd, this, $optOutProperty, $invalidProperty, elIndex),
        });
    },

    /**
     * Toggle opt-out property on email add
     * @param {Object} $optOutProperty
     * @param {Object} $invalidProperty
     * @param {number} elIndex
     * @param {Object} data
     */
    _updateEmailPropertiesOnAdd: function($optOutProperty, $invalidProperty, elIndex, data) {
        let existingAddresses = app.utils.deepCopy(this.model.get(this.name));
        if (data && data.opt_out && data.opt_out === 1) {
            existingAddresses[elIndex].opt_out = true;
            $optOutProperty.addClass('active');
        }
        if (data && data.invalid_email && data.invalid_email === 1) {
            existingAddresses[elIndex].invalid_email = true;
            $invalidProperty.addClass('active');
        }
    ...
    /**
     * Event handler to toggle opt-out property.
     * @param {Event} e
     */
    handleMouseLeave: function(e) {
        if (this.modelId || !e.currentTarget || !e.currentTarget.value) {
            return;
        }
        const emailRow = e.currentTarget.parentElement;
        if (emailRow.contains(e.target) && emailRow.contains(e.relatedTarget)) {
            return;
        }
        const elIndex = $(emailRow).index();
        let $optOutProperty = $(emailRow).find('[data-emailproperty="opt_out"]');
        let $invalidProperty = $(emailRow).find('[data-emailproperty="invalid_email"]');
        app.api.call('GET', app.api.buildURL('Mail/findByEmail', null, null, {'email': e.currentTarget.value}), null, {
            success: _.bind(this._updateEmailProperties, this, $optOutProperty, $invalidProperty, elIndex),
        });
    },

    /**
     * Toggle opt-out property on email edit
     * @param {Object} $optOutProperty
     * @param {Object} $invalidProperty
     * @param {number} elIndex
     * @param {Object} data
     */
    _updateEmailProperties: function($optOutProperty, $invalidProperty, elIndex, data) {
        let existingAddresses = app.utils.deepCopy(this.model.get(this.name));
        if (data && data.opt_out && data.opt_out === 1 && !$optOutProperty.hasClass('active')) {
            existingAddresses[elIndex].opt_out = true;
        } else if ((!data || !data.opt_out || data.opt_out !== 1) && $optOutProperty.hasClass('active')) {
            existingAddresses[elIndex].opt_out = false;
        }
        if (data && data.invalid_email && data.invalid_email === 1 && !$invalidProperty.hasClass('active')) {
            existingAddresses[elIndex].invalid_email = true;
        } else if ((!data || !data.invalid_email || data.invalid_email !== 1) && $invalidProperty.hasClass('active')) {
            existingAddresses[elIndex].invalid_email = false;
        }
        this.model.set(this.name, existingAddresses);
    },
})
