...
({
    ...
    updateCount: function(options) {
        if (!this.disposed) {
            if (options) {
                // cache initial data
                if (_.isUndefined(this.initialCount)) {
                    this.initialCount = options;
                }
                // use cached data if total is provided
                if (options.total) {
                    options = _.extend({}, this.initialCount, options);
                    this.initialCount = {};
                }
            }
            options = options || _.extend({}, this.initialCount);
