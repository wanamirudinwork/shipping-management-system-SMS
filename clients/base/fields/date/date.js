...
({
    ...
    _setupDatePicker: function() {
        ...
        if (this.type === 'date' && this.hidePickerAfterLoad) {
    ...
    format: function(value) {
        ...
        let formattedValue = app.date(value);

        if (!formattedValue.isValid()) {
            formattedValue = app.date(value, app.date._serverDateFormat, true);

            if (!formattedValue.isValid()) {
                formattedValue = app.date(value, app.date.convertFormat(this.getUserDateFormat()));
        ...
        return formattedValue.formatUser(true);
    },
