...
({
    ...
    saveClicked: function(evt) {
        if (!$(evt.currentTarget).hasClass('disabled')) {
            const parentModel = this.options.context.get('parentModel');
            if (!_.isUndefined(parentModel)) {
                parentModel.set('editGroupRecId', $(evt.delegateTarget).closest('tbody').data('record-id'));
            }
            this.saveModel();
        }
    },
    ...
    _fetchParentModel: function() {
        //checking if there are any did not save edits in the collection
        const ctxModel = this.context.get('model') || {};

        if (ctxModel.collection && ctxModel.collection.models.find(model => model.modelView === 'edit')) {
            return;
        }

        //addition checking if there are any did not save edits in the collection
        const optModel = this.options.model || {};
        if (optModel.collection && optModel.collection.models.find(model => model.modelView === 'edit')) {
            return;
        }
