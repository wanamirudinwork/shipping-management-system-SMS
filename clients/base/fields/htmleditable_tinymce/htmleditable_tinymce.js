...
({
    ...
    setViewContent: function(value, styleSrc = 'styleguide/assets/css/iframe-sugar.css') {
        let editable = this._getHtmlEditableField();
        if (!editable) {
            return;
        }
        // If the iframe hasn't loaded, wait until it loads
        editable.on('load', _.bind(function() {
            this._setViewContentInternal(editable, value, styleSrc);
        }, this));
        // Set the content in case the iframe has already loaded
        this._setViewContentInternal(editable, value, styleSrc);
    },

    /**
     * Sets the content displayed in the non-editor view if the iframe has loaded.
     * @param {jQuery} editable The jQuery object for the iframe.
     * @param {string} value Sanitized HTML to be placed in view
     * @param {string} styleSrc relative path to iframe_sugar.css
     */
    _setViewContentInternal: function(editable, value, styleSrc = 'styleguide/assets/css/iframe-sugar.css') {
    ...
    _setViewContentThemeStyling: function(frame) {
        try {
            ...
            frame.contentDocument.body.style.background = styles.getPropertyValue('--foreground-base');
            frame.contentDocument.body.style.color = styles.getPropertyValue('--text-base');
        } catch (e) {
    ...
    getTinyMCEConfig: function(){
        return {
            ...
            content_css: [
                ...
                'maple-syrup/build/theme/' + (app.utils.isDarkMode() ? 'sugar-dark.css' : 'sugar-light.css'),
            ],
     ...
     */
    _getContentHeight: function(editable) {
        editable = editable || this._getHtmlEditableField();
