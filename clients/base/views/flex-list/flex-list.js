...
({
    ...
    freezeFirstColumn: function(event) {
        ...
        if (freeze) {
            $firstColumns.addClass(
                `${frozenHeaderClassList} ${this.leftColumns.length ? '' : 'ltr:left-0 rtl:right-0'}`
            );
        } else {
            $firstColumns.removeClass(
                `${frozenHeaderClassList} no-border ${this.leftColumns.length ? '' : 'ltr:left-0 rtl:right-0'}`
            );
        }
    ...
    showFirstColumnBorder: function() {
        ...
        if (scrollPanel.scrollLeft === 0) {
            this.$(firstColumnSelector).removeClass(borderClass);
            this.hasFirstColumnBorder = false;
        } else if (!this.hasFirstColumnBorder) {
            this.$(firstColumnSelector).addClass(borderClass);
