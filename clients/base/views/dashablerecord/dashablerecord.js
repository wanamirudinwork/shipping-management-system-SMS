...
({
    ...
    _handleRowSaveClicked: function(event) {
        ...
        if (model) {
            ...
            model.doValidate(fieldsToValidate, (isValid) => {
                this._rowValidationComplete(row, model, isValid);
            });
        }
    },

    /**
     * Handles when the validation of a list tab row model is completed
     *
     * @param {jQuery} row the element of the row that was saved
     * @param {Bean} model the model of the row that was saved
     * @param {bool} isValid whether validation was successful or not
     * @private
     */
    _rowValidationComplete: function(row, model, isValid) {
        if (this.module === 'Quotes') {
            if (isValid) {
                this.checkConversionRateLock(() => {
                    this._saveRowModel(row, model);
                }, model);
            }
        } else {
            this._saveRowModel(row, model);
    ...
    /**
     * Handles saving of a list tab row model
     * @param {jQuery} row the element of the row that was saved
     * @param {Bean} model the model of the row that was saved
     * @private
     */
    _saveRowModel: function(row, model) {
        let cjFormBatch;
        const options = {
            success: () => {
    ...
    _addRelateFields: function(module, fields) {
        ...
        if (!_.isEmpty(vardefFieldMetadata) || !_.isEmpty(listViewFieldMetadata)) {
    ...
    _loadDataForTabs: function(tabs, options) {
        this.queueEvents = _.isArray(this.queueEvents) ? this.queueEvents : [];
                ...
                if (tab.type === 'list') {
                    ...
                    tab.model.fetch({
                        ...
                        success: _.bind(function(model) {
                            ...
                            // Action Buttons are subscribed on 'sync' event triggered from model.fetch function,
                            // but they are not initialized on this step yet.
                            // So, we need to trigger 'sync' event when other elements will be subscribed on it.
                            let syncEvent = () => model.trigger('sync', model, null, null);
                            self.queueEvents.push(syncEvent);
                        }, this)
                    });
                }
            });
    ...
    /**
     * Trigger events added to queueEvents
     * @private
     */
    _triggerQueueEvents: function() {
        if (_.isArray(this.queueEvents) && this.queueEvents.length) {
            this.queueEvents.map((item) => item());
            this.queueEvents = [];
        }
    },
     ...
     * @inheritdoc
     *
     * In dashablerecord views, header fields are contained in the toolbar
     * sibling component of the parent layout
     */
    _getHeaderField: function(fieldName) {
        let toolbarComponent = this._getToolbar();
        return toolbarComponent ? toolbarComponent.getField(fieldName) : null;
    },
