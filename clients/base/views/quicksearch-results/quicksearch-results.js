...
({
    ...
    bindDataChange: function() {
        this.listenTo(this.layout, 'quicksearch:tag:open quicksearch:tag:close', this._realignTop);
    ...
    /**
     * Realigns the top of the search results with the bottom of the tags results
     *
     * @private
     */
    _realignTop: function() {
        const $quicksearchTags = this.layout.$('.quicksearch-tags');
        const quicksearchHeight = $quicksearchTags.height() || 0;
        this.$('.typeahead').css({
            top: $quicksearchTags.hasClass('hidden') ? '100%' : `calc(100% + ${quicksearchHeight}px)`
        });
    },
