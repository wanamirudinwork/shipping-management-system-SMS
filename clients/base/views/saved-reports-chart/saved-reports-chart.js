...
({
    ...
    _refreshChart: function() {
        if (!this._chartIsVisible()) {
            this._addListenerToRefreshChart();
            return;
        }
     ...
     * @return {boolean}
     */
    _chartIsVisible: function() {
        const reportSideDrawerChart = this.layout.closestComponent('report-side-drawer-chart');

        if (reportSideDrawerChart instanceof app.view.Component && reportSideDrawerChart.isVisible()) {
            return true;
        }
    ...
    /**
     * Add listener to refresh chart
     */
    _addListenerToRefreshChart: function() {
        this.stopListening(app.events, 'list:preview:decorate');
        this.listenToOnce(app.events, 'list:preview:decorate', this._refreshChart);
    },
    ...
    setChartParams: function(serverData, update) {
        ...
        // make sure this also takes into account the consistent colors
        this.reportChartView = app.view.createView({
            name: 'report-chart',
            readonlyView: true,
            module: 'Reports',
        });

        if (this.reportChartView instanceof app.view.View &&
            typeof this.reportChartView.setupConsistentChartColors === 'function') {
            this.reportChartView.setupConsistentChartColors(serverData, settings);
        }
