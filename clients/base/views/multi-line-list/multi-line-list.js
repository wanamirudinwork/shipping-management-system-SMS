...
({
    ...
    addContextListeners: function() {
        ...
        this.listenTo(this.context, 'filter:fetch:success', function() {
            if (this.metric) {
                this.fetchCount();
            }
        }, this);
        ...
        this.listenTo(this.context, 'filter:create:open filter:create:close filter:toggle:savestate', function() {
            _.delay(_.bind(this.adjustContainerHeight, this));
        }, this);
    },

    /**
     * Update height of the list view according to height of the filter panel
     */
    adjustContainerHeight: function() {
        const filterGroupEl = this.$el.parent().find('.toggleless-filterpanel .search-filter');
        const searchH = filterGroupEl.find('> .control-group').height();
        const filterEl = filterGroupEl.find('> .filter-options:not(.hide)');
        const filterH = (filterEl && filterEl.length) ? filterEl.height() : 0;

        const h = `calc(100% - ${searchH + filterH + 108}px)`;
        this.$el.css('height', h);
    },

    /**
     * Displays the total count of records in filter result.
     */
    displayCount: function() {
        // Don't display total count if cachedCount is null or total is 0
        if (_.isNull(this.cachedCount) || this.cachedCount.total === 0) {
            this.$('.alert').addClass('hide');
            return;
        }
        let totalLabel = '';
        if (this.cachedCount.total === 1) {
            totalLabel = app.lang.get('LBL_CONSOLE_FILTER_RESULT_ONE_RECORD', this.module);
        } else if (this.cachedCount.total > 1) {
            totalLabel =  app.lang.get(
                'LBL_CONSOLE_FILTER_RESULT_MANY_RECORDS',
                this.module,
                {num: this.cachedCount.total}
            );
        }
        this.$('.alert').removeClass('hide');
        this.$('.alert-list-contents').html(totalLabel);
    },

    /**
     * Fetches the total count of records in the collection.
     */
    fetchCount: function() {
        // Don't display total count if the filter is the default filter
        if (_.isEqual(this.collection.defaultFilterDef, this.collection.filterDef)) {
            this.cachedCount = null;
            return;
        }
        // Don't fetch again if the filter is the same as the last fetch
        if (this.cachedCount &&
            _.isEqual(this.cachedCount.filter, this.collection.filterDef)) {
            return;
        }
        this.cachedCount = null;
        // If there is no more records in the collection, don't fetch total count
        if (this.collection.next_offset === -1) {
            this.cachedCount = {
                filter: this.collection.filterDef,
        ...
        if (!this.isLoadingCount) {
            ...
            this.collection.fetchTotal({
                success: _.bind(function(total) {
                    ...
                    // Update the page count in pagination
    ...
    _render: function() {
        ...
        this.$('.rc-handle').height(this.$el.height());
        this.displayCount();
    },
    ...
    _dispose: function() {
        ...
        this.$el.closest('.dashboard').css('overflow-y', '');
