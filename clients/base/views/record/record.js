...
({
    ...
    initialize: function(options) {
        ...
        this.adjustHeaderpane = _.bind(_.debounce(this.adjustHeaderpane, 10), this);
    ...
    handleAclChange: function(diff) {
        ...
        _.each($pencils, function(pencilEl) {
            const $pencilEl = $(pencilEl);
            let field = $pencilEl.data('name');

            let isEditable = _.isUndefined(noEditFieldsMap[field]);

            if (isEditable) {
                field = $pencilEl.closest('.record-cell').data('name');
                isEditable = _.isUndefined(noEditFieldsMap[field]);
            }
    ...
    hasUnsavedChanges: function() {
        ...
        let tags = _.clone(this.model.get('tag'));
        if (tags && tags.length > 0) {
            tags.forEach(item => {
                if (item.encodedValue === encodeURIComponent(item.name)) {
                    delete item.encodedValue;
                }
            });

            if (_.isEqual(changedAttributes.tag, tags)) {
                return false;
            }
        }
    ...
    setTitle: function(title) {
        ...
        if ($title.length > 0) {
            ...
            this.$('.headerpane h1').prepend('<span class="record-cell"><span class="module-title">' +
    ...
    adjustHeaderpaneFields: function() {
        ...
        let $recordCells = this._getRecordCells();
        ...
        if ($recordCells && ($recordCells.length > 0) && (this.getContainerWidth() > 0)) {
            if ($recordCells.length === 1 && $($recordCells[0]).hasClass('btn-toolbar')) {
                ...
                let $ellipsisCell = $(this._getCellToEllipsify($recordCells));

                if ($ellipsisCell.length) {
                    if ($ellipsisCell.hasClass('edit')) {
                        // Make the ellipsis cell widen to 100% on edit
                        $ellipsisCell.css({'width': '100%'});
                        this._setMaxWidthForEllipsifiedCell($ellipsisCell, $ellipsisCell.width());
                    } else {
                        let ellipsisCellWidth = this._calculateEllipsifiedCellWidth($recordCells, $ellipsisCell);
    ...
    _setMaxWidthForEllipsifiedCell: function($ellipsisCell, width) {
        ...
        if (fieldType === 'fullname' || fieldType === 'dashboardtitle') {
            ellipsifiedCell = this._getHeaderField($ellipsisCell.data('name'));
            if (ellipsifiedCell) {
                width -= ellipsifiedCell.getCellPadding();
                ellipsifiedCell.setMaxWidth(width);
            }
        } else {
            $ellipsisCell.children().css({'max-width': (width - 2) + 'px'});
        }
    },

    /**
     * Returns the instance of the field with the given name. In record views,
     * header fields are contained in the record metadata
     *
     * @param {string} fieldName The name of the field
     * @return {Object} The instance of the field
     * @private
     */
