...
({
    ...
    hasUnsavedChanges: function() {
        ...
        let defaults = this.model.getDefault() || {};
        let nonDefaultedAttributesChanged = this._nonDefaultedAttributesHasChanged(defaults);
        let defaultedAttributesChanged = !_.isEmpty(this.model.changedAttributes(defaults));

        return (this.model.isNew() && (nonDefaultedAttributesChanged || defaultedAttributesChanged));
    },

    /**
     * Check if non-defaulted attributes have changed.
     * @param {Object} defaults The default attributes
     * @return {boolean} `true` if non-defaulted attributes have changed, `false` otherwise.
     */
    _nonDefaultedAttributesHasChanged: function(defaults) {
        let nonDefaultedAttributesChanged = !_.isEqual(_.keys(defaults).sort(), _.keys(this.model.attributes).sort());
        if (nonDefaultedAttributesChanged) {
            // check if the non-defaulted attributes have changed
            let changedFields = _.difference(_.keys(this.model.attributes), _.keys(defaults));
            nonDefaultedAttributesChanged = _.some(changedFields, function(changedField) {
                let field = this.model.fields[changedField];
                let result = false;
                if (field) {
                    if (field.type === 'collection') {
                        result = this.model.get(changedField).length > 0;
                    } else if (field.type === 'relate' || field.type === 'link') {
                        result = this.model.get(changedField) != null && this.model.get(changedField).id != null;
                    } else {
                        result = this.model.get(changedField) != null;
                    }
                }
                return result;
            }, this);
        }
        return nonDefaultedAttributesChanged;
    },
    ...
    saveModel: function (success, error) {
        ...
        options = {
            ...
            //Add all parent module record view fields to the response
            params: {
                view: 'record',
            }
        };
