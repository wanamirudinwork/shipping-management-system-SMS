...
({
    ...
    bindDataChange: function() {
        ...
        $(window).on(`resize.${this.cid}`, _.bind(this._resizePhaserCanvas, this));
    ...
    _getTreeState: function() {
        ...
        var treeState = {
             ...
             * @private
             */
            _onResize: function(changes) {
                this._updateGameWorldSize(changes);
            },
             ...
             * @private
             */
            _updateGameWorldSize: function(changes) {
                ...
                // In RTL mode, we need to shift all game objects horizontally
                // by the change in width to keep them aligned properly, since
                // Phaser x values are based on the left side of the object
                if (this.isLangRTL && changes && changes.canvasWidth) {
                    this._shiftAllObjectsX(Math.floor(changes.canvasWidth.new) - Math.floor(changes.canvasWidth.prev));
                }
            ...
            /**
             * Shifts the X position of all objects in the canvas by the specified amount
             *
             * @param {number} xChange
             * @private
             */
            _shiftAllObjectsX: function(xChange) {
                if (xChange && this.rootGroup && this.rootGroup.childGroup) {
                    this.rootGroup.childGroup.setAllChildren('x', xChange, false, false, 1);
                }
            },
            ...
            drawScrollbar: function() {
                let scrollX = this.isLangRTL ? 0 : this.gameWorldWidth - this.scrollBarBkgdWidth;
                let xOffset = 4;
    ...
    _openItemInDrawer: function(data) {
        ...
        }, _.bind(this._onItemInDrawerIsClosed, this));
    },

    /**
     * Callback when the ProductTemplate item is closed in a Drawer layout
     *
     * @protected
     */
    _onItemInDrawerIsClosed: function() {
        setTimeout(() => {
            this._resizePhaserCanvas();
        }, 1000);
    },
    ...
    _resizePhaserCanvas: function() {
        ...
        if (this.phaser && $el.length && this.phaser.scale) {
            let prevCanvasWidth = this.canvasWidth;
            let prevCanvasHeight = this.canvasHeight;

            this._calculateCanvasSizes($el);
            this.phaser.scale.setGameSize(this.canvasWidth, this.canvasHeight);

            // Manually set the height and width properties of the canvas here
            // immediately, since Phaser has a delay sometimes before doing so
            $el.find('canvas').css({
                width: this.canvasWidth,
                height: this.canvasHeight
            });
            ...
            this.phaser.events.onResize.dispatch({
                canvasWidth: {
                    prev: prevCanvasWidth,
                    new: this.canvasWidth,
                },
                canvasHeight: {
                    prev: prevCanvasHeight,
                    new: this.canvasHeight
                }
            });
        }
    },
