...
({
    ...
    doubleClickEdit: function(event) {
        ...
        let modelId = rowName.indexOf(`${this.module}_`) === 0 ? rowName.substr(`${this.module}_`.length) : null;
    ...
    handleListPaginationEvents: function(callback) {
        // Discard unsaved changes before pagination
        if (this.toggledModels && !_.isEmpty(this.toggledModels)) {
            _.each(this.toggledModels, function(model) {
                if (model.hasChanged()) {
                    model.set(model.getSynced(), {silent: true});
                }
            });
        }
