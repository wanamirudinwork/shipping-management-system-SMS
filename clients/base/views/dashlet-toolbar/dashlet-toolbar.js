...
({
    ...
    adjustHeaderPaneTitle: function() {
        let fullNameField = _.findWhere(this.headerFields, {type: 'fullname'});
        if (_.isEmpty(fullNameField)) {
            return;
        }

        // Calculate the width of the headerpane
        let headerPaneWidth = this.$el.closest('.dashlet-header').width();
        ...
        let btnGroupWidth = this.$('.btn-toolbar').outerWidth(true);
        ...
        let titleWidth = headerPaneWidth - btnGroupWidth - recordCellsWidth;
