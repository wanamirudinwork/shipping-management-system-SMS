...
({
    ...
    displayItems: function(data) {
        ...
        if (this.meta.drive_type === 'sharepoint') {
        ...
        app.api.call('read', url, null, {
            success: _.bind(function(result) {
                if (result.success === false) {
                    ...
                    this.variablePath = result.variablePath;
    ...
    _setupDashletToolbar: function() {
        ...
        let cloudDriveButtons = dashlet.find('.cd-radio-buttons');

        if (rightDashletButtons.length !== 1) {
            return;
        }

        if (cloudDriveButtons.length) {
            cloudDriveButtons.remove();
        }

        const toolbarTemplate = app.template.getView('cloud-drive', 'toolbar-buttons');
        cloudDriveButtons = $(toolbarTemplate(this));
