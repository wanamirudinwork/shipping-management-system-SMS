...
class RelateRecordApi extends SugarApi
{
    ...
    /**
     * A list of fields for which we disallow update through updateRelatedLink
     *
     * @var array
     */
    protected array $disabledUpdateFields = [
        'deleted',
        'created_by',
    ];

    /**
     * List of the additional user fields required on a User page.
     *
     * @var array
     */
    protected $userFields = [
        'mail_credentials', 'is_group', 'portal_only', 'receive_notifications', 'export_delimiter',
        'send_email_on_mention', 'default_export_charset', 'use_real_names', 'reminder_time', 'team_name',
        'appearance', 'email_reminder_time', 'site_url', 'user_tabs', 'number_pinned_modules',
        'field_name_placement', 'datef', 'default_currency_significant_digits', 'timef', 'num_grp_sep', 'timezone',
        'dec_sep', 'currency', 'default_locale_name_format', 'currency_show_preferred',
        'currency_create_in_preferred', 'sugarpdf_pdf_font_name_main', 'sugarpdf_pdf_font_size_main',
        'sugarpdf_pdf_font_name_data', 'sugarpdf_pdf_font_size_data', 'sugarpdf_pdf_calendar_font_name_data',
        'calendar_publish_key', 'calendar_publish_location', 'calendar_search_location',
        'calendar_ical_subscription_url', 'fdow', 'user_role_access', 'downloads', 'email_link_type',
    ];
    ...
    protected function checkRelatedSecurity(
        ...
        if ($securityTypeRemote === 'edit' && !empty($args['remote_id'])) {
            $linkSeed->retrieve($args['remote_id']);
        }
    ...
    {
        SugarBean::enterOperation('create_related_record');
    ...
    public function createRelatedLinks(
        ...
        $args = $this->addFieldsForModuleUsers($args);
        $primaryBean = $this->loadBean($api, $args);
        ...
        if (!SugarBean::inOperation('create_related_record')) {
    ...
    /**
     * Add additional fields for module Users and ACLRoles
     *
     * @param $args
     * @return array
     */
    public function addFieldsForModuleUsers($args): array
    {
        if (array_key_exists('link_name', $args) && $args['link_name'] === 'aclroles'
            && array_key_exists('module', $args) && $args['module'] === 'Users'
        ) {
            $args['erased_fields'] = true;
            // 39 additional fields to fill in the User page
            $args['fields'] = implode(',', $this->userFields);
        }
    ...
    public function deleteRelatedLink(ServiceBase $api, array $args)
    {
        ...
        $interactionsHelper = new InteractionsHelper($relatedBean);
