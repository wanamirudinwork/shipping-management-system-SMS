...
{
    /**
     * @var array List of fields that are not allowed to be used in filters for specific modules
     */
    private static array $forbiddenModulesFilterFields = [
        'Users' => [
            'user_hash',
        ],
    ];
    ...
    /**
     * @var array List of fields that are not allowed to be used in filters for current module
     */
    private static array $forbiddenFilterFields = [];
    ...
    protected function parseArguments(ServiceBase $api, array $args, ?SugarBean $seed = null)
    {
        ...
        $options['fields_modified'] = !empty($args['fields']) && is_string($args['fields']) ? $args['fields'] : '';
        ...
        if (!empty($seed) && !empty($seed->field_defs)) {
            foreach ($seed->field_defs as $field => $def) {
                ...
                if (isset($args['fields']) && !empty($def['linked_fields']) && is_array($def['linked_fields'])) {
                    foreach ($def['linked_fields'] as $field) {
                        $options['select'][] = $field;

                        if (!empty($options['fields_modified'])) {
                            $options['fields_modified'] .= ',' . $field;
                        }
                    }
                }
            }
    ...
    public function filterListSetup(ServiceBase $api, array $args, $acl = 'list')
    {
        ...
        // When the fields parameter is modified from the above function set the new value to $args['fields']
        if (!empty($args['fields']) && !empty($options['fields_modified']) && $args['fields'] !== $options['fields_modified']) {
            $args['fields'] = $options['fields_modified'];
        }
        ...
        static::$forbiddenFilterFields = static::$forbiddenModulesFilterFields[$args['module']] ?? [];
    ...
    /**
     * Filter records related to not available modules
    ...
    public function filterList(ServiceBase $api, array $args, $acl = 'list')
    {
        ...
        $this->restrictAccessToAdminOnlyModules($api, $args);
    ...
    protected function runQuery(ServiceBase $api, array $args, SugarQuery $q, array $options, ?SugarBean $seed = null)
    {
        ...
        if (isset($options['id_query'])) {
            ...
            if (array_key_exists('forcedOffset', $args) && $args['forcedOffset'] > 0) {
                $offset = $args['forcedOffset'];
            }

            if (array_key_exists('forcedLimit', $args) && $args['forcedLimit'] > 0) {
                $limit = $args['forcedLimit'];
            }
    ...
    protected static function addFilters(array $filterDefs, SugarQuery_Builder_Where $where, SugarQuery $q)
    {
        foreach ($filterDefs as $filterDef) {
            ...
            foreach ($filterDef as $field => $filter) {
                if (in_array($field, self::$forbiddenFilterFields)) {
                    throw new SugarApiExceptionNotAuthorized("Access for field $field is not allowed");
                }
    ...
    protected static function addFieldFilter(SugarQuery $q, SugarQuery_Builder_Where $where, $filter, $field)
    {
        ...
        foreach ($filter as $op => $value) {
            ...
            // to avoid the issue of losing data when joining the team_set with an $empty filter
            // https://sugarcrm.atlassian.net/browse/BR-11737
            if ($op === '$empty' && $value === '') {
                $emptyFilters = $q->getCustomParam('emptyFilter');

                if (!is_array($emptyFilters)) {
                    $emptyFilters = (array) $emptyFilters;
                }

                $emptyFilters[] = $field;
                $q->setCustomParam('emptyFilter', $emptyFilters);
            }
