...
({
    ...
    /**
     * Apply custom style to the dashlet
     */
    _applyCustomStyle: function() {
        const dashletMeta = _.first(this._components) ? _.first(this._components).meta : false;

        if (!dashletMeta || !dashletMeta.dashletConfig || !dashletMeta.dashletConfig.styleConfig) {
            return;
        }

        const styleConfig = dashletMeta.dashletConfig.styleConfig;

        this._applyCustomStyleToElement('.dashlet-container', styleConfig.container);
        this._applyCustomStyleToElement('[data-dashlet="dashlet"]', styleConfig.body);
        this._applyCustomStyleToElement('.dashlet-title', styleConfig.title);
        this._applyCustomStyleToElement('.dashlet-header', styleConfig.header);
        this._applyCustomStyleToElement('.btn-toolbar i', styleConfig.buttons);
    },

    /**
     * Apply custom style to the dashlet elements
     *
     * @param {string} selector
     * @param {Object} config
     */
    _applyCustomStyleToElement: function(selector, config) {
        if (!config) {
            return;
        }

        if (config.style) {
            this.$(selector).css(config.style);
        }

        if (config.class) {
            this.$(selector).toggleClass(config.class, true);
        }

        if (config.icon) {
            this._addCustomIconToElement(selector, config.icon);
        }
    },

    /**
     * Add custom icon to the element
     *
     * @param {string} selector
     * @param {Object} iconConfig
     */
    _addCustomIconToElement: function(selector, iconConfig) {
        if (!iconConfig) {
            return;
        }
        ...
        if (iconClass && !this.$(selector).find(iconClass).length) {
            this.$(selector).prepend(`<i class="${iconClass}"></i>`);
        }

        if (iconStyle) {
            this.$(selector).find('.' + iconClass.replaceAll(' ', '.')).css(iconStyle);
        }
    },
    ...
    collapse: function(collapsed) {
        ...
        this.$('.dashlet-header').toggleClass('border-b', !collapsed);
