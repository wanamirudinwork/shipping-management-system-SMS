...
({
    ...
    _open: function(def, onClose, sideDrawerClick) {
        ...
        if (def) {
            ...
            const module = (def.context && def.context.module) ?
                def.context.module :
                (def.context && typeof def.context.get === 'function' ? def.context.get('module') : null);

            def.fdTitle = module ?
                app.lang.get('LBL_MODULE_NAME_SINGULAR', module) :
                (def.context && def.context.dataTitle && def.context.dataTitle.module) ?
                     def.context.dataTitle.module : '';
    ...
    /**
     * Tells if an element is contained in the drawer.
     * @param {jQuery} el The element to check.
     * @return {boolean} True if the element is in the drawer, false otherwise.
     */
