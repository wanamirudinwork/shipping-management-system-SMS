...
({
    ...
    processToggles: function(options) {
        ...
        if (subpanels.length) {
            ...
            _.each(this.availableToggles, function(toggle) {
                let disabled = this._setDisabled(toggle) || this._isToggleDisabled(options, toggle.name);
                if (!disabled) {
                    let tempToggle = {
                        toggle: toggle.name,
                        title: toggle.label,
    ...
    /**
     * Check component metadata if the toggle was explicitly disabled
     * @param {Object} options
     * @param {string} toggleName
     * @return {boolean}
     */
    _isToggleDisabled: function(options, toggleName) {
        options = options || {};
        if (_.isUndefined(options.meta) ||
            _.isUndefined(options.meta.availableToggles)) {
            return true;
        }

        const toggleItem = _.find(options.meta.availableToggles, function(item) {
            if (item.name === toggleName) {
                return true;
            }
        });

        if (_.isUndefined(toggleItem) || toggleItem.disabled) {
            return true;
        }
    ...
    _setDisabled: function(toggle) {
        if (toggle.name === 'pipeline') {
            ...
            if (this.options.module === 'Activities') {
                return false;
            } else {
     ...
     * @return void
     */
    _appendComponent: function(component) {
        let target;

        switch (component.name) {
            case 'multi-line-sorting':
            case 'multi-line-list-filter':
            case 'filter':
                target = '.search-filter>.control-group>.controls.btn-group-fit';
                break;
            default:
                target = '.main-content';
        }

        this.$(target).append(component.el);
    },
