...
({
    ...
    /**
     * Checks if {@link #$el this layout element} is in a drawer.
     * In case no drawers are open, this function returns `true`.
     *
     * @return {boolean} `true` if no drawers are open, or if
     *   this layout is in a drawer, `false` otherwise.
     */
    _isActive: function() {
        if (_.isEmpty(app.drawer) || !app.drawer.getActive()) {
            return _.isEmpty(app.sideDrawer) || !app.sideDrawer.isOpen() ||
                app.sideDrawer.contains(this.$el);
        } else {
