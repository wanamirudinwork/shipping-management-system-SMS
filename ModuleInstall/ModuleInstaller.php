...
class ModuleInstaller
{
    ...
    /**
     * Scans a package's manifest for disabled actions, and then scans the PHP files
     * for restricted function calls
     */
    public function scan(string $baseDir): string
    {
        if ((defined('MODULE_INSTALLER_PACKAGE_SCAN') && MODULE_INSTALLER_PACKAGE_SCAN)
            ...
            $this->setInstallationPackageScan();
            $this->ms->scanPackage($baseDir);
            if ($this->ms->hasIssues()) {
                if ($this->silent) {
                    ...
                    return SugarCleaner::cleanHtml(ob_get_clean());
                }
            }
        }
    ...
    /**
     * Scans a package using Rector lib to detect possible PHP version compatibility issues
     */
    public function rectorScan(string $baseDir, array $fileList): ?string
    {
        $reportChunk = $this->ms->rectorScanPackage(array_map(fn(string $f) => $baseDir . $f, $fileList));
        if ($reportChunk != '') {
            if ($this->silent) {
                $e = new SugarException('LBL_UPGRADE_WIZARD_INVALID_PKG', null, 'Administration');
                $e->setExtraData('error_description', $this->ms->getFormattedIssues());
    ...
    public function install($base_dir, $is_upgrade = false, $previous_version = '')
    {
        ...
        if (file_exists($this->base_dir . '/manifest.php')) {
            ...
            Container::getInstance()->get(InMemory::class)->delete($cache_key);
            Container::getInstance()->get(InMemory::class)->clear();
            ...
            // Get the newest app_dropdowns_style
            DropdownsManager::synchronizeDropdownsStyle();
            $GLOBALS['app_dropdowns_style'] = DropdownsManager::returnAppDropdownsStyle();
 ...
 */
    ...
    public function uninstall($base_dir)
    {
        ...
        if (file_exists($this->base_dir . '/manifest.php')) {
            ...
            if (!$this->silent) {
                $current_step++;
                $this->log(translate('LBL_QUICK_REPAIR_AND_REBUILD_PROCESSING'));
                $this->setInstallationProgress($current_step, $total_steps);
                update_progress_bar('install', $current_step, $total_steps);
                echo '</div>';
            }
            ...
            if (!$this->silent) {
                $current_step++;
                $this->log(translate('LBL_QUICK_REPAIR_AND_REBUILD_FINISHED'));
                $this->setInstallationProgress($current_step, $total_steps);
                update_progress_bar('install', $current_step, $total_steps);
                echo '</div>';
            }
    ...
    public static function getPortalConfig()
    {
        ...
        $portalConfig = [
            ...
            'serverUrl' => $config->get('site_url') . '/rest/v11_26',
    ...
    public function getInstallationProgress(): array
    {
        ...
        return [
            ...
            'pre_install_step' => $uh['pre_install_step'] ?? '',
            'files_for_rector_scan' => $uh['files_for_rector_scan'] ?? [],
            'files_for_rector_scan_initial_count' => $uh['files_for_rector_scan_initial_count'] ?? 0,
            'rector_scan_report_chunks' => $uh['rector_scan_report_chunks'] ?? [],
            'rector_scan_package_dir' => $uh['rector_scan_package_dir'] ?? null,
        ];
    }
    ...
    public function setInstallationPreInstallStep(string $stepName): void
    {
        $data = [];
        ...
        $data['pre_install_step'] = $stepName;
        $this->setInstallationProgressData($data);
    }

    public function setInstallationPreCheck(): void
    {
        $data = ['pre_install_step' => 'preCheck'];
        $this->setInstallationProgressData($data);
    }

    public function setInstallationPackageScan(): void
    {
        $data = $this->getInstallationProgress();
        $data['pre_install_step'] = 'packageScan';
        $this->setInstallationProgressData($data);
    }

    public function setInstallationRectorScan(array $fileList, ?string $reportChunk = '', int $chunkSize = 0, ?string $packageDir = null): array
    {
        $data = $this->getInstallationProgress();
        $data['pre_install_step'] = 'rectorScan';
        $data['files_for_rector_scan'] = $fileList;
        if ($packageDir !== null) {
            $data['rector_scan_package_dir'] = $packageDir;
        }
        if ($data['files_for_rector_scan_initial_count'] === 0) {
            $data['files_for_rector_scan_initial_count'] = count($fileList);
        }

        if (empty($fileList)) {
            $chunkStart = max($data['files_for_rector_scan_initial_count'] - $chunkSize, 0);
            $chunkEnd = $data['files_for_rector_scan_initial_count'];
        } else {
            $chunkStart = $data['files_for_rector_scan_initial_count'] - count($fileList);
        ...
        $chunkLabel = sprintf(
            '%d - %d / %d',
            $chunkStart,
            max($chunkEnd, $data['files_for_rector_scan_initial_count']),
            $data['files_for_rector_scan_initial_count'],
        );
        $data['rector_scan_report_chunks'][$chunkLabel] = $reportChunk;
        $this->setInstallationProgressData($data);
    ...
    public function setInstallationErrorPage(): void
    {
        $data = $this->getInstallationProgress();
