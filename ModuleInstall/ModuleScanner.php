...
use Sugarcrm\Sugarcrm\FeatureToggle\Features\RectorModuleScanner;
...
class ModuleScanner
{
    ...
    private $validExt = ['png', 'gif', 'jpg', 'css', 'js', 'php', 'txt', 'html', 'htm', 'tpl', 'pdf', 'md5', 'xml', 'hbs', 'less', 'wsdl', 'json',];
    private $classBlackList = [
        ...
        'sugarcrm\sugarcrm\packagebuilder\filehandlertrait',
        'sugarcrm\sugarcrm\packagebuilder\filehandlertrait_uploadstream',
    ];
    private $blackList = [
        'class_alias',
        'popen',
    ...
    public function __construct()
    {
        ...
        if ($features->isEnabled(RectorModuleScanner::getName())) {
            $this->rectorScanner = new RectorScanner();
        }
    }
    ...
    public function isPackageScanEnabled(): bool
    {
        return empty($this->config['disableFileScan']);
    }

    public function isRectorScanEnabled(): bool
    {
        return $this->isPackageScanEnabled() && $this->rectorScanner !== null;
    }
        ...
        if (isset($installdefs['copy'])) {
            foreach ($installdefs['copy'] as $copy) {
                if (!isset($copy['from'])) {
                    trigger_error(
                        sprintf(
                            'Trying to access array offset %s',
                            !is_array($copy) ? 'on ' . gettype($copy) : ': $copy[from]'
                        )
    ...
    public function rectorScanPackage(array $fileList): ?string
    {
        if (!$this->isRectorScanEnabled()) {
            return null;
        }

        return $this->rectorScanner->scan($fileList);
    }

    public function rectorScanGetReport(string $path, array $reportChunks): ?array
    {
        /**
         * @var array $manifest
         */
        ...
        $rectorIssue = $this->rectorScanner->createIssueFromReportChunks($reportChunks);

        if (is_array($rectorIssue)) {
            ['message' => $issueMessage, 'url' => $reportUrl] = $rectorIssue;
            $issueMessage = translate($issueMessage);
            if (is_string($reportUrl) && strlen($reportUrl) > 0) {
                $issueMessage .= ' <a target="_blank" href="' . Escape::htmlAttr($reportUrl) . '">' . translate('ML_PHP_COMPATIBILITY_DOWNLOAD_REPORT') . '</a>';
            }

            return ['php_compatibility' => [$packageName => $issueMessage]];
        }
     ...
     **/
    public function displayIssues($package = 'Package', ?array $issueList = null)
    {
        ...
        foreach ($issueList !== null ? $issueList : $this->issues as $type => $issues) {
            $echoType = $type === 'php_compatibility' ? translate('ML_PHP_COMPATIBILITY') : ucfirst($type);
            echo '<div class="error"><h2>' . $echoType . ' ' . translate('ML_ISSUES') . '</h2> </div>';
