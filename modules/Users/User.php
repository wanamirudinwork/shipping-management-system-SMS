...
class User extends Person
{
    ...
    public $site_user_id;
    private const USERS_TEAMS_ROLES = ['ACLRoles', 'Teams'];
    ...
    protected function checkIfSaveExceedsLicenseSeats()
    {
        ...
        $hasInvalidMarketLicenseType = $this->hasInvalidMarketLicenseType();
        if (!empty($userLicensesOverCapacity) || $hasInvalidMarketLicenseType) {
            $this->handleSaveExceedsLicenseSeats($userLicensesOverCapacity, $hasInvalidMarketLicenseType);
        }
    }
     ...
     */
    protected function handleSaveExceedsLicenseSeats(array $userLicensesOverCapacity, bool $wrongMarketLicense = false)
    {
        ...
        if ($wrongMarketLicense) {
            LoggerManager::getLogger()->error('Sugar Market license must be assigned with a Sell license.');
        }
        ...
        if (!empty($this->external_auth_only) || isFromApi()) {
            ...
            if ($wrongMarketLicense) {
                $msg .= ' ' . translate('WARN_MARKET_SEAT_FOR_SERVE', 'Administration');
            }
    ...
    protected function hasInvalidMarketLicenseType()
    {
        // Inactive Users do not count toward license seats
        if ($this->status !== 'Active') {
            return false;
        ...
        $licenseTypes = $this->getTopLevelLicenseTypes();
        if (empty($licenseTypes) || !in_array(Subscription::SUGAR_MARKET_KEY, $licenseTypes)) {
            return false;
        }

        foreach ($licenseTypes as $licenseType) {
            if (Subscription::isMangoKey($licenseType) && $licenseType !== Subscription::SUGAR_SERVE_KEY) {
                return false;
            }
        }
    ...
    public function getSystemLicenseTypesSelections(): array
    {
        ...
        foreach (array_keys($subscriptions) as $type) {
            if (!in_array($type, SubscriptionManager::instance()->getAllHiddenProducts())) {
    ...
    /**
     * Gets the options for the PDF Calendar Export
     *
     * @return array the list of PDF Font dropdown options
     */
    public function getCalendarExportFontListOptions()
    {
        $fontManager = new FontManager();
        return $fontManager->getTTFFontList();
    }
    ...
    protected function executeRetrieveQuery($id, bool $deleted, bool $verifyDBfields): array
    {
        $query = new \SugarQuery();
    ...
    public function verify_data($ieVerified = true)
    {
        ...
        if (!empty($this->getNonCrmTypesArePartOfSelectedBundle($licenseType))) {
            if (isFromApi()) {
                throw new SugarApiExceptionInvalidParameter(translate('ERR_USER_LICENSE_TYPE_OFFERRED_IN_BUNDLES', $this->module_name));
            }
    ...
    public function getEmailClientPreference()
    {
        ...
        if (isset($_SESSION['isMobile'])) {
            ...
            $client = 'other';
        } elseif (!$this->isSystemEmailConfigured()) {
            // check if system email is not configured, so return the only possible option - an external client
            $client = 'mailto';
        } else {
    ...
    protected function _getModulesForACL($type = 'dev')
    {
        ...
        // BR-11505 allow to edit not only users, but role and teams on the user's page
        if (in_array($this->module_name, $myModules)) {
            $myModules = array_merge($myModules, self::USERS_TEAMS_ROLES);
        }
    ...
    {
        $allSys = array_keys(SubscriptionManager::instance()->getAllSystemSubscriptionKeys());
        return array_diff($allSys, SubscriptionManager::instance()->getAllHiddenProducts());
    }
    ...
    {
        $allTop = array_keys(SubscriptionManager::instance()->getTopLevelSystemSubscriptionKeys());
        return array_diff($allTop, SubscriptionManager::instance()->getAllHiddenProducts());
    }
    ...
    final public function hasLicenses(array $requiredLicenses): bool
    {
        ...
        if ($sm->hasSubscription(Subscription::SUGAR_MARKET_KEY, true)) {
            $licenses = array_merge($licenses, [Subscription::SUGAR_MARKET_KEY]);
        }
    ...
    public function updateUsersLicenseTypes(array $licenseTypes = [], bool $buildCache = true)
    {
        ...
        $hasMarketKey = $this->hasMarketKey();
        ...
        foreach ($users as $userData) {
            ...
            if ($userBean && $userBean->needToUpdateLicenseType($licenseTypes)) {
                ...
                $newLicenseTypes = $this->addMarketKey($newLicenseTypes);
    ...
    /**
     * check if system subscription contains market key
     * @return bool
     */
    protected function hasMarketKey() : bool
    {
        return SubscriptionManager::instance()->hasSubscription(Subscription::SUGAR_MARKET_KEY);
    }

    /**
     * add market key if the system contains market key
     * and user's license type contains sell, ent
     * @param array $licenseTypes
     * @return array
     */
    protected function addMarketKey(array $licenseTypes) : array
    {
        if (!$this->hasMarketKey()) {
            return $licenseTypes;
        }
        
        $canAssignMarketKey = false;
        // check if the user has any auto-assigning market keys
        foreach ($licenseTypes as $type) {
            if (Subscription::canAutoAssignMarketKey($type)) {
                $canAssignMarketKey = true;
                break;
            }
        }

        if ($canAssignMarketKey) {
            $licenseTypes = array_unique(array_merge($licenseTypes, [Subscription::SUGAR_MARKET_KEY]));
        }
        return $licenseTypes;
    }
    ...
    /**
     * update all users with MARKET license
     * @throws SugarApiExceptionNotFound
     * @throws SugarQueryException
     */
    public function updateUsersMarketLicenseType() : void
    {
        ...
        foreach ($users as $userData) {
            $userBean = BeanFactory::getBean('Users', $userData['id']);
            if ($userBean && $userBean->status === 'Active') {
                $userLicenseTypes = $userBean->getLicenseTypes();
                if (!safeInArray(Subscription::SUGAR_MARKET_KEY, $userLicenseTypes)
                    && !empty(array_intersect($userLicenseTypes, Subscription::AUTO_ASSIGNING_MARKET_KEYS))) {
                    $userLicenseTypes = array_merge($userLicenseTypes, [Subscription::SUGAR_MARKET_KEY]);
                    $userBean->setLicenseType($userLicenseTypes);
                    $userBean->save();
                }
            }
        }
