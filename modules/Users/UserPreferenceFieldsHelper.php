...
class UserPreferenceFieldsHelper
{
    ...
    /**
     * Retrieves the sugarpdf_pdf_calendar_font_name_data preference, or one of its
     * fallbacks
     *
     * @param SugarBean $user the User whose preferences are being fetched
     * @return string the sugarpdf_pdf_calendar_font_name_data preference value
     */
    protected function get_sugarpdf_pdf_calendar_font_name_data(SugarBean $user)
    {
        $pref = $user->getPreference('sugarpdf_pdf_calendar_font_name_data');
        if (empty($pref)) {
            include_once 'include/Sugarpdf/sugarpdf_config.php';
            $pref = PDF_CALENDAR_FONT_NAME_DATA;
        }
