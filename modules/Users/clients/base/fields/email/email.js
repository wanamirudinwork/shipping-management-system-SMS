...
({
    ...
    _getAdditionalParams: function(email) {
        return {
            disabledPrimary: app.config.idmModeEnabled && !this.model.get('is_group') &&
                !this.model.get('portal_only'),
            disabled: app.config.idmModeEnabled && email.primary_address && !this.model.get('is_group') &&
                !this.model.get('portal_only')
        };
    }
})
