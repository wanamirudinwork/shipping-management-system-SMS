...
({
    ...
    /**
     * Stores Authorized Email Account status
     */
    status: undefined,

    /**
     * Stores Connected status
     */
    ...
    bindDataChange: function() {
        ...
        if (this.model) {
            this.listenTo(this.model, 'change:mail_credentials', () => {
                let mailCredentials = this.model.get('mail_credentials') || {};
                let smtpType = mailCredentials.mail_smtptype || '';
                let eapmId = mailCredentials.eapm_id || '';
                this._testConnection(smtpType, eapmId);
            });

            this.listenTo(this.view.context, 'change:connectionStatus', function() {
                this.status = this.view.context.get('connectionStatus');
                this.isConnected = this.view.context.get('isConnected');
                this.render();
            }, this);
        }
    },
     ...
     */
    _testConnection: function(smtpType, eapmId) {
        ...
        let callback = {
            success: (data) => {
                ...
                this.view.context.set({
                    connectionStatus: app.lang.get(this._getStatus(eapmId, data.isConnected)),
     ...
     * Gets current status
     */
    _getStatus: (eapmId, isConnected) =>
        (eapmId && isConnected) ? 'LBL_MAIL_AUTHORIZED_ACCOUNT' : 'LBL_MAIL_NOT_AUTHORIZED_ACCOUNT',
