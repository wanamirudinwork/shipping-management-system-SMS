...
({
    ...
    resetDefaults: function() {
        this._super('resetDefaults');
        if (this.viewBy === 'RevenueLineItems' && this.hasRliAccess) {
            // now lets check for RLI's
            let rliContext = this.context.getChildContext({link: 'revenuelineitems'});
            rliContext.prepare();

            // there should be only one record in the related context collection
            // when sugarlogic is initialized on create view
            if (rliContext.get('collection').length === 1) {
                let model = rliContext.get('collection').at(0);
                model.setDefault(model.attributes);
            }
        }
    },
