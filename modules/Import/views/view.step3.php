...
class ImportViewStep3 extends ImportView
{
    ...
    public function display()
    {
        ...
        $required_fields = $this->bean->get_import_required_fields();
        ...
        for ($field_count = 0; $field_count < $ret_field_count; $field_count++) {
            ...
            foreach ($fields as $fieldname => $properties) {
                ...
                if (!empty($moduleStrings['LBL_EXPORT_' . strtoupper($fieldname)])) {
                    ...
                    $displayname = $this->getDisplayName($properties);
                }
                // see if this is required
                $req_mark = '';
                $req_class = '';
                if (array_key_exists($fieldname, $required_fields)) {
        ...
        $required = [];
        foreach (array_keys($required_fields) as $name) {
            $properties = $this->bean->getFieldDefinition($name);
            $required[$name] = $this->getDisplayName($properties);
        }
...
function toggleDefaultColumnVisibility(hide)
{
    ...
    {
        var newStyle = 'none';
        var bgColor = 'var(--foreground-base)';
...
EOJAVASCRIPT;
     ...
     * @return string
     */
    private function getDisplayName(mixed $properties): string
    {
        $key = !empty($properties['vname']) ? $properties['vname'] : $properties['name'];
        if (!isset(self::$translationsCache[$this->bean->module_dir][$key])) {
            self::$translationsCache[$this->bean->module_dir][$key] = str_replace(':', '', translate($key, $this->bean->module_dir));
        }
        return self::$translationsCache[$this->bean->module_dir][$key];
    }
}
