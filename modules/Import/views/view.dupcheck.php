...
class ImportViewDupcheck extends ImportView
{
    ...
    public function display()
    {
        ...
        $fieldMap = $this->getImportMap();

        $selectedOperator = '';
        if (isset($fieldMap['dupe_operator'])) {
            $selectedOperator = $fieldMap['dupe_operator'];
        }
        $this->ss->assign('DUPLICATE_OPERATORS_OPTIONS', get_select_options_with_id($duplicateOperators, $selectedOperator));
    ...
    protected function getImportMap()
    {
        if (!empty($_REQUEST['source_id'])) {
            $import_map_seed = BeanFactory::getBean('Import_1', $_REQUEST['source_id'], ['encode' => false]);
