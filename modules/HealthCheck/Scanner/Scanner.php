...
use Sugarcrm\Sugarcrm\modules\HealthCheck\Scanner\Checks\Monolog as MonologHealthCheck;
...
class HealthCheckScanner
{
    ...
    protected $deprecatedPHPCodePatterns = [
        ...
        '/\b(Zend_Service_|Zend_Gdata\b|Zend_Gdata_|Zend_Version)/i' => 'removedZendComponent',
    ];
    ...
    protected array $filesWithOutdatedMonolog = [];
    ...
    protected function checkSmartyTemplatesModifierDeprecationErrors()
    {
        ...
        $getValidDirectoryIterator = function (string $path) {
            if (is_dir($path)) {
                $directoryIterator = new RecursiveDirectoryIterator($path, FilesystemIterator::SKIP_DOTS);
                $filterIterator = new RecursiveCallbackFilterIterator($directoryIterator, function ($current, $key, $iterator) {
                    // Only include directories or .tpl files, exclude other files
                    if ($current->isDir()) {
                        return true;
                    } elseif ($current->isFile() && preg_match('/\.tpl$/i', $current->getFilename())) {
                        return true;
                    }
                    return false;
                });
        ...
        $customIterator = $getValidDirectoryIterator('./custom');
        if ($customIterator) {
            $iterator->append($customIterator);
        }

        $modulesIterator = $getValidDirectoryIterator('./modules');
        if ($modulesIterator) {
            $iterator->append($modulesIterator);
        }

        try {
            foreach ($iterator as $match) {
                $curFile = $match->getPathname();
                if (defined('SUGAR_SHADOW_TEMPLATEPATH')) {
                    $realName = realpath(str_replace(SHADOW_INSTANCE_DIR . '/', '', $curFile));
                    if (0 === strpos($realName, (string)SUGAR_SHADOW_TEMPLATEPATH)) {
                        continue;
            ...
            $this->log('checkSmartyTemplatesModifierDeprecationErrors: excpetion: ' . $e->getMessage(), 'BUCKET');
        }
    }
...
ENDP;
    ...
    protected function scanCustomDir()
    {
        ...
        foreach ($files as $name => $file) {
            ...
            $this->scanFileForOutdatedMonologUsage($file, $fileContents);
        }
    }
    ...
    public function scanFileForOutdatedMonologUsage($file, $fileContents): void
    {
        require_once __DIR__ . '/Checks/Monolog.php';

        $check = new MonologHealthCheck();
        $issues = $check->check($fileContents);

        foreach ($issues as $issue) {
            $message = $issue->getMessage();
            $line = $issue->getLine();
            $this->filesWithOutdatedMonolog['incompatibleMonologCustomization'][] = $file . ':' . $line . ': ' . $message;
        }
    }
    ...
    protected function updateCustomDirScanStatus()
    {
        ...
        foreach ($this->filesWithOutdatedMonolog as $reportId => $files) {
            $this->updateStatus($reportId, array_unique($files));
        }
    }
    ...
    protected function checkMaxPathLengthLimit(): void
    {
        $this->log("Checking filesystem path length limit.");
        if (!$this->isLongPathSupported()) {
            $this->updateStatus('maximumPathLengthLimitation');
        }
    }

    protected function isLongPathSupported()
    {
        $testDir = sugar_cached('longpath_test_' . time());
        try {
            if (!sugar_mkdir($testDir, null, true)) {
                return false;
            }
            $longFolderPath = $testDir . DIRECTORY_SEPARATOR . str_repeat('a', 200);
            if (!sugar_mkdir($longFolderPath)) {
                return false;
            }
            $longFilePath = $longFolderPath . DIRECTORY_SEPARATOR . str_repeat('b', 200) . '.txt';
            $fileHandle = fopen($longFilePath, 'w');
            if (!$fileHandle) {
                return false;
            }
            // Successfully created a file with path > 260 chars
            fwrite($fileHandle, "Test content");
            fclose($fileHandle);
            return true;
        } catch (\Throwable $t) {
            return false;
        ...
        switch ($errno) {
            ...
            case 16384:
                if (basename($errfile) === 'smarty_internal_compile_private_modifier.php'
                    || basename($errfile) === 'smarty_internal_templatecompilerbase.php'
                ) {
