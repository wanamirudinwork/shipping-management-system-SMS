...
({
        ...
        app.api.call('create', app.api.buildURL('stage2/token'), null, {
            success: _.bind(function(data) {
                if (!_.isUndefined(data.errorMessage)) {
                    this.setStage2errorCode('Failed to get Hint access token: ' ,data);
                    return;
                }
                this.stage2accessTokenExpireDate = Date.now() + this.tokenExpirationTimeOut;
                this.stage2accessToken = data.accessToken;
                this.initNewsRequest();
            }, this),
            error: _.bind(this.setStage2errorCode, this, ' ')
        });
    },
    ...
    getNotificationServiceToken: function() {
        app.api.call('create', app.api.buildURL('stage2/notificationsServiceToken'), null, {
            success: _.bind(function(data) {
                if (!_.isUndefined(data.errorMessage)) {
                    this.setStage2errorCode('Failed to get Notifications Service Token: ' ,data);
                    return;
                }
                var now = new Date();
                now.setTime(now.getTime() + data.ttlMs);
                this.notificationsAccessTokenExpireDate = now;
                this.notificationsAccessToken = data.accessToken;
                this.initNewsRequest();
            }, this),
            error: _.bind(function(err) {
                this.setStage2errorCode('Failed to get Notifications Service Token: ', err);
            }, this)
        });
    },
