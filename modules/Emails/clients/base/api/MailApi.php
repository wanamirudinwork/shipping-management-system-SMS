...
class MailApi extends ModuleApi
{
    ...
    public function registerApiRest()
    {
        $api = [
            ...
            'findByEmail' => [
                'reqType' => 'GET',
                'path' => ['Mail', 'findByEmail'],
                'pathVars' => [''],
                'method' => 'findByEmail',
                'shortHelp' => 'Find the email by email address',
                'longHelp' => 'modules/Emails/clients/base/api/help/mail_findByEmail_get.html',
            ],
        ];
    ...
    /**
     * Returns the outbound emails current user can use
     *
     * @throws SugarApiExceptionMissingParameter
     * @throws SugarApiExceptionNotAuthorized
     */
    public function getOutboundEmails(ServiceBase $api, array $args): array
    {
        $seed = BeanFactory::newBean('OutboundEmail');
        $q = new SugarQuery();
        $q->from($seed);
        $beans = $seed->fetchFromQuery($q, ['email_address']);
        $options = [];
        foreach ($beans as $bean) {
            if ($bean->isConfigured()) {
                $options[] = $bean->email_address;
            }
        }
        if (empty($options)) {
            $errorLbl = 'LBL_ERR_NO_OUTBOUND_ACCOUNTS_AVAILABLE';
            throw new SugarApiExceptionNotAuthorized($errorLbl, null, 'Emails');
        }
     ...
     * @throws SugarApiExceptionNotAuthorized
     */
    public function findByEmail(ServiceBase $api, array $args): ?array
    {
        if (empty($args['email'])) {
            return null;
        }
        $seed = BeanFactory::newBean('EmailAddresses');
        $query = new SugarQuery();
        $query->from($seed);
        $query->where()->equals('email_address', $args['email']);
        $stmt = $query->execute();

        return !empty($stmt[0]) ? $stmt[0] : null;
    }
