...
({
    ...
    _onProductBundleChange: function(productBundles) {
        ...
        if (productBundles && productBundles.length > 0) {
            hasDefaultGroup = _.some(productBundles.models, bundle => bundle.get('default_group'));
        }
        ...
        if (!hasDefaultGroup) {
            ...
            defaultGroupModel = _.find(productBundles.models, bundle => bundle.get('default_group'));
            this.defaultGroupId = defaultGroupModel.cid;
        }

        let editGroupRecId = this.model.get('editGroupRecId');
        productBundles.each(function(bundle) {
            // The block prevents numerous unnecessary API requests to the server ("/ExpressionEngine/.../related")
            // that "freeze" the UI, especially when there are many groups of QLIs in the Quote
            if (editGroupRecId && bundle.id !== editGroupRecId && !bundle.get('default_group')) {
                return;
            }
            ...
            let group = this._getComponentByGroupId(bundle.id);
        ...
        this.model.unset('editGroupRecId');
    },
