...
({
    ...
    _isValidDatePickerFormat: function(date) {
        ...
        const userFormat = app.user.getPreference('datepref');
        const acceptedFormat = dateFormatMapping[userFormat] || userFormat;
    ...
    _getTargetField: function() {
        ...
        if (targetField.type === 'enum' && targetField.function && reportId) {
    ...
    _updateFilterInput: function() {
        ...
        if (qualifierName === 'anything') {
            ...
            this._inputType = false;
        } else if (_.contains(['date', 'datetime', 'service-enddate'], fieldType)) {
    ...
    _updateUsernameFilterData: function(qualifierName) {
        ...
        } else if (_.contains(['is', 'is_not', 'reports_to'], qualifierName) &&
            _.isEmptyValue(this._filterData.input_name0)) {
            this._inputType = 'select-single';
            this._inputValue = _.first(Object.keys(this._users));
