...
({
    ...
    /**
     * Add margins to the elements
     */
    _addMargins: function() {
        const tailwindPxPerUnit = 4;
        const marginType = app.lang.direction === 'rtl' ? 'margin-right' : 'margin-left';

        _.each(this._flattenedFilters, function addMargins(widget) {
            const elWidget = this.$('#' + widget.elementId);
            elWidget.css(marginType, `${widget.column * tailwindPxPerUnit}px`);
        }, this);
    },
    ...
    _generatePillHtml: function(widget, posX) {
        ...
        pillEl.style[app.lang.direction === 'rtl' ? 'right' : 'left'] = `${posX}px`;
    ...
    _generateLineHtml: function(parent, child, lineWidth, color, calculatedPosY) {
        const isRTL = app.lang.direction === 'rtl';
        const marginClass = isRTL ? 'margin-right' : 'margin-left';

        const posX = parseFloat(parent.css(marginClass)) + parseFloat(parent.css('width')) - lineWidth / 2;
        const posY = calculatedPosY ? calculatedPosY : child.position().top;

        const width = parseFloat(child.css(marginClass)) - posX - lineWidth / 2;
        const height = child.position().top - parent.position().top - parent.outerHeight() / 2 - lineWidth / 2;

        let lineEl = document.createElement('div');

        lineEl.style.position = 'absolute';
        lineEl.style[isRTL ? 'borderRight' : 'borderLeft'] = `solid ${lineWidth}px ${color}`;
        lineEl.style.borderBottom = `solid ${lineWidth}px ${color}`;
        lineEl.style[isRTL ? 'right' : 'left'] = `${posX}px`;
