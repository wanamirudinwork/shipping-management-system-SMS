...
class ReportsApi extends ModuleApi
{
    ...
    public function createRecordList(ServiceBase $api, array $args)
    {
        ...
        SugarMetric_Manager::getInstance()->addTransactionParam('ReportIdentifier', 'id => ' . $args['record']);
    ...
    public function getReportRecords(ServiceBase $api, array $args)
    {
        ...
        unset($args['record']);
        $args['forcedOffset'] = $offset;
        $args['forcedLimit'] = $limit;
        $args['max_num'] = -1;
        $args['offset'] = 0;
        $filterApi = new FilterApi();
        $result = $filterApi->filterList($api, $args);
        $recordsPerPage = intVal($sugar_config['list_max_entries_per_page']);

        if ($limit === $recordsPerPage && safeCount($result['records']) === $limit) {
    ...
    public function retrieveSavedReportRecordsById(ServiceBase $api, array $args): array
    {
        ...
        SugarMetric_Manager::getInstance()->addTransactionParam('ReportIdentifier', 'id => ' . $args['record'] . ' ; type => ' . $args['reportType']);
