...
({
    ...
    render: function() {
        ...
        const recordlistComponent = this.getSideComp('recordlist', true);
        if (recordlistComponent) {
            // allow selected records to persist during sorting
            recordlistComponent.independentMassCollection = true;
        }
    },
     ...
     */
    togglePreview: function(previewEnabled, model) {
        const previewError = app.lang.get('LBL_PREVIEW_ERROR');
        const previewComponent = this.getSideComp('report-side-drawer-preview');
        const chartComponent = this.getSideComp('report-side-drawer-chart');

        if (!previewComponent || !chartComponent) {
     ...
     * @param {boolean} drillComponent
     * @returns
     */
    getSideComp: function(componentName, drillComponent = false) {
        let component = null;
        const rowHolder = this._getRowHolderComponent();
        ...
        const drillIndex = 0;
        const reportIndex = 1;
        const targetIndex = drillComponent ? drillIndex : reportIndex;
        const listSide = rowHolder._components[targetIndex];
        ...
        if (drillComponent) {
            const drillList = listSide.getComponent('drillthrough-list');

            if (drillList) {
                component = drillList.getComponent(componentName);
            }
        } else {
            component = listSide.getComponent(componentName);
        }
    ...
    /**
     * Get row holder component
     * @returns {View.View} rowHolderComponent
     */
    _getRowHolderComponent: function() {
        let rowHolderComponent = null;

        const columnHeader = this.getComponent('column-holder');
        if (!columnHeader) {
            return rowHolderComponent;
        }

        const rowHolder = columnHeader.getComponent('row-holder');
        if (!rowHolder) {
            return rowHolderComponent;
        }
    ...
    _registerEvents: function() {
        ...
        this.listenTo(app.events, 'preview:render', this._previewRecord, this);
        this.listenTo(app.events, 'preview:close', this._closePreview, this);
    },
     ...
     */
    _previewRecord: function(model) {
        model.fetch();

        const previewComponent = this.getSideComp('report-side-drawer-preview');
        const previewComponentVisible = previewComponent.$el.css('display') !== 'none';

        const sameModel = previewComponent.model && previewComponent.model.id === model.id

        if (previewComponentVisible && sameModel) {
            this.togglePreview(false, model);
        } else {
            this.togglePreview(true, model);
        }
    },
