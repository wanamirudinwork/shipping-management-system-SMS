...
class AdministrationViewApiplatforms extends SugarView
{
    ...
    public function display()
    {
        ...
        if (!hasMarketLicense()) {
            $allPlatforms = $this->removeMarketPlatforms($allPlatforms);
        }
        ...
        $api_platforms = array_map(function ($platform) use ($platforms, $platformOptions) {
            ...
            return [
                'name' => $platform,
                'custom' => safeInArray($platform, $platforms),
    ...
    /**
     * Remove platforms for Sugar Market integration.
     * @param array $allPlatforms
     * @return array
     */
    protected function removeMarketPlatforms(array $allPlatforms): array
    {
        foreach ($allPlatforms as $k => $platform) {
            if ((strpos($platform, 'salesfusion_') !== false ||
                strpos($platform, 'SugarMarket_') !== false)) {
                unset($allPlatforms[$k]);
            }
        }
        // re-index the array
