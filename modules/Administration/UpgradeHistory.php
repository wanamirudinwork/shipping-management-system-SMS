...
class UpgradeHistory extends SugarBean
{
     ...
     */
    public function getProcessStatus(?array $onlyFields = null): array
    {
        $data = (array)@json_decode($this->process_status, true);
        if ($onlyFields !== null) {
            $data = array_intersect_key($data, array_flip($onlyFields));
        }
