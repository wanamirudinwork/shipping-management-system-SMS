...
var handleOauth2TabSwitch = function(smtptype, defaults) {
	...
	} else if (!authInfo[smtptype]['isConnected']) {
		document.getElementById("auth_warning").style.display = 'block';
		document.getElementById("auth_warning").innerHTML = '{/literal}{$APP.LBL_ALERT_TITLE_NOTICE}{literal} <span style="font-weight: normal;">{/literal}{$APP.LBL_EMAIL_RE_AUTHORIZE_ACCOUNT}{literal}</span>';
	...
	document.getElementById('auth_status').value = authInfo[smtptype]['eapm_id'] && authInfo[smtptype]['isConnected'] ?
