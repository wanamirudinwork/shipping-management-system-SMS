...
({
    ...
    onCancelRowEdit: function(rowModel) {
        ...
        if (rowModel.isNew()) {
            rowId = rowModel.cid;
            this.collection.remove(rowModel, {silent: true});

            const $relatedRow = this.$('tr[name="' + rowModel.module + '_' + rowModel.cid + '"]');
            $relatedRow.remove();
    ...
    onAddNewItemToGroup: function(linkName, prepopulateData) {
        ...
        // Adding a new model to the collection without trigger the render
        this.collection.add(relatedModel, {silent: true});

        // Get new row as DOM element by rendering template (with store the list of fields before and after)
        const fieldsBefore = _.keys(this.fields);
        let rowTemplate = Handlebars.helpers.partial('row', this, {}, {
            data: {},
            hash: {
                view: this,
                fields: this._fields,
                groupId: this.model.cid,
                listColSpan: this.listColSpan,
                module: relatedModel.module,
                model: relatedModel,
            }
        });
        const fieldsAfter = _.keys(this.fields);

        // Adding a new row to the group within DOM
        let groupHeader = this.$el.find('.quote-data-group-header');
        groupHeader.length ? groupHeader.after(rowTemplate.string) : this.$el.prepend(rowTemplate.string);

        // Render new fields for a row
        const recentFields = _.difference(fieldsAfter, fieldsBefore);
        _.each(recentFields, field => {
            const fieldController = this.fields[field];
            const fieldEl = this.$(`span[sfuuid=${field}]`);
            if (fieldController && fieldEl.length) {
                this._renderField(fieldController, fieldEl);
            }
        });

        this._setRowFields();
        this.toggleRow(relatedModel.module, relatedModel.cid, true);
