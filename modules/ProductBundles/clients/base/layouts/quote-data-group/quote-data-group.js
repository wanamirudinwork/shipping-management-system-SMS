...
({
     ...
     */
    _render: function() {
        // The block prevents multiple unnecessary renderings of all groups (which "freezes" the UI,
        // especially when there are many records in the group) when only one group is being edited.
        const editGroupRecId = this.layout ? this.layout.model.get('editGroupRecId') : null;
        if (editGroupRecId && this.model.get('id') != editGroupRecId && this.groupId != this.layout.defaultGroupId) {
            return;
        }
