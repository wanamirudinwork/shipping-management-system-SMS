...
class ParserDropDown extends ModuleBuilderParser
{
    ...
    public function saveDropDown($params, $finalize = true, $lang = '')
    {
        ...
        if ($type != 'studio') {
            ...
            DropdownsManager::buildDropdownStyle(
                $dropdown_name,
                $dropdown,
                false
            );
    ...
    public function updateSalesStageClassifications($salesStages)
    {
        ...
        $newClosedWonSalesStages  = array_filter($dropdown, fn($value) => $value == 'Closed Won');
        $newClosedLostSalesStages = array_filter($dropdown, fn($value) => $value == 'Closed Lost');

        return [
            'new_closed_won_sales_stages'  => array_keys($newClosedWonSalesStages),
            'new_closed_lost_sales_stages' => array_keys($newClosedLostSalesStages),
        ];
    }
