...
{foreach from=$layout item='panel' key='panelid'}
    ...
    <div class='le_panel' id='{$idCount}'>
        ...
        {/if}
        {if $no_tabs != true}
        <select id="le_paneltype_select_{$idCount}" data-target="tabDefs_{$panelid|escape:'html'}_newTab" onchange="document.forms.prepareForSave[this.getAttribute('data-target')].value=this.value; showHideBox(this.value, {$idCount}, '{$panelid|escape:'javascript'|escape:'html':'UTF-8'}', '{$firstpanelid|escape:'javascript'|escape:'html':'UTF-8'}', {$firstpanelidcount|escape:'javascript'|escape:'html':'UTF-8'});"
...
{foreach from=$layout item='panel' key='panelid'}
{capture name=panel_upper assign=panel_upper}{$panelid|upper}{/capture}
<input type="hidden" name="tabDefs_{$panelid|escape:'html'}_newTab" value="{if $tabDefs[$panel_upper].newTab == true}1{else}0{/if}" />
<input type="hidden" name="tabDefs_{$panelid|escape:'html'}_panelDefault" value="{$tabDefs[$panel_upper].panelDefault}" />
{/foreach}
