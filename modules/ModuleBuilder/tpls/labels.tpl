            ...
            {foreach from=$MOD item='label' key='key'}
                <tr>
                    ...
                    <td class="labels-cell align-left">
                        <input type='hidden' name="label_{$key|escape:'html'}" id="label_{$key|escape:'html'}" value='no_change'><input id="input_{$key|escape:'html'}" data-target="label_{$key|escape:'html'}" onchange="document.getElementById(this.getAttribute('data-target')).value = this.value; ModuleBuilder.state.markAsDirty();" value="{$label|escape:'html'}" size="60">
                    </td>
