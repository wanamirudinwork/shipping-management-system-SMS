...
HTML;
     ...
     * @return string
     */
    public function addToSugarQuery(SugarQuery $sugar_query, $options = [])
    {
        $alias = '';
        ...
        $sugar_query->joinTable(self::getTableName(), ['alias' => $sfAlias, 'joinType' => $joinType, 'linkingTable' => true])
    ...
    protected function subscribeAfterFavorite()
    {
        if (!empty($this->id)) {
            ...
            if ($this->deleted == 0) {
                $beanToFollow = BeanFactory::getBean($this->module, $this->record_id, ['strict_retrieve' => true]);
