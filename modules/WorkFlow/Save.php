...
if (!is_admin($current_user) && empty($access[$base_module])) {
    $GLOBALS['log']->security("{$current_user->user_name} attempted to edit WorkFlow");
...
if (!empty($_POST['is_duplicate']) && $_POST['is_duplicate'] == 'true') {
    ...
    $connection = $focus->db->getConnection();
    foreach ($actions_list as $action) {
        ...
        $query = "SELECT id FROM workflow WHERE parent_id = ?";
        $rows = $connection->fetchAllAssociative($query, [$action->id]);
        ...
        foreach ($rows as $row) {
            ...
            $query = "SELECT id FROM workflow_alertshells WHERE parent_id = ?";
            $alertshells = $connection->fetchAllAssociative($query, [$row['id']]);

            foreach ($alertshells as $alertshell) {
                ...
                $query = "SELECT id FROM workflow_alerts WHERE parent_id = ? ";
                $alerts = $connection->fetchAllAssociative($query, [$alertshell['id']]);
