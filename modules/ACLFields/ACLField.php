...
{
    private const MAX_RECURSION_LEVELS = 50;
    ...
    public static function getAvailableFields($module, $object = false)
    {
        ...
        if (!isset($callDepth[$module])) {
            $callDepth[$module] = 0;
        }

        $callDepth[$module]++;

        if ($callDepth[$module] > self::MAX_RECURSION_LEVELS) {
            LoggerManager::getLogger()->fatal('Potential recursive call to ACLField::getAvailableFields detected for module ' . $module);
            $callDepth[$module]--;
            return [];
        }
