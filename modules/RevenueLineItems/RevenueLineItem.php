...
class RevenueLineItem extends SugarBean
{
    ...
    // This is used to update/cascade caused from Opportunity to all related RLIs
    ...
    protected function mapProbabilityFromSalesStage()
    {
        ...
        if (isset($prob_arr[$this->sales_stage])) {
            $this->probability = intval($prob_arr[$this->sales_stage]);
        }
    }
    ...
    protected function setCommitStage()
    {
        ...
        if ($settings['is_setup']) {
            ...
            if (safeInArray($this->sales_stage, $lost)) {
                $this->commit_stage = 'exclude';
                return;
            }

            // If this save is on update/cascade way then return
            if ($this->isCascadeUpdate) {
                return;
            }

            $forecastRangeSetting = $settings['forecast_ranges'] ?? null;
            //set the probability according the sales stage
            $this->mapProbabilityFromSalesStage();
    
            if (!empty($forecastRangeSetting)) {
                $ranges = $settings[$forecastRangeSetting . '_ranges'];
    
                if (!empty($ranges)) {
                    foreach ($ranges as $key => $value) {
                        if ($this->probability >= $value['min'] &&
                            $this->probability <= $value['max']) {
