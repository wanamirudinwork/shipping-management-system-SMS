...
class DRI_Workflow extends Basic
{
    ...
    **/
    ...
    public function cancel()
    {
        ...
        $this->cancelling_smart_guide = true;
    ...
    /**
     * Update stage in stages cache
     * @param \SugarBean $stage
     */
    public function updateStageInCache(\SugarBean $stage)
    {
        foreach (self::$stages[$this->id] as $index => $currentStage) {
            if ($currentStage->id === $stage->id) {
                self::$stages[$this->id][$index] = $stage;
            }
        }
    }
    ...
    /**
     * Calculate journey score, progress and momomentum
     *
     * @param bool $updateStage
     * @param bool $updateJourney
     * @return void
     */
    public function calculate($updateStage, $updateJourney = false)
    {
        if ($updateJourney) {
            $this->calculateState();
        } else {
            $this->calculateState($updateStage);
        }
        $this->calculateProgress($updateStage);
        $this->calculateMomentum($updateStage);
    }
}
