...
({
    ...
    /**
     * Holds the default value for number of timeperiods shown from the current that are displayed ( forward&backward )
     */
    timeperiodShownDefault: '02',
    ...
    addFiscalYearField: function() {
        if (!this.fiscalYearField) {
            ...
            if (this.tpStartDate instanceof app.date) {
                let currentSelectedYear = this.tpStartDate.year();
                if (this.fiscalYearField.items &&
                    this.fiscalYearField.items.current_year) {
                    //using base 10 to explicitly prevent unexpected behavior
                    const currentYear = parseInt(this.fiscalYearField.items.current_year, 10);
                    if (!isNaN(currentYear) && currentYear !== currentSelectedYear) {
                        this.fiscalYearField.items = {
                            current_year: currentSelectedYear,
                            next_year: currentSelectedYear + 1
                        };

                        this.fiscalYearField.def.startYear = currentSelectedYear;
                        this.model.set('timeperiod_fiscal_year', 'current_year');
                        this.fiscalYearField.render();
                    }
                }
            }
    ...
    _setUpTimeperiodShowField: function(field) {
        ...
        let fieldValue = this.model.get(field.name);

        if (field.hasOwnProperty('def') && _.isObject(field.def.options)) {
            let fieldOptionsKey = _.findKey(field.def.options, enumVal => enumVal === fieldValue);
            fieldValue =  fieldOptionsKey || this.timeperiodShownDefault;
        } else {
            fieldValue = this.timeperiodShownDefault;
        }

        this.model.set(field.name, fieldValue, {silent: true});

        field.def.value = this.model.get(field.name) || this.timeperiodShownDefault;
        return field;
    },
