...
({
    ...
    initForecastsModule: function(data) {
        ...
        let lastTimePeriod = app.user.lastState.get('Forecasts:time-period');
        let defaultTimePeriod = data.defaultSelections.timeperiod_id.id;
        let timeperiodId = lastTimePeriod || defaultTimePeriod;
        // if the time period from local storage is not in the options, set it to the default
        if (!_.has(data.timePeriodOptions,timeperiodId)) {
            timeperiodId = defaultTimePeriod;
            app.user.lastState.set('Forecasts:time-period', timeperiodId);
        }
