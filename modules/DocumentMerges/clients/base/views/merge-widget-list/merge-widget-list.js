...
({
    ...
    _mergesRetrieved: function(data) {
        ...
        _.map(this.merges, function(merge) {
            ...
            let icon = this.icons[merge.file_type] || this.icons.DOC;
            merge.icon_path = `themes/default/images/icon_${icon}.svg`;
        }.bind(this));
    ...
    downloadDocument: function(evt) {
        let mergeObject = _.find(this.merges, {id: this._getMergeId(evt)});
        if (!mergeObject || mergeObject.status !== 'success') {
            return;
        }
    ...
    pollMerge: function(documentMergeId) {
        var timesRun = 0;
        var maxRun = 1200; //equivalent of running the timer for 10 minutes if we run it once at 2 seconds

        this._documentGeneratedEventTriggered = false;
        var timer = setInterval(function() {
            timesRun++;
            //if it takes longer than 10 minutes do not wait
            if (timesRun === maxRun) {
                clearInterval(timer);
            }
