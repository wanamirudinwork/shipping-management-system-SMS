...
class Popup_Picker
{
    ...
    public function process_page()
    {
        ...
        if ($this->_create) {
            ...
            <input type="hidden" name="return_module" value="{$attr($currentModule)}">
            <input type="hidden" name="return_action" value="Popup">
            <input type="submit" name="button" class="button" title="{$attr($lbl_save_button_title)}" accesskey="{$attr($lbl_save_button_key)}" value="  {$attr($lbl_save_button_label)}  " />
            <input type="button" name="button" class="button" title="{$attr($app_strings['LBL_CANCEL_BUTTON_TITLE'])}" accesskey="{$attr($app_strings['LBL_CANCEL_BUTTON_KEY'])}" value="{$attr($app_strings['LBL_CANCEL_BUTTON_LABEL'])}" onclick="toggleDisplay('addform');" />
EOQ;
            ...
            if (!empty($this->_popupMeta['customInput']) && is_array($this->_popupMeta['customInput'])) {
                foreach ($this->_popupMeta['customInput'] as $key => $value) {
                    $formSave .= '<input type="hidden" name="' . $attr($key) . '" value="' . $attr($value) . '">';
                }
            }

            if (!empty($_REQUEST['custom_method'])) {
                $formSave .= '<input type="hidden" name="custom_method" value="' . $attr($_REQUEST['custom_method']) . '">';
            }

            if (!empty($_REQUEST['user_id'])) {
                $formSave .= '<input type="hidden" name="user_id" value="' . $attr($_REQUEST['user_id']) . '">';
            }
            $createButtonTranslation = translate($this->_popupMeta['create']['createButton']);
            $createButton = <<<EOQ
            <input type="button" id="showAdd" name="showAdd" class="button" value="{$attr($createButtonTranslation)}" onclick="toggleDisplay('addform');" />
EOQ;
        ...
        if (isset($_REQUEST['mass'])) {
            foreach (array_unique($_REQUEST['mass']) as $record) {
                $button .= <<<HTML
                <input style="display: none" checked type="checkbox" name="mass[]" value="{$attr($record)}" />
HTML;
            }
        }
        ...
        $multi_select = false;
        $jsVar = function ($value): TemplateVariable {
            return (new TemplateVariable((string) $value))->escapeJs();
        };
        if (!empty($_REQUEST['mode']) && strtoupper($_REQUEST['mode']) == 'MULTISELECT') {
            $multi_select = true;
            $button .= "<input type='hidden' name='mode' value='MultiSelect'>";
            $button .= <<<HTML
            <input type="button" name="button" class="button" onclick="send_back_teams('{$jsVar($currentModule)}',document.MassUpdate,'mass[]','{$jsVar($app_strings['ERR_NOTHING_SELECTED'])}', request_data);" 
                title="{$attr($app_strings['LBL_SELECT_BUTTON_TITLE'])}" accesskey="{$attr($app_strings['LBL_SELECT_BUTTON_KEY'])}" 
                value="{$attr($app_strings['LBL_SELECT_BUTTON_LABEL'])}"  id="search_form_select" />
HTML;
        }
        ...
        if (!$hide_clear_button) {
            $button .= <<<HTML
            <input type="button" name="button" class="button" onclick="send_back('','');" title="{$attr($app_strings['LBL_CLEAR_BUTTON_TITLE'])}" 
                value="{$attr($app_strings['LBL_CLEAR_BUTTON_LABEL'])}" id="search_form_clear" />
HTML;
        }

        $button .= <<<HTML
        <input type="submit" name="button" class="button" onclick="window.close();" title="{$attr($app_strings['LBL_CANCEL_BUTTON_TITLE'])}"
            accesskey="{$attr($app_strings['LBL_CANCEL_BUTTON_KEY'])}" 
            value="  {$attr($app_strings['LBL_CANCEL_BUTTON_LABEL'])}  " id="search_form_cancel" />
HTML;
