    ...
    public function http_GET()
    {
        ...
        if ($this->vcal_type == 'ics') {
            if (!empty($this->ical_publish_key) && !empty($this->calendars_uid)) {
                ...
                $qb->where($qb->expr()->eq('id', DBManagerFactory::getConnection()->quote($this->calendars_uid)));
                ...
                if (isset($_REQUEST['export'])) {
                    ...
                    $qb->delete('calendar_ical_configs')
                        ->where($qb->expr()->eq('id', DBManagerFactory::getConnection()->quote($this->calendars_uid)));
