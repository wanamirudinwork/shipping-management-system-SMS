 ...
 ({
    ...
    _initializeScheduler: function() {
        ...
        let kendoOptions = _.extend({}, {
            ...
            pdfExport: _.debounce(this._pdfExport, 0),
     ...
     * @param {Object} e
     */
    _pdfExport: function(e) {
        const calendarFont = app.user.getPreference('sugarpdf_pdf_calendar_font_name_data');
        const notoSansFont = 'NotoSansSC-Regular';
        const currentFont = this.wrapper.css('font-family');
        let cssFontFamily = '';

        if (calendarFont === notoSansFont) {
            cssFontFamily += notoSansFont;
        }

        //refreshLayout adds heights to elements that needs to be removed
        _.defer(_.bind(function() {
            this.wrapper.find('.k-scheduler-content').css('height', '');
            this.wrapper.find('.k-scheduler-times').css('height', '');
            this.wrapper.find('.k-scheduler-table').css('height', '');
        }, this));

        e.sender.wrapper.css('font-family', cssFontFamily);

        e.promise.done(_.bind(function() {
            this.element.css('font-family', currentFont);
        }, this));
    },
