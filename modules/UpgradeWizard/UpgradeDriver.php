...
abstract class UpgradeDriver
{
    ...
    protected function preflightPHP()
    {
        if (version_compare(PHP_VERSION, '8.2.0', '<')) {
            return $this->error('PHP versions below 8.2.0 are not supported!', true);
        }
        return true;
    }
    ...
    {
        return $this->mod_strings[$msg] ?? $msg;
    }
    ...
    {
        set_error_handler([$this, 'scriptErrorHandler'], E_ALL & ~E_DEPRECATED);
        ob_start();
        try {
            $script->run();
        } catch (Exception $e) {
            $this->error('runScript Exception: ' . $e->getMessage());
            $this->error('Exception stacktrace: ' . $e->getTraceAsString());
        }
        $out = ob_get_clean();
        if ($out) {
            $this->log("OUTPUT: $out");
