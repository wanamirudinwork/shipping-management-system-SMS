...
return new Container([
    ...
    Listener::class => function (ContainerInterface $container): Listener {
        ...
        // add PreFetch only when it turned on in config
        if ((new NormalizedTeamSecurity(new SugarBean()))->setOptions($container->get(SugarConfig::class)->all())
                ->getOption('teamset_prefetch')) {
            $compositeItems[] = new PreFetch($conn);
        }

        return new Composite(...$compositeItems);
    },
    ...
    CacheInterface::class => function (ContainerInterface $container): CacheInterface {
        ...
        if ($config->get('external_cache_disabled')) {
            return $container->get(InMemoryCache::class);
        }
        ...
        return new Replicate(
            $backend,
            $container->get(InMemoryCache::class)
        );
    },
    ...
    InMemoryCache::class => function (): InMemoryCache {
