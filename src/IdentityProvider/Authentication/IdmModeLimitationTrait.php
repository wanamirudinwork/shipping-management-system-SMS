...
trait IdmModeLimitationTrait
{
    ...
    {
        // Static cache to store results of isLimitedForModuleInIdmMode calls
        static $moduleCache = [];

        // Check cache first and populate if needed
        if (!isset($moduleCache[$module])) {
            $moduleCache[$module] = $this->isLimitedForModuleInIdmMode($module);
        }
