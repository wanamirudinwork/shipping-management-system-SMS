...
class IdmProvider extends BasicGenericProvider
{
    ...
    /**
     * Introspect refresh_token and return resource owner details
     * @param AccessToken $token
     * @return string
     * @throws \RuntimeException
     */
    public function introspectRefreshToken($refreshToken, $url): array
    {
        $cacheKey = 'oidc_introspect_refresh_token_' . hash('sha256', $refreshToken);
        $result = $this->getCache($cacheKey);
        if (!is_null($result)) {
            return $result;
        }
        ...
        $options = [
            ...
            'body' => $this->buildQueryString(['token' => $refreshToken]),
        ];
