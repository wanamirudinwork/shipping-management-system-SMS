...
class ResolverFactory
{
    public static function getInstance(): Resolver
    {
        if (\SugarConfig::getInstance()->get('security.use_doh', false)) {
            ...
            $preferIpv6 = \SugarConfig::getInstance()->get('security.preferIpv6', false);
            $resolver = new CachedResolver($cache, new ChainResolver(new GoogleResolver($options), new NativeResolver($preferIpv6)));
        } else {
            $dns = \SugarConfig::getInstance()->get('security.dns', []);
            $preferIpv6 = \SugarConfig::getInstance()->get('security.preferIpv6', false);
            $resolver = new ChainResolver(new ConfigResolver($dns), new NativeResolver($preferIpv6));
        }
