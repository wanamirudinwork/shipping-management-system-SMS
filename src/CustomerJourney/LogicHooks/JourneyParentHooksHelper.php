...
class JourneyParentHooksHelper
{
    ...
    private function shouldStartJourney(\SugarBean $bean, $arguments)
    {
        ...
        $template = \DRI_Workflow_Template::getById($templateId);

        if (empty($template) || !$template->active) {
            return false;
        }

        return (empty($bean->is_customer_journey_activity) || $bean->isPASaveRequest)
            && (empty($bean->dri_customer_journey_started_template) || $templateId !== $bean->dri_customer_journey_started_template)
