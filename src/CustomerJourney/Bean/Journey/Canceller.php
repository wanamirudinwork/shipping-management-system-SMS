...
class Canceller
{
    ...
    public function cancel(\DRI_Workflow $journey)
    {
        ...
        try {
            ...
            if ($journey->state !== \DRI_Workflow::STATE_CANCELLED &&
                (empty($response) || ($response['activity_change_not_allowed'] === false &&
    ...
    private function cancelActionSetNotApplicable(\DRI_Workflow $journey)
    {
        ...
        $response = [
            'activity_change_not_allowed' => false,
            'is_child_read_only' => false,
        ];
        ...
        foreach ($journey->getStages() as $stage) {
            ...
            if (!isset($journey->process_activities)) {
                foreach ($stage->getActivities() as $activity) {
                    ...
                    if ($activityHandler->isParent($activity)) {
                        ...
                        //If the parent is not completed then we need to cancel the parent as well
                        if (!$activityHandler->isBlocked($activity) &&
                            !$activityHandler->isBlockedByStage($activity) &&
                            !$activityHandler->isCompleted($activity)) {
                            if (!$activityHandler->isStatusReadOnly($activity)) {
                                $this->cancelActivity($activity, $stage, $activityHandler, $activityHelper, $activityHookHelper, $journey);
                            } else {
                        ...
                        foreach ($childActivities as $childActivity) {
                            ...
                            if (!$childActivityHandler->isBlocked($childActivity) && !$childActivityHandler->isBlockedByStage($childActivity)) {
                                if (!$childActivityHandler->isStatusReadOnly($childActivity)) {
                                    $childActivity->cancelling_smart_guide = isset($journey->cancelling_smart_guide) ? true : false;
                                    $activityOldStatus = $childActivity->status;

                                    // we want for completed tasks to remain that way
                                    if ($childActivityHandler->isCompleted($childActivity) === false) {
            ...
            foreach ($blockedByStage as $id => $activity) {
                if (!$activityHandler->isStatusReadOnly($activity)) {
                    ...
                    $activity->cancelling_smart_guide = isset($journey->cancelling_smart_guide) ? true : false;
            ...
            if (!$isCompleted && !isset($statusReadOnlyActivityStages[$stage->id]) && !isset($activityChangeNotAllowedStages[$stage->id])) {
                ...
                JourneyHooks::setInternalSave(true);
                $stage->save();
            }
        }

        JourneyHooks::setInternalSave(false);
    ...
    /**
     * Cancel the activity by setting the status to Not Applicable
     * @param mixed $activity
     * @param mixed $stage
     * @param mixed $activityHandler
     * @param mixed $activityHelper
     * @param mixed $activityHookHelper
     * @return void
     */
    protected function cancelActivity($activity, $stage, $activityHandler, $activityHelper, $activityHookHelper, $journey)
    {
        $activity->cancelling_smart_guide = isset($journey->cancelling_smart_guide) ? true : false;
        $activityOldStatus = $activity->status;
        if ($activityHandler->isCompleted($activity) === false) {
            $activityHandler->setStatus($activity, $activityHandler->getNotApplicableStatus($activity));
        }
        $activity->processing_smart_guide = true;
        $activityHookHelper->validateAllowedBy($activity, $activityHandler);
