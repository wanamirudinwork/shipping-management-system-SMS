...
trait SummaryDetailsHelper
{
    ...
    protected function getSummaryNextRow(\Report $report)
    {
        ...
        if (is_array($nextRow) && array_key_exists('cells', $nextRow) && is_array($nextRow['cells'])) {
            $nextRow['cells'] = ReportUtils::formatRowData($report, $nextRow['cells']);
        }
    ...
    private function createColumnRow(\Report $report): array
    {
        ...
        for ($j = 0; $j < $report->current_summary_row_count; $j++) {
            ...
            if ($columnRow !== 0) {
                ...
                $columnRowData[] = ReportUtils::formatRowData($report, $columnRow);
            }
        }
