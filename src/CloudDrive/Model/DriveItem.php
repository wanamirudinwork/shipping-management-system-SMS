...
use Microsoft\Graph\Generated\Models\DriveItem as ModelDriveItem;
...
class DriveItem
{
     ...
     * @return DriveItem
     */
    public static function fromSharepoint($data): DriveItem
    {
        $isFolder = false;
        $name = $data->getName();

        // Handle Site type
        if ($data instanceof \Microsoft\Graph\Generated\Models\Site) {
            $name = $data->getDisplayName();
            $isFolder = true;
        }

        // Handle Drive type
        if ($data instanceof \Microsoft\Graph\Generated\Models\Drive) {
            $isFolder = $data->getDriveType() === 'documentLibrary';
        }

        // Handle DriveItem type
        if ($data instanceof \Microsoft\Graph\Generated\Models\DriveItem) {
            $isFolder = (bool)$data->getFolder();
            if ($remoteItem = $data->getRemoteItem()) {
                $data = $remoteItem;
            }
        }
        ...
        if ($parentReference = $data->getParentReference()) {
            $item->setDriveId($parentReference->getDriveId());
            $item->setParents($parentReference->getId());
        }

        // Determine if the item is good for DocuSign
        $item->setGoodForDocuSign(self::isGoodForDocuSign($data, $name));

        // Handle Ownership and Metadata
        $user = $data->getCreatedBy()?->getUser();
        $item->setOwners([$user]);
        $item->setWebViewLink($data->getWebUrl());

        if ($lastModified = $data->getLastModifiedDateTime()) {
            $item->setDateModified($lastModified->format('c'));
        }
        $item->setDownloadUrl($item->downloadUrl);
