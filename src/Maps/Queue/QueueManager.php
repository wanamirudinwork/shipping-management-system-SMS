...
class QueueManager
{
    ...
    /**
     * Create consumer job for given identifier
     *
     * @param string $identifier
     * @param string $consumerClass
     */
    public function createConsumer(string $identifier, string $consumerClass)
    {
        ...
        foreach ([SchedulersJob::JOB_STATUS_QUEUED, SchedulersJob::JOB_STATUS_RUNNING] as $status) {
            ...
            if (!empty($result)) {
                $this->getLogger()->info(sprintf('%s consumer for %s already present', $clientName, $identifier));
                return;
            }
        }

        // No job is found for this identifier, let's create one.
        ...
        $this->getLogger()->info(sprintf('Create %s consumer for %s', $clientName, $identifier));
    }
