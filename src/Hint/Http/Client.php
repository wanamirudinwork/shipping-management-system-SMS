...
class Client implements LoggerAwareInterface
{
    ...
    public function getHintBackendHostGeo()
    {
        ...
        // Set region to EU_WEST
        if (in_array($geoRegion, self::HINT_EU_WEST_AWS_REGIONS, true)) {
            return self::EU_WEST_REGION;
        }
    ...
    private function processRequest($request, $customOpts = null, $suppressNotFoundLog = false)
    {
        ...
        switch ($response->getCode()) {
            ...
            case 201:
                return 201;
            default:
                $this->logger->warning('Unexpected HTTP return status code: ' . $response->getCode());
                $this->logger->warning('Error: ' . print_r($requestInfo, true));
                $this->logger->warning('Error cause: ' . print_r($body, true));
