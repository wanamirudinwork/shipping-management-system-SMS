...
class SugarBean
{
    ...
    */
    ...
    public function save($check_notify = false)
    {
        ...
        $interactionsHelper = new InteractionsHelper($this);
        $interactionsHelper->saveInteraction();
    ...
    private function saveData($isUpdate, $check_notify)
    {
        ...
        if (!empty($this->last_parent_id) && $this->parent_id !== $this->last_parent_id && static::enterOperation('updating_relationships')) {
            $interactionsHelper = new InteractionsHelper($this);
            $interactionsHelper->deleteInteraction($this->last_parent_id);
        ...
        $auditQueueBefore = $this->auditedStateQueue;
        $this->call_custom_logic('before_save', [
            'check_notify' => $check_notify,
            'isUpdate' => $isUpdate,
        ]);
        $this->syncAuditStateQueueChanges($auditQueueBefore);
    ...
    /**
     * Sync audit log to avoid redundant intermediate value changes, like: null -> "-" -> "name".
     * This is useful for:
     * 1. calculated fields functionality
     * 2. logic hooks
     */
    protected function syncAuditStateQueueChanges(array $auditQueueBefore): void
    {
        $auditQueueAfter = $this->auditedStateQueue;
        if (!empty($auditQueueBefore) && count($auditQueueAfter) > count($auditQueueBefore)) {
            $previousQueueIndex = count($auditQueueBefore) - 1;
            $changes = array_pop($this->auditedStateQueue)['changes'];

            foreach ($changes as $field => $item) {
                // this field added after the updateCalculatedFields call, skip it
                if (!isset($this->auditedStateQueue[$previousQueueIndex]['changes'][$field])) {
                    continue;
                }
                // audit log should have final value only, avoid adding intermediate values. Example: set name=1,
                // then updateCalculatedFields updates it using formula. In this case audit log should have a single
                // record, not two.
                $this->auditedStateQueue[$previousQueueIndex]['changes'][$field]['after'] = $item['after'];
            }
        }
    }
    ...
    public function fetchFromQuery(SugarQuery $query, array $fields = [], array $options = [])
    {
        ...
        //cloning original query to preserve where, joins and other properties for consistency
        $originalQuery = clone $query;

        //after this, all the joins are added to the $query object
        $query->select($queryFields);
        if (DBManager::isMysql($query->getDBManager()) && safeCount($query->join) > MysqlManager::MYSQL_MAX_JOINS) {
            $safeJoins = min($options['safeJoins'] ?? MysqlManager::MYSQL_MAX_JOINS, MysqlManager::MYSQL_MAX_JOINS);
            $rowSets = [];
            $queries = [];
            $queries[] = clone $originalQuery;
            $currentQuery = 0;
            foreach ($queryFields as $field) {
                //instead of one $query with 61+ joins let's run multiple $queries with "safe" number of joins
                //add fields one by one to the smaller query, this will add joins if needed (for relate fields for example)
                $queries[$currentQuery]->select([$field]);
                if (safeCount($queries[$currentQuery]->join) >= $safeJoins) {
                    //if the last field that was added to the query added more joins than we expect - run that query
                    $rowSets[$currentQuery] = $this->getRowsFromQuery($queries[$currentQuery], $options, $fields);
                    $currentQuery++;
                    //create a new query object from $originalQuery to fetch more fields
                    $queries[$currentQuery] = clone $originalQuery;
                }
            }
            //the last query did not overflow the number of joins, meaning that it was not executed
            if (!array_key_exists($currentQuery, $rowSets)) {
                $rowSets[$currentQuery] = $this->getRowsFromQuery($queries[$currentQuery], $options, $fields);
            }
            //merge the rows from all smaller queries into one $rows array
            $rows = array_merge(...$rowSets);
            return $this->buildBeansFromRows($additionalQueryFields, $rows, $options, $beans, $secondaryFields, $fields);
        }

        $rows = $this->getRowsFromQuery($query, $options, $fields);
        return $this->buildBeansFromRows($additionalQueryFields, $rows, $options, $beans, $secondaryFields, $fields);
    }

    /**
     * @param mixed $additionalQueryFields
     * @param array $rows
     * @param array $options
     * @param array $beans
     ...
     * @return array
     */
    private function buildBeansFromRows(mixed $additionalQueryFields, array $rows, array $options, array $beans, array $secondaryFields, array $fields): array
    {
     ...
     * @return array
     */
    private function getRowsFromQuery(SugarQuery $query, array $options, array $fields): array
    {
    ...
    {
        $interactionsHelper = new InteractionsHelper($this);
        $interactionsHelper->deleteInteraction();
